...
const isAgDependency = (dependency) =>
  dependency.startsWith('ag-') || dependency.startsWith('@ag-');
const gridDependency = (dependency) =>
  dependency.startsWith('ag-grid') || dependency.startsWith('@ag-grid');
const chartDependency = (dependency) => dependency.startsWith('ag-charts');

const getAgDependencies = (packageJson) =>
  packageJson.dependencies
    ? Object.keys(packageJson.dependencies).filter(isAgDependency)
    : [];
const getAgDevDependencies = (packageJson) =>
  packageJson.devDependencies
    ? Object.keys(packageJson.devDependencies).filter(isAgDependency)
    : [];
const getAgPeerDependencies = (packageJson) =>
  packageJson.peerDependencies
    ? Object.keys(packageJson.peerDependencies).filter(isAgDependency)
    : [];
...
const extractSubAngularProjectDependencies = (packageDirectory, directory) => {
  ...
  if (directory.includes('angular') && !directory.includes('example')) {
    ...
    agSubAngularGridDeps = getAgPeerDependencies(
      currentSubProjectPackageJsonFile,
    )
      .filter(gridDependency)
      .map((dependency) => ({
        [dependency]:
          currentSubProjectPackageJsonFile.peerDependencies[dependency],
      }))
      .reduce((accumulator, current) => {
        const dependency = Object.keys(current)[0];
        accumulator[dependency] = current[dependency];
    ...
    agSubAngularChartDeps = getAgPeerDependencies(
      currentSubProjectPackageJsonFile,
    )
...
const getPackageInformation = () => {
  ...
  packages.forEach((packageDirectory) => {
    fs.readdirSync(packageDirectory).forEach((directory) => {
      ...
      const dependencies = getAgDependencies(projectPackageJson).filter(
        (dependency) => dependency !== 'ag-grid-testing',
      );
      const devDependencies = getAgDevDependencies(projectPackageJson).filter(
        (dependency) => dependency !== 'ag-grid-testing',
      );

      const agGridDeps = extractDependencies(
        projectPackageJson,
        dependencies,
        devDependencies,
        gridDependency,
      );
      const agChartDeps = extractDependencies(
        projectPackageJson,
        dependencies,
        devDependencies,
        chartDependency,
      ...
      } = extractSubAngularProjectDependencies(packageDirectory, directory);
