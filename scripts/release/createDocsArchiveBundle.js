...
const LATEST_HASH = require('child_process')
  .execSync('cat .git/refs/heads/latest')
  .toString()
  .trim();
const LATEST_HASH_TIMESTAMP = require('child_process')
  .execSync(`git show -s --format=%ci ${LATEST_HASH}`)
  .toString()
  .trim();

if (process.argv.length !== 3) {
  console.log(
    'Usage: node scripts/release/createDocsArchiveBundle.js [Version Number]',
  );
  console.log(
    'For example: node scripts/release/createDocsArchiveBundle.js 19.1.0',
  );
...
function getArchiveFilename() {
  const now = new Date();
  return `archive_${now.getFullYear()}${
    now.getMonth() + 1
  }${now.getDate()}_${newVersion}.zip`;
}
...
function isDocHeaderPhp(filename) {
  return (
    filename === `${DIST_PATH}/documentation-main/documentation_header.php`
  );
}
...
function preDocHeader(fileContents, latestHash, latestHashTimestamp) {
  ...
  fileContents = `${fileContents.substr(
    0,
    startIndex,
  )}${newVersion}${fileContents.substr(endIndex, fileContents.length)}`;
  ...
  fileContents = `${fileContents.substr(
    0,
    startIndex,
  )}${latestHash}${fileContents.substr(endIndex, fileContents.length)}`;
  ...
  fileContents = `${fileContents.substr(
    0,
    startIndex,
  )}${latestHashTimestamp}${fileContents.substr(
    endIndex,
    fileContents.length,
  )}`;
