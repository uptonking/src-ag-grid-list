...
if (process.argv.length < 5) {
  console.log(
    'Usage: node scripts/release/versionModules.js [New Version] [Dependency Version] [package directories] [charts version]',
  );
  console.log(
    'For example: node scripts/release/versionModules.js 19.1.0 ^19.1.0 \'["charts-packages", "examples-charts"]\' 1.0.0',
  );
...
function updateLernaJson() {
  fs.readFile(LERNA_JSON, 'utf8', (err, contents) => {
    ...
    fs.writeFileSync(LERNA_JSON, JSON.stringify(copyOfFile, null, 2), 'utf8');
  });
}
...
function updateDependency(
  ...
  Object.entries(dependencyContents)
    .filter(([key, value]) => {
      return gridDependenct(key) || chartDependency(key);
    })
    .filter(([key, value]) => {
    ...
    .forEach(([key, value]) => {
      if (chartsDependencyVersion) {
        dependencyContents[key] = chartDependency(key)
          ? chartsDependencyVersion
          : dependencyVersion;
      } else {
