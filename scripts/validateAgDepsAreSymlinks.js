...
packageNames.forEach((packageName) => {
  ...
  const agDependencies = Object.keys(agPackage.agGridDeps).concat(
    Object.keys(agPackage.agChartDeps),
  );
  ...
  agDependencies.forEach((agDependency) => {
    ...
    if (!fs.existsSync(nodeModuleDirPath)) {
      console.log(
        `${agDependency} is listed as a dependency of ${packageName} (${projectRoot}) but isn't under node_modules`,
      );
    } else {
      const nodeModuleDir = fs.lstatSync(nodeModuleDirPath);
      if (!nodeModuleDir.isSymbolicLink()) {
        console.log(
          `${nodeModuleDirPath} is not a symbolic link - check ag version in ${projectRoot}/package.json`,
        );
      }
    }
  });
