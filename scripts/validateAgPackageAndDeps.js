...
if (process.argv.length < 6) {
  console.log(
    'Usage: node scripts/sanityCheckPackages.js [Grid Version] [Grid Dependency Version] [Chart Version] [Chart Dependency Version]',
  );
  console.log(
    'For example: node scripts/validateAgPackageAndDeps.js 23.0.0 ~23.0.0 1.0.0 ~1.0.0',
  );
...
const [
  ...
  scriptPath,
  gridNewVersion,
  gridDependencyVersion,
  chartNewVersion,
  chartDependencyVersion,
] = process.argv;
...
packageNames.forEach((packageName) => {
  ...
  checkDependency(
    packageName,
    version,
    isGridPackage ? gridNewVersion : chartNewVersion,
  );
  Object.keys(agPackage.agGridDeps).forEach((dependencyName) =>
    checkDependency(
      dependencyName,
      agGridDeps[dependencyName],
      gridDependencyVersion,
    ),
  );
  ...
  if (agSubAngularGridDeps) {
    Object.keys(agSubAngularGridDeps).forEach((dependencyName) =>
      checkDependency(
        dependencyName,
        agSubAngularGridDeps[dependencyName],
        gridDependencyVersion,
      ),
    );
  }
  if (agSubAngularChartDeps) {
    Object.keys(agSubAngularChartDeps).forEach((dependencyName) =>
      checkDependency(
        dependencyName,
        agSubAngularChartDeps[dependencyName],
        chartDependencyVersion,
      ),
    );
  }
