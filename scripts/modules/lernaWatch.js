...
const buildDependencies = async (dependencies, command = 'build-cjs') => {
  ...
  console.log(
    `Building ${dependencies
      .map((dependency) => `--scope ${dependency}`)
      .join(' ')}`,
  );
  console.log(
    '------------------------------------------------------------------------------------------',
  ...
  const scopedDependencies = dependencies
    .map((dependency) => `--scope ${dependency}`)
    .join(' ');
...
const spawnWatcher = async ({ paths, buildChains }, singleModule) => {
  ...
  const packageName = (changeFile) =>
    manifest(findParentPackageManifest(changeFile)).name;
...
const spawnCssWatcher = async ({ paths, buildChains }) => {
  if (process.env.AG_NO_CSS) {
    console.log(
      'Disabling CSS watching - manually launch sass-native-watch.sh instead',
    );
    return;
  }
...
const filterAgGridOnly = (dependencyTree) => {
  ...
  agRoots.forEach((root) => {
    prunedDependencyTree[root] = dependencyTree[root]
      ? dependencyTree[root].filter((dependency) => dependency.includes('@ag-'))
      : [];
  });
  return prunedDependencyTree;
};

const buildBuildTree = (
  startingPackage,
  dependencyTree,
  dependenciesOrdered,
) => {
  ...
  dependenciesOrdered.forEach((dependency) => {
    ...
    remainingPackages.forEach((remainingPackage) => {
      dependencyTree[remainingPackage] = dependencyTree[
        remainingPackage
      ].filter((packageDependency) => packageDependency !== dependency);
...
const excludePackage = (packageName) =>
  !exclude.includes(packageName) && !packageName.includes('-example');
...
const getOrderedDependencies = async (packageName) => {
  ...
  dependenciesOrdered = dependenciesOrdered.filter((dependency) =>
    excludePackage(dependency.name),
  );

  const paths = dependenciesOrdered.map((dependency) => dependency.location);
  const orderedPackageNames = dependenciesOrdered.map(
...
const watch = async (singleModule = false) => {
  const cacheFilePath = path.resolve(
    __dirname,
    '../../.lernaBuildChain.cache.json',
  );
  if (!fs.existsSync(cacheFilePath)) {
    const { paths, orderedPackageNames } = await getOrderedDependencies(
      '@ag-grid-community/core',
    );

    const buildChains = {};
    for (let packageName of orderedPackageNames) {
      buildChains[packageName] = await generateBuildChain(
        packageName,
        orderedPackageNames,
      );
    }
