 ...
 * @param names
 */
export const formatRe = (names: string[]) =>
  new RegExp('^(?:' + names.map(requote).join('|') + ')', 'i');
...
export default function formatLocale(
    ...
    return function (str: string) {
      ...
      if ('V' in d) {
        ...
        if ('Z' in d) {
          ...
          week =
            day > 4 || day === 0 ? utcMonday.ceil(week) : utcMonday.floor(week);
  ...
  function parseSpecifier(
    ...
    while (i < n) {
      ...
      if (code === percentCharCode) {
        ...
        const parse =
          parses[(char in pads ? specifier.charAt(i++) : char) as FormatKeys];
  ...
  function formatWeekNumberISO(date: Date, fill: string): string {
    ...
    date =
      day >= 4 || day === 0
        ? timeThursday.floor(date)
        : timeThursday.ceil(date);
    const yearStart = year.floor(date);
    return pad(
      timeThursday.count(yearStart, date) + (yearStart.getDay() === 4 ? 1 : 0),
      fill,
      2,
    );
  }
  ...
  function formatUTCWeekNumberISO(date: Date, fill: string): string {
    ...
    date =
      day >= 4 || day === 0 ? utcThursday.floor(date) : utcThursday.ceil(date);
    const yearStart = utcYear.floor(date);
    return pad(
      utcThursday.count(yearStart, date) +
        (yearStart.getUTCDay() === 4 ? 1 : 0),
      fill,
      4,
    );
  }
  ...
  function parseMicroseconds(d: ParsedDate, string: string, i: number): number {
    ...
    return n
      ? ((d.L = Math.floor(parseFloat(n[0]) / 1000)), i + n[0].length)
      : -1;
  }
  ...
  function parseWeekNumberSunday(
    d: ParsedDate,
    string: string,
    i: number,
  ): number {
  ...
  function parseWeekdayNumberSunday(
    d: ParsedDate,
    string: string,
    i: number,
  ): number {
