    ...
    for (const key in options) {
      ...
      if (skipKeys.indexOf(key) < 0) {
        ...
        if (
          value &&
          key in mapping &&
          !(meta.setAsIs && meta.setAsIs.indexOf(key) >= 0)
        ) {
          if (Array.isArray(value)) {
            const subComponents = value
              .map((config) => create(config, path + '.' + key))
      ...
      if (skipKeys.indexOf(key) < 0) {
        ...
        if (meta.setAsIs && meta.setAsIs.indexOf(key) >= 0) {
          ...
          if (Array.isArray(oldValue) && Array.isArray(value)) {
              ...
              if (key === 'series') {
                ...
                for (const config of configs) {
                  const axisToUpdate = find(axes, (axis) => {
                    return (
                      axis.type === config.type &&
                      axis.position === config.position
                    );
                  });
                  if (axisToUpdate) {
            ...
            const subComponent =
              isObject(value) && create(value, value.type ? path : keyPath);
            if (subComponent) {
              component[key] = subComponent;
            } else {
              component[key] = value;
