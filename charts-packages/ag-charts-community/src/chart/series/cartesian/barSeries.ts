...
export class BarSeries extends CartesianSeries {
  ...
  getKeys(direction: ChartAxisDirection): string[] {
    ...
    const keys =
      directionKeys &&
      directionKeys[
        this.flipXY ? flipChartAxisDirection(direction) : direction
      ];
    const values: string[] = [];
    ...
    xData.forEach((category, i) => {
      ...
      yDatum.forEach((curr, j) => {
        ...
        if (labelFormatter) {
          ...
          labelText =
            yValueIsNumber && isFinite(yValue) ? yValue.toFixed(2) : '';
        }
          ...
          label:
            seriesItemEnabled.get(yKey) && labelText
              ? {
                  ...
                  fill: labelColor,
                  x: flipXY
                    ? y + ((yValue >= 0 ? -1 : 1) * Math.abs(bottomY - y)) / 2
                    : barX + barWidth / 2,
                  y: flipXY
                    ? barX + barWidth / 2
                    : y + ((yValue >= 0 ? 1 : -1) * Math.abs(bottomY - y)) / 2,
