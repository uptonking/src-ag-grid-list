...
const aggregationFunctions: { [key in AggregationName]: AggregationFunction } =
  {
    ...
    mean: (bin, yKey) =>
      aggregationFunctions.sum(bin, yKey) /
      aggregationFunctions.count(bin, yKey),
  };
...
export class HistogramSeries extends CartesianSeries {
  ...
  private rectSelection: Selection<Rect, Group, any, any> = Selection.select(
    this.rectGroup,
  ).selectAll<Rect>();
  private textSelection: Selection<Text, Group, any, any> = Selection.select(
    this.textGroup,
  ).selectAll<Text>();
  ...
  private deriveBins(): [number, number][] {
    ...
    if (bins && binCount) {
      console.warn(
        'bin domain and bin count both specified - these are mutually exclusive properties',
      );
    }
    ...
    const binStarts = ticks(
      xDomain[0],
      xDomain[1],
      this.binCount || defaultBinCount,
    );
    const binSize = tickStep(
      xDomain[0],
      xDomain[1],
      this.binCount || defaultBinCount,
    );
    ...
    const expandStartToBin: (n: number) => [number, number] = (n) => [
      n,
      n + binSize,
    ];
  ...
  private placeDataInBins(data: any[]): HistogramBin[] {
    ...
    bins.forEach((b) =>
      b.calculateAggregatedValue(this._aggregation, this.yKey),
    );
  ...
  private generateNodeData(): HistogramNodeDatum[] {
    ...
    const defaultLabelFormatter = (b: HistogramBin) =>
      String(b.aggregatedValue);
    const {
      label: {
  ...
  getTooltipHtml(nodeDatum: HistogramNodeDatum): string {
    ...
    if (tooltipRenderer) {
      ...
      let contentHtml = yKey
        ? `<b>${yName || yKey} (${aggregation})</b>: ${toFixed(
            aggregatedValue,
          )}<br>`
        : '';
