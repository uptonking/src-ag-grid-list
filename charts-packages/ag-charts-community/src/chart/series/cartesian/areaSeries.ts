...
export class AreaSeries extends CartesianSeries {
  ...
  private areaSelection: Selection<Path, Group, AreaSelectionDatum, any> =
    Selection.select(this.areaGroup).selectAll<Path>();
  private strokeSelection: Selection<Path, Group, AreaSelectionDatum, any> =
    Selection.select(this.strokeGroup).selectAll<Path>();
  private markerSelection: Selection<Marker, Group, any, any> =
    Selection.select(this.markerGroup).selectAll<Marker>();
  ...
  update(): void {
    ...
    const { areaSelectionData, markerSelectionData } =
  ...
  private generateSelectionData(): {
    ...
    xData.forEach((xDatum, i) => {
      ...
      yDatum.forEach((curr, j) => {
        ...
        const areaDatum =
          areaSelectionData[j] || (areaSelectionData[j] = { yKey, points: [] });
  ...
  private updateAreaSelection(areaSelectionData: AreaSelectionDatum[]): void {
    ...
    const enterAreas = updateAreas.enter.append(Path).each((path) => {
  ...
  private updateStrokeSelection(areaSelectionData: AreaSelectionDatum[]): void {
    const { strokes, strokeWidth, strokeOpacity, data, seriesItemEnabled } =
      this;
  ...
  private updateMarkerNodes(): void {
    ...
    const markerStrokeWidth =
      marker.strokeWidth !== undefined ? marker.strokeWidth : this.strokeWidth;
    const markerSize = marker.size;
    const { xKey, seriesItemEnabled } = this;
    const { highlightedDatum } = this.chart;
    const { fill: highlightFill, stroke: highlightStroke } =
      this.highlightStyle;

    this.markerSelection.each((node, datum) => {
      const highlighted = datum === highlightedDatum;
      const markerFill =
        highlighted && highlightFill !== undefined
          ? highlightFill
          : marker.fill || datum.fill;
      const markerStroke =
        highlighted && highlightStroke !== undefined
          ? highlightStroke
          : marker.stroke || datum.stroke;
      ...
      node.strokeWidth =
        markerFormat && markerFormat.strokeWidth !== undefined
          ? markerFormat.strokeWidth
          : markerStrokeWidth;
      node.size =
        markerFormat && markerFormat.size !== undefined
          ? markerFormat.size
          : markerSize;
      ...
      node.visible =
        marker.enabled && node.size > 0 && !!seriesItemEnabled.get(datum.yKey);
    });
  }
  ...
  getTooltipHtml(nodeDatum: MarkerSelectionDatum): string {
    ...
    if (tooltipRenderer) {
      ...
      const titleString = yName
        ? `<div class="${Chart.defaultTooltipClass}-title" ${titleStyle}>${yName}</div>`
        : '';
      const seriesDatum = nodeDatum.seriesDatum;
      const xValue = seriesDatum[xKey];
      const yValue = seriesDatum[yKey];
      const xString =
        typeof xValue === 'number' ? toFixed(xValue) : String(xValue);
      const yString =
        typeof yValue === 'number' ? toFixed(yValue) : String(yValue);
