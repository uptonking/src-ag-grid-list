...
export interface ScatterTooltipRendererParams
  extends CartesianTooltipRendererParams {
  sizeKey?: string;
  sizeName?: string;
...
export class ScatterSeries extends CartesianSeries {
  ...
  private nodeSelection: Selection<Group, Group, ScatterNodeDatum, any> =
  ...
  getTooltipHtml(nodeDatum: ScatterNodeDatum): string {
    ...
    if (tooltipRenderer) {
      ...
      let contentHtml =
        `<b>${xName || xKey}</b>: ${
          typeof xValue === 'number' ? toFixed(xValue) : xValue
        }` +
        `<br><b>${yName || yKey}</b>: ${
          typeof yValue === 'number' ? toFixed(yValue) : yValue
        }`;
