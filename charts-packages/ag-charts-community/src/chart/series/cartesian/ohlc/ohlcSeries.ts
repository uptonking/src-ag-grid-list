...
export class OHLCSeries extends CartesianSeries {
  ...
  private groupSelection: Selection<Group, Group, GroupSelectionDatum, any> =
  ...
  processData(): boolean {
    ...
    const data =
      dateKey && openKey && highKey && lowKey && closeKey && this.data
        ? this.data
        : [];

    if (dirtyDateData) {
      this.xDomain = this.calculateDomain(
        (this.dateData = data.map((d) => d[dateKey])),
      );
    ...
    if (dirtyOpenData || dirtyHighData || dirtyLowData || dirtyCloseData) {
      const yDomains = new Array<any>().concat(
        this.openData,
        this.highData,
        this.lowData,
        this.closeData,
  ...
  update(): void {
    ...
    const groupSelectionData: GroupSelectionDatum[] = dateData.map(
      (dateDatum, i) => {
    ...
    groupSelection.selectByClass(Marker).each((node, datum) => {
      const highlighted =
        highlightedDatum &&
      ...
      node.fill =
        highlighted && highlightFill !== undefined ? highlightFill : datum.fill;
      node.stroke =
  ...
  getTooltipHtml(nodeDatum: GroupSelectionDatum): string {
    ...
    if (tooltipRenderer) {
      ...
      let contentHtml =
        `<b>${dateName}</b>: ${this.dateFormatter(dateValue)}` +
      ...
      if (labelKey) {
        contentHtml =
          `<b>${labelName}</b>: ${seriesDatum[labelKey]}<br>` + contentHtml;
      }
  ...
  listSeriesItems(legendData: LegendDatum[]): void {
    const { data, id, dateKey, closeKey, title, visible } = this;
