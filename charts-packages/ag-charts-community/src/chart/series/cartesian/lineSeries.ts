...
export class LineSeries extends CartesianSeries {
  ...
  private nodeSelection: Selection<Group, Group, LineNodeDatum, any> =
    Selection.select(this.group).selectAll<Group>();
  ...
  protected onMarkerEnabledChange(
    event: PropertyChangeEvent<CartesianSeriesMarker, boolean>,
  ) {
    if (!event.value) {
      this.nodeSelection = this.nodeSelection.setData([]);
      this.nodeSelection.exit.remove();
  ...
  processData(): boolean {
    ...
    this.xDomain = isContinuousX
      ? this.fixNumericExtent(numericExtent(xData), 'x')
      : xData;
    this.yDomain = isContinuousY
      ? this.fixNumericExtent(numericExtent(yData), 'y')
      : yData;
  ...
  private updateLinePath() {
    ...
    xData.forEach((xDatum, i) => {
      ...
      const isGap =
        yDatum == null ||
        (isContinuousY && (isNaN(yDatum) || !isFinite(yDatum))) ||
        xDatum == null ||
        (isContinuousX && (isNaN(xDatum) || !isFinite(xDatum)));
  ...
  private updateNodes() {
    ...
    const markerStrokeWidth =
      marker.strokeWidth !== undefined ? marker.strokeWidth : strokeWidth;
  ...
  getTooltipHtml(nodeDatum: LineNodeDatum): string {
    ...
    if (tooltipRenderer) {
      ...
      const titleString = title
        ? `<div class="${Chart.defaultTooltipClass}-title" ${titleStyle}>${title}</div>`
        : '';
