...
export class PieSeries extends PolarSeries {
  ...
  private groupSelection: Selection<Group, Group, PieNodeDatum, any> =
  ...
  set fills(values: string[]) {
    this._fills = values;
    this.strokes = values.map((color) =>
      Color.fromString(color).darker().toHexString(),
    );
    this.scheduleData();
  }
  get fills(): string[] {
  ...
  processData(): boolean {
    ...
    const angleData: number[] = data.map(
      (datum, index) =>
        (seriesItemEnabled[index] && Math.abs(+datum[angleKey])) || 0,
    );
    ...
    const labelData = labelKey
      ? data.map((datum) => String(datum[labelKey]))
      : [];
    ...
    angleDataRatios.reduce((start, end) => {
      ...
      if (
        midAngle180 >= quadrantStart &&
        midAngle180 < (quadrantStart += halfPi)
      ) {
        textAlign = 'center';
        textBaseline = 'bottom';
      } else if (
  ...
  update(): void {
    ...
    const visible = (this.group.visible =
      this.visible && this.seriesItemEnabled.indexOf(true) >= 0);
  ...
  private updateNodes() {
    ...
    this.groupSelection
      ...
      .each((sector, datum, index) => {
        ...
        sector.stroke =
          highlighted && stroke !== undefined
            ? stroke
            : strokes[index % strokes.length];
        sector.fillOpacity = fillOpacity;
        sector.strokeOpacity = strokeOpacity;
        sector.centerOffset =
          highlighted && centerOffset !== undefined ? centerOffset : 0;
    ...
    this.groupSelection
      .selectByTag<Line>(PieNodeTag.Callout)
    ...
    {
      const { offset, fontStyle, fontWeight, fontSize, fontFamily, color } =
        this.label;

      this.groupSelection
        .selectByTag<Text>(PieNodeTag.Label)
  ...
  getTooltipHtml(nodeDatum: PieNodeDatum): string {
    ...
    if (tooltipRenderer) {
      ...
      const titleString = title
        ? `<div class="${Chart.defaultTooltipClass}-title" ${titleStyle}>${text}</div>`
        : '';
      const label = labelKey ? `${nodeDatum.seriesDatum[labelKey]}: ` : '';
      const value = nodeDatum.seriesDatum[angleKey];
      const formattedValue =
        typeof value === 'number' ? toFixed(value) : value.toString();
