...
export abstract class Chart extends Observable {
  ...
  private onCaptionChange(
    event: PropertyChangeEvent<this, Caption | undefined>,
  ) {
  ...
  set axes(values: ChartAxis[]) {
    ...
    this._axes = values
      .filter((a) => !a.linkedTo)
      .concat(values.filter((a) => a.linkedTo));
    ...
    if (canAdd) {
      ...
      if (afterIndex >= 0) {
        if (afterIndex + 1 < allSeries.length) {
          seriesRoot.insertBefore(
            series.group,
            allSeries[afterIndex + 1].group,
          );
        } else {
          seriesRoot.append(series.group);
        }
  ...
  protected assignAxesToSeries(force: boolean = false) {
    ...
    this.axes.forEach((axis) => {
      ...
      const directionAxes =
        directionToAxesMap[direction] || (directionToAxesMap[direction] = []);
    ...
    this.series.forEach((series) => {
      ...
      if (series instanceof CartesianSeries) {
        if (!series.xAxis) {
          console.warn(
            `Could not find a matching xAxis for the ${series.id} series.`,
          );
          return;
        }
        if (!series.yAxis) {
          console.warn(
            `Could not find a matching yAxis for the ${series.id} series.`,
          );
          return;
        }
      }
  ...
  processData(): void {
    ...
    this.series
      .filter((s) => s.visible)
      .forEach((series) => series.processData());
    this.updateLegend();
  ...
  private updateLegend() {
    ...
    this.series
      ...
      .forEach((series) => series.listSeriesItems(legendData));
  ...
  protected positionLegend() {
    ...
    switch (legend.position) {
      ...
      case 'left':
        ...
        translationY =
          captionAutoPadding + (height - legendBBox.height) / 2 - legendBBox.y;
    ...
    legendGroup.translationX = Math.floor(
      translationX + legendGroup.translationX,
    );
    legendGroup.translationY = Math.floor(
      translationY + legendGroup.translationY,
    );
  }
  ...
  protected onMouseMove(event: MouseEvent) {
    ...
    if (tooltipTracking) {
      const closestDatum = this.pickClosestSeriesNodeDatum(
        event.offsetX,
        event.offsetY,
      );
      if (closestDatum && closestDatum.point) {
        ...
        this.onSeriesDatumPick(
          ...
          closestDatum,
          nodeDatum === closestDatum ? (pick.node as Shape) : undefined,
        );
      } else {
        hideTooltip = true;
      }
  ...
  private onSeriesDatumPick(
    ...
    const html =
      datum.series.tooltipEnabled && datum.series.getTooltipHtml(datum);
