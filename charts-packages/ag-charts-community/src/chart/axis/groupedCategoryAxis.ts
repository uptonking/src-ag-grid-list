...
export class GroupedCategoryAxis extends ChartAxis {
  ...
  private resizeTickTree() {
    const s = this.scale;
    const range = s.domain.length
      ? [s.convert(s.domain[0]), s.convert(s.domain[s.domain.length - 1])]
      : s.range;
  ...
  update() {
    ...
    const updateGridLines = this.gridLineSelection.setData(
      this.gridLength ? ticks : [],
    );
    ...
    labelSelection.each((label, datum, index) => {
      ...
      if (!datum.children.length) {
        ...
        label.translationX -=
          maxLeafLabelWidth - lineHeight + this.label.padding;
        if (isHorizontal) {
    ...
    separatorSelection.each((line, datum, i) => {
      ...
      line.y2 = datum.y;
      line.visible =
        datum.y >= requestedRange[0] - epsilon &&
        datum.y <= requestedRange[1] + epsilon;
    ...
    axisLineSelection.each((line, _, index) => {
      ...
      line.visible =
        labels.length > 0 && (index === 0 || (labelGrid && isLabelTree));
    });
