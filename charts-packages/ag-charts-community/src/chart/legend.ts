...
export class Legend extends Observable {
  ...
  private itemSelection: Selection<MarkerLabel, Group, any, any> =
    Selection.select(this.group).selectAll<MarkerLabel>();

  private oldSize: [number, number] = [0, 0];

  @reactive('layoutChange') data: LegendDatum[] = [];
  @reactive('layoutChange') enabled = true;
  @reactive('layoutChange') orientation: LegendOrientation =
  ...
  performLayout(width: number, height: number) {
    const { markerShape, layoutHorizontalSpacing, layoutVerticalSpacing } =
      this;
    const updateSelection = this.itemSelection.setData(
      this.data,
      (_, datum) => {
    ...
    const itemSelection = (this.itemSelection =
    ...
    switch (this.orientation) {
      case LegendOrientation.Horizontal:
        ...
        do {
          ...
          paddedItemsWidth =
            itemsWidth + (columnCount - 1) * layoutHorizontalSpacing;
        } while (paddedItemsWidth > width && columnCount > 1);

        paddedItemsHeight =
          itemHeight * rowCount + (rowCount - 1) * layoutVerticalSpacing;
      ...
      case LegendOrientation.Vertical:
        ...
        do {
          ...
          paddedItemsWidth =
            itemsWidth + (columnCount - 1) * layoutHorizontalSpacing;
          paddedItemsHeight =
            itemsHeight + (rowCount - 1) * layoutVerticalSpacing;
