...
} from './chart/groupedCategoryChart';
...
export class ChartBuilder {
  ...
  static createBarChart(
    ...
    if (options.series) {
      chart.series = options.series.map((s) =>
        ChartBuilder.initBarSeries(new BarSeries(), s),
      );
    }
  ...
  static createGroupedColumnChart(
    container: HTMLElement,
    options: CartesianChartOptions<BarSeriesOptions>,
  ): GroupedCategoryChart {
    const chart = this.createGroupedCategoryChart(
  ...
  static createLineChart(
    container: HTMLElement,
    options: CartesianChartOptions<LineSeriesOptions>,
  ): CartesianChart {
    const chart = this.createCartesianChart(
      container,
      ChartBuilder.createAxis(options.xAxis, 'category'),
    ...
    if (options.series) {
      chart.series = options.series.map((s) =>
        ChartBuilder.initLineSeries(new LineSeries(), s),
      );
    }
  ...
  static createScatterChart(
    container: HTMLElement,
    options: CartesianChartOptions<ScatterSeriesOptions>,
  ): CartesianChart {
    const chart = this.createCartesianChart(
      container,
      ChartBuilder.createAxis(options.xAxis, 'number'),
    ...
    if (options.series) {
      chart.series = options.series.map((s) =>
        ChartBuilder.initScatterSeries(new ScatterSeries(), s),
      );
    }
  ...
  static createAreaChart(
    container: HTMLElement,
    options: CartesianChartOptions<AreaSeriesOptions>,
  ): CartesianChart {
    const chart = this.createCartesianChart(
      container,
      ChartBuilder.createAxis(options.xAxis, 'category'),
    ...
    if (options.series) {
      chart.series = options.series.map((s) =>
        ChartBuilder.initAreaSeries(new AreaSeries(), s),
      );
    }
  ...
  static createDoughnutChart(
    container: HTMLElement,
    options: PolarChartOptions<PieSeriesOptions>,
  ): PolarChart {
  ...
  static createPieChart(
    ...
    if (options.series) {
      chart.series = options.series.map((s) =>
        ChartBuilder.initPieSeries(new PieSeries(), s),
      );
    }
  ...
  static initCartesianChart<C extends CartesianChart, T extends SeriesOptions>(
    chart: C,
    options: CartesianChartOptions<T>,
  ): C {
  ...
  static initChart<C extends Chart, T extends SeriesOptions>(
    chart: C,
    ...
    this.setTransformedValueIfExists(
      chart,
      'title',
      (t) => ChartBuilder.createTitle(t),
      options.title,
    );
    this.setTransformedValueIfExists(
      chart,
      'subtitle',
      (t) => ChartBuilder.createSubtitle(t),
      options.subtitle,
    );
    this.setTransformedValueIfExists(
      chart,
      'padding',
      (p) => new Padding(p.top, p.right, p.bottom, p.left),
      options.padding,
    );

    if (options.background) {
      this.setValueIfExists(chart.background, 'fill', options.background.fill);
      this.setValueIfExists(
        chart.background,
        'visible',
  ...
  static initLabelOptions(
    series: SeriesLabelOptions,
  ...
  static initBarSeries(
    ...
    this.setTransformedValueIfExists(
      series,
      'shadow',
      (s) => ChartBuilder.createDropShadow(s),
      options.shadow,
    );
  ...
  static initPieSeries(
    ...
    this.setTransformedValueIfExists(
      series,
      'title',
      (t) => ChartBuilder.createPieTitle(t),
      options.title,
    );
    ...
    this.setValueIfExists(
      series,
      'outerRadiusOffset',
      options.outerRadiusOffset,
    );
    this.setValueIfExists(
      series,
      'innerRadiusOffset',
      options.innerRadiusOffset,
    );

    const { field, fill, stroke, highlightStyle, callout, label, tooltip } =
      options;
  ...
  private static markerShapes: Map<MarkerShape, new () => Marker> =
    convertToMap([
      ['circle', Circle],
      ['cross', Cross],
      ['diamond', Diamond],
    ...
    if (item) {
      ...
      if (marker) {
        this.setValueIfExists(
          legend,
          'markerShape',
          ChartBuilder.getMarkerByName(marker.shape),
        );
  ...
  static initNavigatorMask(
    mask: NavigatorMask,
    options: NavigatorMaskOptions,
  ): void {
    if (options) {
  ...
  static initNavigatorHandle(
    handle: NavigatorHandle,
    options: NavigatorHandleOptions,
  ): void {
    if (options) {
  ...
  static initAxis<T extends NumberAxis | CategoryAxis | GroupedCategoryAxis>(
    axis: T,
    options: AxisOptions,
  ): void {
    this.setTransformedValueIfExists(
  ...
  ): CategoryAxis | NumberAxis | TimeAxis {
  ...
  static readonly types = (() => {
    const types = new Map<
      AxisType,
      typeof CategoryAxis | typeof NumberAxis | typeof TimeAxis
    >();
  ...
  private static setValueIfExists<T, K extends keyof T>(
    target: T,
    property: K,
    value?: T[K],
  ...
  private static setTransformedValueIfExists<T, K extends keyof T, V>(
    target: T,
    property: K,
