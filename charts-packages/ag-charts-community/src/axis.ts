  ...
  update() {
    ...
    groupSelection
      ...
      .attrFn('visible', function (node) {
        return (
          node.translationY >= requestedRangeMin &&
          node.translationY <= requestedRangeMax
        );
      });
    ...
    if (this.gridLength && gridStyle.length) {
      ...
      if (this.radialGrid) {
        const angularGridLength = normalizeAngle360Inclusive(
          toRadians(this.gridLength),
        );

        gridLines = groupSelection
          .selectByTag<Arc>(Tags.GridLine)
          .each((arc, datum) => {
        ...
        gridLines = groupSelection
          .selectByTag<Line>(Tags.GridLine)
          .each((line) => {
            line.x1 = 0;
            ...
            line.y2 = 0;
            line.visible =
              Math.abs(line.parent!.translationY - scale.range[0]) > 1;
          });
      }
    ...
    const fractionDigits =
      (ticks as any).fractionDigits >= 0 ? (ticks as any).fractionDigits : 0;

    const labelSelection = groupSelection
      .selectByClass(Text)
    ...
    if (title && title.enabled) {
      ...
      titleNode.x =
        (titleRotationFlag * sideFlag * (lineNode.y1 + lineNode.y2)) / 2;
      titleNode.x =
        (titleRotationFlag *
          sideFlag *
      ...
      if (sideFlag === -1) {
        titleNode.y =
          titleRotationFlag *
          (-padding - bbox.width + Math.max(bbox.x + bbox.width, 0));
      } else {
