...
type ValueFn<P, GDatum, PDatum> = (
  parent: P,
  data: PDatum,
  index: number,
  groups: (P | undefined)[],
) => GDatum[];
type KeyFn<N, G, GDatum> = (
  node: N,
  datum: GDatum,
  index: number,
  groups: (G | undefined)[],
) => string;
type NodeCallback<G, GDatum> = (
  node: G,
  datum: GDatum,
  index: number,
  group: (G | undefined)[],
) => void;
type NodeSelector<N, G, GDatum> = (
  node: G,
  datum: GDatum,
  index: number,
  group: (G | undefined)[],
) => N;
type NodeSelectorAll<N, G, GDatum> = (
  node: G,
  datum: GDatum,
  index: number,
  group: (G | undefined)[],
...
export class Selection<
  G extends Node | EnterNode,
  P extends Node | EnterNode,
  GDatum = any,
  PDatum = any,
> {
  ...
  static select<G extends Node, P extends Node | EnterNode>(
    node: G | (() => G),
  ) {
    return new Selection<G, P>(
   ...
   * @param creator
   */
  private appendFn<N extends Node>(
    creator: NodeSelector<N, G, GDatum>,
  ...
  selectAllByClass<N extends Node, NDatum = any>(
    Class: new () => N,
  ...
  selectAllByTag<N extends Node, NDatum = any>(
    tag: number,
  ): Selection<N, G, NDatum, GDatum> {
    return this.selectAll<N, NDatum>((node) => {
      const nodes: N[] = [];
  ...
  merge(
    other: Selection<G, P, GDatum, PDatum>,
  ...
  setData<GDatum>(
    ...
    for (let j = 0; j < numGroups; j++) {
      ...
      const enterGroup = (enterGroups[j] = new Array<EnterNode | undefined>(
        dataSize,
      ));
      const updateGroup = (updateGroups[j] = new Array<G | undefined>(
        dataSize,
      ));
    ...
    for (let i = 0; i < groupSize; i++) {
      ...
      if (node) {
        const keyValue = (keyValues[i] =
          Selection.keyPrefix + key(node, node.datum, i, group));
