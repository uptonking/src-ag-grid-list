...
export default class {
  ...
  addSubscriber(subscriber, symbol) {
    ...
    if (!this.updateInterval) {
      this.updateInterval = setInterval(
        this.applyDeltasToTickerData.bind(this),
        500,
      );
    }
  }
  ...
  initialiseTickerData() {
    ...
    const allSymbols = uniq(
      concat(NASDAQ_SYMBOLS, LSE_SYMBOLS, JSE_SYMBOLS, DE_SYMBOLS),
    );
  ...
  applyDeltasToTickerData() {
    ...
    symbolsToAlter.forEach((symbol) => {
      ...
      this.tickerData[symbol][propertyToAlter] =
        +this.tickerData[symbol][propertyToAlter] + this.random(-2, 2);
    });
  ...
  createTickerDetail(symbol) {
    ...
    let previousPrice =
      +currentPrice + this.random(-tenthOfCurrentPrice, tenthOfCurrentPrice);

    let twentiethOfCurrentPrice = currentPrice / 20;
    let yearAgoPrice = this.random(
      -twentiethOfCurrentPrice,
      twentiethOfCurrentPrice,
    );

    let range = `${this.formatNumber(previousPrice)} - ${this.formatNumber(
      currentPrice,
    )}`;
    let fiftyTwoWeek = `${this.formatNumber(
      yearAgoPrice,
    ...
    let historicalData = this.generateHistoricalData(
      100,
      this.timestamp,
      currentPrice,
    );
...
const EXCHANGES = [
  {
    ...
    symbol: 'NASDAQ',
    supportedStocks: NASDAQ_SYMBOLS,
  },
  {
    name: 'London Stock Exchange',
    symbol: 'LSE',
    supportedStocks: LSE_SYMBOLS,
  },
