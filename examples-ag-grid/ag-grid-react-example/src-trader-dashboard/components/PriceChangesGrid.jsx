...
export default class extends Component {
  ...
  onGridReady(params) {
    ...
    let rowData = map(this.props.selectedExchange.supportedStocks, (symbol) =>
      this.exchangeService.getTicker(symbol),
    );
  ...
  onSelectionChanged() {
    ...
    this.props.onSelectionChanged(
      selectedNode ? selectedNode.data.symbol : null,
    );
  }
  ...
  componentWillReceiveProps(nextProps) {
    if (
      nextProps.selectedExchange.supportedStocks !==
      this.props.selectedExchange.supportedStocks
    ) {
      if (!this.gridApi) {
        return;
      }
      ...
      let rowData = map(symbolsAdded, (symbol) =>
        this.exchangeService.getTicker(symbol),
      );
      this.gridApi.applyTransaction({ add: rowData });
  ...
  render() {
    return (
      <div style={{ height: 410, width: 800 }} className='ag-theme-balham'>
        <AgGridReact
          // properties
