...
const getExamples = (path) => {
  ...
  files.forEach((name) => {
    if (name === 'lib') {
      throw new Error(
        "Example folder can't be named 'lib' " +
...
const moduleExports = (env) => {
  ...
  const activeExampleName =
    env && env.example ? env.example : defaultExampleName;
  const hasActiveExample = existsSync(join(srcPath, activeExampleName));

  if (activeExampleName !== defaultExampleName && !hasActiveExample) {
    console.warn(
      `Could not find specified example "${activeExampleName}". Building all examples instead.`,
    );
  }

  const examples = hasActiveExample
    ? [{ path: srcPath, name: activeExampleName }]
    : getExamples(srcPath);
