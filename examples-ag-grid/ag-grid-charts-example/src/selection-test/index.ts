...
function test_SelectionSelect() {
  ...
  const rootSelection: Selection<Group, Node | EnterNode, any, any> =
    Selection.select(rootGroup);
...
function test_selectAll_setData_enter() {
  ...
  const selection: Selection<Rect, Group, number, number> = rootSelection
    .selectAll()
    .setData(data)
    .enter.append(Rect)
...
function test_call_merge() {
  ...
  function style1(
    selection: Selection<Rect, Node | EnterNode, number, number>,
  ) {
...

  {
    ...
    rootGroup.children.forEach((value, index) => {
      console.assert((value as Rect).x === index * 80);
      console.assert(
        (value as Rect).strokeWidth === ((value as Rect).datum as number),
      );
    });
  }
