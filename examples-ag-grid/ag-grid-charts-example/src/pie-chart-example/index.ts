...
import {
  FontStyle,
  FontWeight,
} from 'ag-charts-community/src/scene/shape/text';
...
function createPieChart() {
  ...
  createButton('Use tooltip renderer', () => {
    pieSeries.tooltipRenderer = (params) => {
      return `<em>Value</em>: <span style='color: red;'>${
        params.datum[params.angleKey]
      }</span>`;
    };

    pieSeries2.tooltipRenderer = (params) => {
      const radiusValue = params.radiusKey
        ? `<br>Radius: ${params.datum[params.radiusKey]}`
        : '';
  ...
  createSlider(
    'labelColor',
    ['black', 'red', 'gold', 'rgb(221, 221, 221)'],
    (v) => {
  ...
  createSlider(
    'legend position',
    ['right', 'bottom', 'left', 'top'] as LegendPosition[],
    (v) => {
  ...
  createSlider(
    'legend markerShape',
    ['square', 'circle', 'diamond', 'cross', 'plus', 'triangle'],
    (v) => {
  ...
  createSlider(
    'legend fontFamily',
    ['Verdana', 'Papyrus', 'Comic Sans', 'Palatino'],
    (v) => {
