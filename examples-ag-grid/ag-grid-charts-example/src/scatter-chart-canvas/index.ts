...
function renderChart(
  ...
  const xData = configs.map((config, i) =>
    data[i].map((datum) => datum[config.xKey] as number),
  );
  const yData = configs.map((config, i) =>
    data[i].map((datum) => datum[config.yKey] as number),
  );
  const defaultRadius = 6;
  const radiusData = configs.map((config, i) =>
    data[i].map(
      (datum) =>
        (config.radiusKey ? datum[config.radiusKey] : defaultRadius) as number,
    ),
  );
  ...
  configs.forEach((_, i) =>
    drawSeries(colors[i % colors.length], xData[i], yData[i], radiusData[i]),
  );
  ctx.restore();
  ...
  const legendTop =
    padding.top + (seriesHeight - legendItemHeight * configs.length) / 2;
  ctx.strokeStyle = 'black';
  ctx.textAlign = 'left';
...
document.addEventListener('DOMContentLoaded', () => {
...

  {
    ...
    carData.forEach((car) => {
      const countryCars =
        carsByCountry[car.origin] || (carsByCountry[car.origin] = []);
