...
function createHeightWeightGenderChart() {
  ...
  d3.csv('../../data/body-data.csv').then((rawData) => {
    ...
    maleSeries.stroke = Color.fromString(maleSeries.fill)
      .darker()
      .toHexString();
    ...
    femaleSeries.stroke = Color.fromString(femaleSeries.fill)
      .darker()
      .toHexString();
  ...
  createSlider(
    'Marker type',
    [Circle, Square, Diamond, Cross, Plus, Triangle],
    (v) => {
...
function createFakeScatterChart() {
  ...
  for (var x = start; x < 2013; x += 0.05) {
    ...
    var y =
      (4 + Math.random()) * (x - start) -
      15 +
      (-distance / 2 + Math.random() * distance);
    data.push({
      time: x,
      mm: y,
    });
