...
function createNumericLineChart() {
  ...
  niceCheckboxLabel.innerHTML =
    'Data domain auto-rounding (desirable for static charts but not for animated ones)';
  ...
  createSlider(
    'stroke width',
    [0, 2, 4, 6, 8],
    (value) => (lineSeries.strokeWidth = value),
  );
  createSlider(
    'marker stroke width',
    [0, 2, 4, 6, 8],
    (value) => (lineSeries.marker.strokeWidth = value),
  );
  createSlider(
    'marker size',
    [0, 2, 4, 6, 8],
    (value) => (lineSeries.marker.size = value),
  );
}
...
function createMultiLineChart() {
  ...
  createButton('Remove the bar series', () => {
    if (chart.removeSeries(barSeries)) {
      ...
      console.log(
        'No series removed. The chart does not contain the given series.',
      );
    }
  });
