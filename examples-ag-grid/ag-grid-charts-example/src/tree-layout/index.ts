...
class TreeNode {
  ...
  getSiblings(): TreeNode[] {
    return this.parent
      ? this.parent.children.filter((_, i) => i !== this.number)
      : [];
  }
}
...
function ancestor(
  ...
  return v.getSiblings().indexOf(vim.ancestor) >= 0
    ? vim.ancestor
    : defaultAncestor;
}
...
function firstWalk(node: TreeNode, distance: number) {
  ...
  if (children.length) {
    ...
    const midpoint =
      (children[0].prelim + children[children.length - 1].prelim) / 2;
...
function treeLayout(root: TreeNode, size: TreeSize) {
  ...
  const existingSpacingX =
    (info.dimensions.right - info.dimensions.left) / spaceCount;
  const desiredSpacingX = size.width / spaceCount;
  const scalingX = desiredSpacingX / existingSpacingX;
  const existingSpacingY =
    (info.dimensions.bottom - info.dimensions.top) / spaceCount;
