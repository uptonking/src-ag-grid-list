...
function processFinVizData() {
  ...
  function process(obj: any): any {
    return {
      ...
      size: obj.dx * obj.dy,
      children:
        obj.children && obj.children.map((child: any) => process(child)),
    };
  }
...
function createStockTreeMap() {
  ...
  let groupSelection: Selection<Group, Group, any, any> =
    Selection.select(rootGroup).selectAll<Group>();
  ...
  function update() {
    ...
    const dataRoot = d3
      .hierarchy(data)
      .sum((datum) => (datum.children ? 1 : datum.size));
    const treemapLayout = d3
      ...
      .paddingTop((node) => {
        ...
        const hasTitle =
          node.depth > 0 && node.children && textSize.width <= innerNodeWidth;
        (node as any).hasTitle = hasTitle;

        if (name === 'Property & Casualty Insurance') {
          console.log(
            `node.depth: ${node.depth}, textSize.width: ${textSize.width}, innerNodeWidth: ${innerNodeWidth}`,
          );
        }
    ...
    groupSelection.selectByClass(Rect).each((rect, datum, index) => {
      ...
      if (!fill) {
        fill = isParent
          ? '#272931'
          : colorInterpolator(colorScale(-5 + Math.random() * 10));
    ...
    groupSelection
      ...
      .each((text, datum, index) => {
        ...
        const font = isLeaf
          ? innerNodeHeight > 40 && innerNodeWidth > 40
            ? fonts.label.big
            : innerNodeHeight > 20 && innerNodeHeight > 20
            ? fonts.label.medium
            : fonts.label.small
          : datum.depth > 1
          ? fonts.subtitle
    ...
    groupSelection
      .selectByTag<Text>(TextNodeTag.Change)
      .each((text, datum, index) => {
        const isLeaf = !datum.children;
        const innerNodeWidth = datum.x1 - datum.x0 - nodePadding * 2;
        // const innerNodeHeight = datum.y1 - datum.y0 - nodePadding * 2;
        ...
        const isVisible =
          hasLabel && !!textBBox && textBBox.width < innerNodeWidth;
  ...
  function update() {
    ...
    const treemapLayout = d3
      ...
      .paddingTop((node) => {
        ...
        const hasTitlePadding =
          node.depth > 0 && node.children && nameSize.width - 4 < width;
    ...
    groupSelection.selectByClass(Text).each((text, datum, index) => {
      ...
      if (bbox) {
        text.visible =
          (!isRoot &&
            datum.x1 - datum.x0 - 2 * 2 > bbox.width &&
            datum.y1 - datum.y0 - 2 * 2 > bbox.height) ||
          datum.hasTitle;
      }
      text.x = datum.x0 + 3;
      text.y = datum.y0 + 4;
