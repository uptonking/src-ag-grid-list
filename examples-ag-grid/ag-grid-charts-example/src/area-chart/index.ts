...
function renderChart(
  ...
  yScale.domain = [
    0,
    isFullStack ? stackTotal : Math.max(...yData.map((d) => d[d.length - 1])),
  ];
  ...
  if (isTopStroke) {
    ...
    topStrokes.forEach((stroke, keyIndex) => {
      ctx.beginPath();
      ctx.strokeStyle = isBlackStroke
        ? 'black'
        : colors[keyIndex % colors.length];
      stroke.forEach((p, i) => {
  ...
  const legendTop =
    padding.top + (seriesHeight - legendItemHeight * yKeys.length) / 2;
  ctx.strokeStyle = 'black';
