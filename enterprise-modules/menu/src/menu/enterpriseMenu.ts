...
export class EnterpriseMenu extends BeanStub {
  ...
  public static TABS_DEFAULT = [
    EnterpriseMenu.TAB_GENERAL,
    EnterpriseMenu.TAB_FILTER,
    EnterpriseMenu.TAB_COLUMNS,
  ];
  ...
  constructor(column: Column, initialSelection: string, restrictTo?: string[]) {
    ...
    this.tabFactories[EnterpriseMenu.TAB_GENERAL] =
      this.createMainPanel.bind(this);
    this.tabFactories[EnterpriseMenu.TAB_FILTER] =
      this.createFilterPanel.bind(this);
    this.tabFactories[EnterpriseMenu.TAB_COLUMNS] =
      this.createColumnsPanel.bind(this);
  ...
  @PostConstruct
  public init(): void {
    const tabs = this.getTabsToCreate().map((menuTabName) =>
  ...
  private getTabsToCreate() {
    ...
    return this.column
      .getMenuTabs(EnterpriseMenu.TABS_DEFAULT)
  ...
  private getDefaultMenuOptions(): string[] {
    ...
    const groupedByThisColumn =
      this.columnController.getRowGroupColumns().indexOf(this.column) >= 0;
    ...
    if (isInMemoryRowModel) {
      if (usingTreeData) {
        ...
        allowExpandAndContract = pivotModeOn
          ? rowGroupCount > 1
          : rowGroupCount > 0;
      }
    }
  ...
  private createMainPanel(): TabbedItem {
    ...
    this.mainMenuList.addEventListener(
      MenuItemComponent.EVENT_ITEM_SELECTED,
  ...
  private createFilterPanel(): TabbedItem {
    const filterWrapper: FilterWrapper =
      this.filterManager.getOrCreateFilterWrapper(this.column, 'COLUMN_MENU');
  ...
  public afterGuiAttached(params: any): void {
    ...
    const onBodyScroll = (event: any) => {
      ...
      if (event.direction === 'horizontal') {
        ...
        if (
          Math.abs(newScroll - initialScroll) >
          this.gridOptionsWrapper.getScrollbarWidth()
        ) {
          params.hidePopup();
        }
      }
