...
export class MenuList extends ManagedFocusComponent {
  ...
  private static SEPARATOR_TEMPLATE /* html */ = `<div class="ag-menu-separator">
  ...
  public addItem(menuItemDef: MenuItemDef): void {
    const cMenuItem = this.createManagedBean(
      new MenuItemComponent(menuItemDef),
    );
    ...
    const handleMouseEnter = (
      cMenuItem: MenuItemComponent,
      menuItemParams: MenuItemDef,
    ) => {
    ...
    cMenuItem.addGuiEventListener('mouseenter', () =>
      handleMouseEnter(cMenuItem, menuItemDef),
    );
    cMenuItem.addGuiEventListener('mouseleave', (e) =>
      handleMouseLeave(e, cMenuItem, menuItemDef),
    );
  }
  ...
  private mouseLeaveItem(
    ...
    const isParent =
      this.subMenuComp && this.subMenuComp.getParentComponent() === menuItem;
  ...
  private findTopMenu(): MenuList | undefined {
    ...
    while (true) {
      const nextParent =
        parent && parent.getParentComponent && parent.getParentComponent();

      if (
        !nextParent ||
        !(
          nextParent instanceof MenuList ||
          nextParent instanceof MenuItemComponent
        )
      ) {
        break;
      }
  ...
  private handleNavKey(key: number): void {
    ...
    const left = this.gridOptionsWrapper.isEnableRtl()
      ? Constants.KEY_RIGHT
      : Constants.KEY_LEFT;
  ...
  private addHoverForChildPopup(
    menuItemComp: MenuItemComponent,
