...
export class MenuItemComponent extends Component {
  ...
  private static TEMPLATE /* html */ = `<div class="ag-menu-option" tabindex="-1">
  ...
  private init() {
    ...
    if (this.params.subMenu) {
      if (this.gridOptionsWrapper.isEnableRtl()) {
        ...
        this.ePopupPointer.appendChild(
          _.createIconNoSpan('smallLeft', this.gridOptionsWrapper),
        );
      } else {
        // for normal, we show arrow going right
        this.ePopupPointer.appendChild(
          _.createIconNoSpan('smallRight', this.gridOptionsWrapper),
        );
      }
    } else {
