...
export class SetFilterListItem extends Component {
  ...
  private renderCell(target: ColDef, eTarget: HTMLElement, params: any): void {
    ...
    const cellRendererPromise =
      this.userComponentFactory.newSetFilterCellRenderer(filterParams, params);

    if (cellRendererPromise == null) {
      const valueToRender =
        params.valueFormatted == null ? params.value : params.valueFormatted;

      eTarget.innerText =
        valueToRender == null ? `(${this.translate('blanks')})` : valueToRender;
