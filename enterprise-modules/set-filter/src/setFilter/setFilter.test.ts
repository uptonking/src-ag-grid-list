...
function createSetFilter(
  ...
  valueFormatterService.formatValue.mockImplementation(
    (_1, _2, _3, value) => value,
  );

  const gridOptionsWrapper = mock<GridOptionsWrapper>(
    'getLocaleTextFunc',
    'isEnableOldSetFilterModel',
  );
  gridOptionsWrapper.getLocaleTextFunc.mockImplementation(
    () => (_: string, defaultValue: string) => defaultValue,
  );

  const context = mock<Context>('createBean');
  context.createBean.mockImplementation((bean) => bean);

  const eMiniFilter = mock<AgInputTextField>(
    'getGui',
    'setValue',
    'onValueChange',
    'getInputElement',
  ...
  eMiniFilter.getInputElement.mockImplementation(() =>
    mock<HTMLInputElement>('addEventListener'),
  );

  const eGui = mock<HTMLElement>('querySelector', 'appendChild');
  eGui.querySelector.mockImplementation(() =>
    mock<HTMLElement>('appendChild', 'addEventListener'),
  );

  const eSelectAll = mock<AgCheckbox>(
    'setValue',
    'getInputElement',
    'onValueChange',
  );
  eSelectAll.getInputElement.mockImplementation(() =>
    mock<HTMLInputElement>('addEventListener'),
  );
...
describe('applyModel', () => {
  it('returns false if nothing has changed', () => {
    const setValueModel = mock<SetValueModel>(
      'getModel',
  ...
  it.each(['windows', 'mac'])(
    'will not apply model with zero values in %s Excel Mode',
    (excelMode) => {
      const setValueModel = mock<SetValueModel>(
        'getModel',
        'isEverythingVisibleSelected',
  ...
  it.each(['windows', 'mac'])(
    'can reset model in %s Excel Mode',
    (excelMode) => {
      const setValueModel = mock<SetValueModel>(
        'getModel',
        'isEverythingVisibleSelected',
