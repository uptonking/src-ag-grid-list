...
export class SetFilter extends ProvidedFilter {
  ...
  protected createBodyTemplate(): string {
    return /* html */ `
            <div>
                <div ref="eFilterLoading" class="ag-filter-loading ag-hidden">${this.translate(
                  'loadingOoo',
                )}</div>
                ...
                <div ref="eFilterNoMatches" class="ag-filter-no-matches ag-hidden">${this.translate(
                  'noMatches',
                )}</div>
  ...
  private handleKeyTab(e: KeyboardEvent): void {
    ...
    if (
      (e.shiftKey &&
        this.focusController.focusLastFocusableElement(nextRoot)) ||
  ...
  private handleKeySpace(e: KeyboardEvent): void {
    ...
    if (_.exists(currentItem)) {
      const component = this.virtualList.getComponentAt(
        currentItem,
      ) as SetFilterListItem;
  ...
  protected setModelIntoUi(model: SetFilterModel): Promise<void> {
    ...
    if (model instanceof Array) {
      const message =
        'ag-Grid: The Set Filter Model is no longer an array and models as arrays are ' +
  ...
  private checkSetFilterDeprecatedParams(params: ISetFilterParams): void {
    if (params.syncValuesLikeExcel) {
      const message =
        'ag-Grid: since version 22.x, the Set Filter param syncValuesLikeExcel is no longer' +
    ...
    if (params.selectAllOnMiniFilter) {
      const message =
        'ag-Grid: since version 22.x, the Set Filter param selectAllOnMiniFilter is no longer' +
    ...
    if (params.suppressSyncValuesAfterDataChange) {
      const message =
        'ag-Grid: since version 23.1, the Set Filter param suppressSyncValuesAfterDataChange has' +
    ...
    if (params.suppressRemoveEntries) {
      const message =
        'ag-Grid: since version 23.1, the Set Filter param suppressRemoveEntries has' +
        ' been deprecated and will be removed in a future major release.';
  ...
  private createSetListItem(value: any): Component {
    const listItem = this.createBean(
      new SetFilterListItem(value, this.setFilterParams, (key) =>
        this.translate(key),
      ),
    );
  ...
  private initMiniFilter() {
    ...
    this.addManagedListener(eMiniFilter.getInputElement(), 'keypress', (e) =>
      this.onMiniFilterKeyPress(e),
    );
  }
  ...
  public applyModel(): boolean {
    ...
    if (result) {
      ...
      if (appliedModel) {
        ...
        _.forEach(
          appliedModel.values,
          (value) => (this.appliedModelValues[value] = true),
        );
      } else {
  ...
  protected isModelValid(model: SetFilterModel): boolean {
    return this.setFilterParams.excelMode
      ? model == null || model.values.length > 0
      : true;
  }
  ...
  public doesFilterPass(params: IDoesFilterPassParams): boolean {
    ...
    if (Array.isArray(value)) {
      return _.some(
        value,
        (v) => this.appliedModelValues[_.makeNull(v)] === true,
      );
    }
  ...
  public onNewRowsLoaded(): void {
    ...
    this.syncAfterDataChange(
      valuesType === SetFilterModelValuesType.TAKEN_FROM_GRID_VALUES,
      keepSelection,
    );
  }
  ...
  public setFilterValues(options: string[]): void {
    this.valueModel
      .overrideValues(options, this.isNewRowsActionKeep())
      .then(() => {
        this.refresh();
        this.onUiChanged();
      });
  ...
  public resetFilterValues(): void {
    this.valueModel.setValuesType(
      SetFilterModelValuesType.TAKEN_FROM_GRID_VALUES,
    );
  ...
  private updateUiAfterMiniFilterChange(): void {
    ...
    const hideResults =
      this.valueModel.getMiniFilter() != null &&
  ...
  private onMiniFilterKeyPress(e: KeyboardEvent): void {
    if (
      _.isKeyPressed(e, Constants.KEY_ENTER) &&
      !this.setFilterParams.excelMode
    ) {
