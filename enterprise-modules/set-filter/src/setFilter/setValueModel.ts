...
export class SetValueModel implements IEventEmitter {
  ...
  constructor(
    ...
    this.formatter =
      this.filterParams.textFormatter || TextFilter.DEFAULT_FORMATTER;
        ...
        case SetFilterModelValuesType.PROVIDED_LIST: {
          const values =
            this.valuesType === SetFilterModelValuesType.TAKEN_FROM_GRID_VALUES
              ...
              : _.toStrings(this.providedValues as any[]);
  ...
  private updateAvailableValues(allValues: string[]): void {
    const availableValues = this.showAvailableOnly()
      ? this.sortValues(this.getValuesFromRows(true))
      : allValues;
  ...
  private sortValues(values: string[]): string[] {
    ...
    return _.filter(values, (v) => v != null)
      .sort(comparator)
      .concat(null);
  }
  ...
  private getValuesFromRows(removeUnavailableValues = false): string[] {
    ...
    this.clientSideRowModel.forEachLeafNode((node) => {
      ...
      if (
        !node.data ||
        (removeUnavailableValues && !this.doesRowPassOtherFilters(node))
      ) {
        return;
      }
  ...
  private updateDisplayedValues(): void {
    ...
    const formattedFilterText = this.formatter(
      this.miniFilterText,
    ).toUpperCase();

    const matchesFilter = (valueToCheck: string): boolean =>
      valueToCheck != null &&
      valueToCheck.toUpperCase().indexOf(formattedFilterText) >= 0;

    this.availableValues.forEach((value) => {
      if (value == null) {
        if (
          this.filterParams.excelMode &&
          matchesFilter(`(${this.translate('blanks')})`)
        ) {
        ...
        if (
          matchesFilter(textFormatterValue) ||
          matchesFilter(valueFormatterValue)
        ) {
          this.displayedValues.push(value);
        }
      }
  ...
  public selectAllMatchingMiniFilter(clearExistingSelection = false): void {
    if (this.miniFilterText == null) {
      ...
      _.forEach(this.displayedValues, (value) =>
  ...
  public isEverythingVisibleSelected(): boolean {
    return (
      _.filter(this.displayedValues, (it) => this.isValueSelected(it))
