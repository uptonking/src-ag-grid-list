...
export class SetFloatingFilterComp
  extends Component
  implements IFloatingFilter
{
  ...
  private updateSetFilterText(): void {
    ...
    this.params.parentFilterInstance((setFilter: SetFilter) => {
      ...
      const availableValues = _.filter(values, (v) =>
        valueModel.isValueAvailable(v),
      );
      ...
      const formattedValues = _.map(availableValues, (value) => {
        ...
        return valueToRender == null
          ? `(${localeTextFunc('blanks', 'Blanks')})`
          : valueToRender;
      });

      const arrayToDisplay =
        formattedValues.length > 10
          ? formattedValues.slice(0, 10).concat('...')
          : formattedValues;
