...
export class ClipboardService extends BeanStub implements IClipboardService {
  ...
  public pasteFromClipboard(): void {
    ...
    this.executeOnTempElement(
      ...
      (element: HTMLTextAreaElement) => {
        ...
        let parsedData = _.stringToArray(
          data,
          this.gridOptionsWrapper.getClipboardDeliminator(),
        );

        const userFunc =
          this.gridOptionsWrapper.getProcessDataFromClipboardFunc();
        ...
        const pasteOperation = (
          ...
          const pasteIntoRange =
            rangeActive && !this.hasOnlyOneValueToPaste(parsedData);

          if (pasteIntoRange) {
            this.pasteIntoActiveRange(
              parsedData,
              cellsToFlash,
              updatedRowNodes,
              changedPath,
            ...
            this.pasteStartingFromFocusedCell(
              parsedData,
              cellsToFlash,
              updatedRowNodes,
              focusedCell,
  ...
  private pasteIntoActiveRange(
    ...
    const abortRepeatingPasteIntoRows =
      this.getRangeSize() % clipboardData.length != 0;
    ...
    const rowCallback: RowCallback = (
      ...
      const processCellFromClipboardFunc =
  ...
  private pasteStartingFromFocusedCell(
    ...
    const currentRow: RowPosition = {
      rowIndex: focusedCell.rowIndex,
      rowPinned: focusedCell.rowPinned,
    };
    const columnsToPasteInto =
      this.columnController.getDisplayedColumnsStartingAt(focusedCell.column);

    if (this.hasOnlyOneValueToPaste(parsedData)) {
      this.pasteSingleValue(
        parsedData,
        updatedRowNodes,
        cellsToFlash,
        changedPath,
  ...
  public copyRangeDown(): void {
    ...
    const pasteOperation = (
      ...
      const processCellForClipboardFunc =
  ...
  private removeLastLineIfBlank(parsedData: string[][]): void {
    ...
    const lastLineIsBlank =
      lastLine && lastLine.length === 1 && lastLine[0] === '';
  ...
  private pasteMultipleValues(
    ...
    clipboardGridData.forEach((clipboardRowData) => {
      ...
      if (rowNode) {
        ...
        currentRow = this.cellNavigationService.getRowBelow({
  ...
  private iterateActiveRanges(
    ...
    if (onlyFirst) {
      ...
      cellRanges.forEach((range, idx) =>
        this.iterateActiveRange(
          range,
          rowCallback,
          columnCallback,
          idx === cellRanges.length - 1,
        ),
      );
    }
  }
  ...
  public copySelectedRangeToClipboard(includeHeaders = false): void {
    ...
    const columnCallback = (columns: Column[]) => {
      ...
      const processHeaderForClipboardFunc =
        this.gridOptionsWrapper.getProcessHeaderForClipboardFunc();
      const columnNames = columns.map((column) => {
        const name = this.columnController.getDisplayNameForColumn(
          column,
          'clipboard',
          true,
        );
        return (
          this.processHeader(column, name, processHeaderForClipboardFunc) || ''
        );
      });
  ...
  private copyFocusedCellToClipboard(includeHeaders = false): void {
    ...
    if (includeHeaders) {
      const headerValue = this.columnController.getDisplayNameForColumn(
        column,
        'clipboard',
        true,
      );
  ...
  private processCell<T>(
    ...
    value: T,
    type: string,
    func?: (params: ProcessCellForExportParams) => T,
  ): T {
    if (func) {
      const params = {
        column,
  ...
  private processHeader<T>(
    column: Column,
    value: T,
    func?: (params: ProcessHeaderForExportParams) => T,
  ): T {
    if (func) {
  ...
  private getRangeSize(): number {
    ...
    if (ranges.length > 0) {
      startRangeIndex = this.rangeController.getRangeStartRow(
        ranges[0],
      ).rowIndex;
