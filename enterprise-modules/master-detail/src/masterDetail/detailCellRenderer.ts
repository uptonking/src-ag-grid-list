...
export class DetailCellRenderer extends Component implements ICellRenderer {
  private static TEMPLATE /* html */ = `<div class="ag-details-row">
  ...
  public refresh(): boolean {
    ...
    const doNotRefresh =
      !this.needRefresh || this.params.refreshStrategy === 'nothing';
    if (doNotRefresh) {
  ...
  private checkForDeprecations(): void {
    if (this.params.suppressRefresh) {
      console.warn(
        'ag-Grid: as of v23.2.0, cellRendererParams.suppressRefresh for Detail Cell Renderer is no ' +
  ...
  private setupAutoGridHeight(): void {
    ...
    const onRowHeightChangedDebounced = _.debounce(
      gridApi.onRowHeightChanged.bind(gridApi),
      20,
    );
