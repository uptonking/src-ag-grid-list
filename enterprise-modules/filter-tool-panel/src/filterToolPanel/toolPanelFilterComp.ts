...
export class ToolPanelFilterComp extends Component {
  ...
  public setColumn(column: Column): void {
    this.column = column;
    this.eFilterName.innerText = this.columnController.getDisplayNameForColumn(
      this.column,
      'header',
      false,
    ) as string;
  ...
  public expand(): void {
    ...
    const container = _.loadTemplate(
      /* html */ `<div class="ag-filter-toolpanel-instance-filter" />`,
    );
    const filterPromise = this.filterManager.getOrCreateFilterWrapper(
      this.column,
      'TOOLBAR',
    ).filterPromise;
