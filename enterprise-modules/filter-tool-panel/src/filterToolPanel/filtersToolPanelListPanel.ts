...
export class FiltersToolPanelListPanel extends Component {
  ...
  public init(params: ToolPanelFiltersCompParams): void {
    ...
    this.addManagedListener(
      this.eventService,
      Events.EVENT_TOOL_PANEL_VISIBLE_CHANGED,
      (event) => {
  ...
  public onColumnsChanged(): void {
    ...
    const shouldSyncColumnLayoutWithGrid =
      !this.params.suppressSyncLayoutWithGrid && !pivotModeActive;
    shouldSyncColumnLayoutWithGrid
  ...
  private buildTreeFromProvidedColumnDefs(): void {
    this.destroyFilters();
    const columnTree: OriginalColumnGroupChild[] =
      this.columnController.getPrimaryColumnTree();
  ...
  public setFiltersLayout(colDefs: AbstractColDef[]): void {
    this.destroyFilters();
    const columnTree: OriginalColumnGroupChild[] =
      this.toolPanelColDefService.createColumnTree(colDefs);
    this.filterGroupComps = this.recursivelyAddComps(columnTree, 0);
    ...
    return _.flatten(
      tree.map((child) => {
        if (child instanceof OriginalColumnGroup) {
          return _.flatten(
            this.recursivelyAddFilterGroupComps(
              child as OriginalColumnGroup,
              depth,
            ),
          );
        }
  ...
  private recursivelyAddFilterGroupComps(
    ...
    if (
      ...
      columnGroup.getColGroupDef().suppressFiltersToolPanel
    ) {
      return [];
    }

    const newDepth = columnGroup.isPadding() ? depth : depth + 1;
    const childFilterComps = _.flatten(
      this.recursivelyAddComps(columnGroup.getChildren(), newDepth),
    );
  ...
  private shouldDisplayFilter(column: Column) {
    const suppressFiltersToolPanel =
      column.getColDef() && column.getColDef().suppressFiltersToolPanel;
  ...
  public expandFilterGroups(expand: boolean, groupIds?: string[]): void {
    ...
    if (groupIds) {
      const unrecognisedGroupIds = groupIds.filter(
        (groupId) => updatedGroupIds.indexOf(groupId) < 0,
      );
  ...
  public expandFilters(expand: boolean, colIds?: string[]): void {
    ...
    const updateGroupExpandState = (
      filterComp: ToolPanelFilterGroupComp | ToolPanelFilterComp,
    ): boolean => {
    ...
    if (colIds) {
      const unrecognisedColIds = colIds.filter(
        (colId) => updatedColIds.indexOf(colId) < 0,
      );
  ...
  public performFilterSearch(searchText: string) {
    this.searchFilterText = _.exists(searchText)
  ...
  private searchFilters(searchFilter: string | null) {
    const passesFilter = (groupName: string) => {
      return (
        !_.exists(searchFilter) ||
        groupName.toLowerCase().indexOf(searchFilter) !== -1
      );
    };

    const recursivelySearch = (
      filterItem: ToolPanelFilterItem,
    ...
    this.filterGroupComps.forEach((filterGroup, idx) => {
      ...
      if (firstVisible === undefined) {
        ...
        !_.containsClass(filterGroup.getGui(), 'ag-hidden') &&
        lastVisible !== idx
      ) {
        lastVisible = idx;
      }
  ...
  private refreshFilters() {
    this.filterGroupComps.forEach((filterGroupComp) =>
      filterGroupComp.refreshFilters(),
    );
  }
