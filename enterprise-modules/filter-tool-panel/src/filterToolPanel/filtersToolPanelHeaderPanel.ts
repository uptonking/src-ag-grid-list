...
export class FiltersToolPanelHeaderPanel extends Component {
  ...
  private showOrHideOptions(): void {
    ...
    this.eSearchTextField.setInputPlaceholder(
      translate('searchOoo', 'Search...'),
    );

    const isFilterGroupPresent = (col: Column) =>
      col.getOriginalParent() && col.isFilterAllowed();
    const filterGroupsPresent = this.columnController
      .getAllGridColumns()
      .some(isFilterGroupPresent);
  ...
  private onSearchTextChanged(): void {
    if (!this.onSearchTextChangedDebounced) {
      this.onSearchTextChangedDebounced = _.debounce(() => {
        this.dispatchEvent({
          type: 'searchChanged',
          searchText: this.eSearchTextField.getValue(),
        });
      }, 300);
    }
  ...
  private onExpandClicked(): void {
    const event =
      this.currentExpandState === EXPAND_STATE.EXPANDED
        ? { type: 'collapseAll' }
        : { type: 'expandAll' };
  ...
  public setExpandState(state: EXPAND_STATE): void {
    ...
    _.setDisplayed(
      ...
      this.currentExpandState === EXPAND_STATE.COLLAPSED,
    );
    _.setDisplayed(
      this.eExpandIndeterminate,
      this.currentExpandState === EXPAND_STATE.INDETERMINATE,
    );
  }
}
