...
export type ToolPanelFilterItem =
...
export class ToolPanelFilterGroupComp extends Component {
  ...
  constructor(
    columnGroup: OriginalColumnGroupChild,
    childFilterComps: ToolPanelFilterItem[],
  ...
  public init(): void {
    ...
    this.childFilterComps.forEach((filterComp) => {
      ...
      filterComp.addCssClassToTitleBar(
        `ag-filter-toolpanel-group-level-${this.depth + 1}-header`,
      );
    });
  ...
  private forEachToolPanelFilterChild(
    action: (filterComp: ToolPanelFilterItem) => void,
  ) {
  ...
  private addExpandCollapseListeners() {
    ...
    const collapseListener = this.isColumnGroup()
      ? () => this.expandedCallback()
      : () =>
          this.forEachToolPanelFilterChild((filterComp) =>
            filterComp.collapse(),
          );

    this.addManagedListener(
      this.filterGroupComp,
      AgGroupComponent.EVENT_EXPANDED,
      expandListener,
    );
    this.addManagedListener(
      this.filterGroupComp,
      AgGroupComponent.EVENT_COLLAPSED,
      collapseListener,
    );
  }
  ...
  private addFilterChangedListeners() {
    if (this.columnGroup instanceof OriginalColumnGroup) {
      ...
      const anyChildFiltersActive = () =>
        group.getLeafColumns().some((col) => col.isFilterActive());
  ...
  private setGroupTitle() {
    this.filterGroupName =
      this.columnGroup instanceof OriginalColumnGroup
