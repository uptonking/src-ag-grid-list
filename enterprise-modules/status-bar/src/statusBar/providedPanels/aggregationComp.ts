...
export class AggregationComp extends Component implements IStatusPanelComp {
  ...
  private setAggregationComponentValue(
    ...
    const statusBarValueComponent =
      this.getAggregationValueComponent(aggFuncName);
    if (_.exists(statusBarValueComponent) && statusBarValueComponent) {
      statusBarValueComponent.setValue(
        _.formatNumberTwoDecimalPlacesAndCommas(value),
      );
  ...
  private getAggregationValueComponent(
    ...
    const aggregationPanelConfig =
      _.exists(this.gridOptions.statusBar) && this.gridOptions.statusBar
        ? _.find(
            this.gridOptions.statusBar.statusPanels,
            (panel) => panel.statusPanel === 'agAggregationComponent',
          )
        : null;
    if (_.exists(aggregationPanelConfig) && aggregationPanelConfig) {
      ...
      if (
        ...
        (_.exists(aggregationPanelConfig.statusPanelParams) &&
          ...
          _.exists(
            _.find(
              aggregationPanelConfig.statusPanelParams.aggFuncs,
              (func) => func === aggFuncName,
            ),
          ))
      ) {
  ...
  private onRangeSelectionChanged(): void {
    const cellRanges = this.rangeController
    ...
    if (cellRanges && !_.missingOrEmpty(cellRanges)) {
      cellRanges.forEach((cellRange) => {
        let currentRow: RowPosition | null =
          this.rangeController.getRangeStartRow(cellRange);
        const lastRow = this.rangeController.getRangeEndRow(cellRange);

        while (true) {
          const finishedAllRows =
            _.missing(currentRow) ||
            !currentRow ||
            this.rowPositionUtils.before(lastRow, currentRow);
    ...
    this.setAggregationComponentValue(
      'avg',
      sum / numberCount,
      gotNumberResult,
    );
