...
export class StatusBar extends Component {
  ...
  private postConstruct(): void {
    if (this.gridOptions.statusBar && this.gridOptions.statusBar.statusPanels) {
      const leftStatusPanelComponents =
        this.gridOptions.statusBar.statusPanels.filter(
      ...
      const centerStatusPanelComponents =
        this.gridOptions.statusBar.statusPanels.filter(
      ...
      const rightStatusPanelComponents =
        this.gridOptions.statusBar.statusPanels.filter(
          (componentConfig) =>
            !componentConfig.align || componentConfig.align === 'right',
        );
      this.createAndRenderComponents(
  ...
  private createAndRenderComponents(
    statusBarComponents: any[],
    ePanelComponent: HTMLElement,
  ) {
    const componentDetails: {
      key: string;
      promise: Promise<IStatusPanelComp>;
    }[] = [];
    ...
    statusBarComponents.forEach((componentConfig) => {
      ...
      const promise = this.userComponentFactory.newStatusPanelComponent(
        componentConfig,
        params,
      );
    ...
    Promise.all(componentDetails.map((details) => details.promise)).then(() => {
      componentDetails.forEach((componentDetail) => {
        componentDetail.promise.then((component: IStatusPanelComp) => {
          ...
          if (this.isAlive()) {
            this.statusBarService.registerStatusPanel(
              componentDetail.key,
              component,
            );
