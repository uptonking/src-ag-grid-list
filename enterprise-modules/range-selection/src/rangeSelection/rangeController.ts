...
export class RangeController extends BeanStub implements IRangeController {
  ...
  public isContiguousRange(cellRange: CellRange): boolean {
    ...
    const allPositions = rangeColumns
  ...
  public setRangeToCell(cell: CellPosition, appendRange = false): void {
    ...
    const suppressMultiRangeSelections =
  ...
  public updateRangeEnd(
    cellRange: CellRange,
    cellPosition: CellPosition,
    silent = false,
  ): void {
    const endColumn = cellPosition.column;
    const colsToAdd = this.calculateColumnsBetween(
  ...
  private refreshRangeStart(cellRange: CellRange) {
    ...
    const shouldMoveLeftCol =
      startColumn === columns[0] && startColumn !== left;
    ...
    const shouldMoveRightCol =
      startColumn === _.last(columns) && startColumn === right;
  ...
  public getRangeEdgeColumns(cellRange: CellRange): {
    left: Column;
    right: Column;
  } {
    const allColumns = this.columnController.getAllDisplayedColumns();
  ...
  public extendLatestRangeInDirection(key: number): CellPosition | undefined {
    ...
    const endCell: CellPosition = {
      column: endCellColumn,
      rowIndex: endCellIndex,
      rowPinned: endCellFloating,
    };
    const newEndCell = this.cellNavigationService.getNextCellToFocus(
      key,
      endCell,
    );
  ...
  public createCellRangeFromCellRangeParams(
    ...
    if (params.columns) {
      columns = params.columns
        .map((c) => this.columnController.getColumnWithValidation(c))
        .filter((c) => c);
    } else {
      const columnStart = this.columnController.getColumnWithValidation(
        params.columnStart,
      );
      const columnEnd = this.columnController.getColumnWithValidation(
        params.columnEnd,
      );
  ...
  public isCellInSpecificRange(cell: CellPosition, range: CellRange): boolean {
    const columnInRange =
      range.columns !== null && _.includes(range.columns, cell.column);
  ...
  private isLastCellOfRange(cellRange: CellRange, cell: CellPosition): boolean {
    ...
    const lastRow = this.rowPositionUtils.before(startRow, endRow)
      ? endRow
      : startRow;
    const isLastRow =
      cell.rowIndex === lastRow.rowIndex &&
      cell.rowPinned === lastRow.rowPinned;
    const rangeFirstIndexColumn = cellRange.columns[0];
    const rangeLastIndexColumn = _.last(cellRange.columns);
    const lastRangeColumn =
      cellRange.startColumn === rangeFirstIndexColumn
        ? rangeLastIndexColumn
        : rangeFirstIndexColumn;
  ...
  public isBottomRightCell(cellRange: CellRange, cell: CellPosition): boolean {
    const allColumns = this.columnController.getAllDisplayedColumns();
    const allPositions = cellRange.columns
      .map((c) => allColumns.indexOf(c))
      .sort((a, b) => a - b);
    const { startRow, endRow } = cellRange;
    const lastRow = this.rowPositionUtils.before(startRow, endRow)
    ...
    const isRightColumn =
      allColumns.indexOf(cell.column) === _.last(allPositions);
    const isLastRow =
      cell.rowIndex === lastRow.rowIndex &&
      _.makeNull(cell.rowPinned) === _.makeNull(lastRow.rowPinned);
  ...
  public getCellRangeCount(cell: CellPosition): number {
    ...
    return this.cellRanges.filter((cellRange) =>
      this.isCellInSpecificRange(cell, cellRange),
    ).length;
  }
  ...
  private isRowInRange(
    ...
    const equalsFirstRow =
      thisRow.rowIndex === firstRow.rowIndex &&
      thisRow.rowPinned == firstRow.rowPinned;
    const equalsLastRow =
      thisRow.rowIndex === lastRow.rowIndex &&
      thisRow.rowPinned == lastRow.rowPinned;
  ...
  public onDragStart(mouseEvent: MouseEvent): void {
    ...
    const mouseCell =
      this.mouseEventService.getCellPositionForEvent(mouseEvent);
    ...
    if (
      ...
      (!shiftKey || _.exists(_.last(this.cellRanges)!.type))
    ) {
      this.removeAllCellRanges(true);
    }
  ...
  public onDragging(mouseEvent: MouseEvent | null): void {
    ...
    const isMouseAndStartInPinned = (position: string) =>
      cellPosition &&
      cellPosition.rowPinned === position &&
      this.newestRangeStartCell!.rowPinned === position;

    const skipVerticalScroll =
      isMouseAndStartInPinned('top') || isMouseAndStartInPinned('bottom');
...
class AutoScrollService {
  ...
  public check(
    ...
    skipVerticalScroll: boolean = false,
  ): void {
    ...
    this.tickDown =
      mouseEvent.clientY > rect.bottom - 20 && !skipVerticalScroll;
  ...
  private doTick(): void {
    ...
    if (this.tickUp) {
      this.gridPanel.setVerticalScrollPosition(
        vScrollPosition.top - tickAmount,
      );
    }
    ...
    if (this.tickLeft) {
      this.gridPanel.setHorizontalScrollPosition(
        hScrollPosition.left - tickAmount,
      );
    }
