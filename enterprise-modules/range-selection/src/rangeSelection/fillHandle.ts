...
export class FillHandle extends AbstractSelectionHandle {
  ...
  private handleValueChanged(
    ...
    const initialRangeEndRow =
      this.rangeController.getRangeEndRow(initialRange);
    const initialRangeStartRow =
      this.rangeController.getRangeStartRow(initialRange);
    const finalRangeEndRow = this.rangeController.getRangeEndRow(finalRange);
    const finalRangeStartRow =
      this.rangeController.getRangeStartRow(finalRange);
    ...
    if (
      ...
      const columns = isVertical
        ...
        : initialRange.columns.filter(
            (col) => finalRange.columns.indexOf(col) < 0,
          );

      const startRow = isVertical
        ? this.cellNavigationService.getRowBelow(finalRangeEndRow)
        : finalRangeStartRow;
    ...
    const iterateAcrossCells = (column?: Column, columns?: Column[]) => {
      let currentRow: RowPosition | undefined | null = this.isUp
        ? initialRangeEndRow
        : initialRangeStartRow;
      let finished = false;
          ...
          columns.forEach((col) =>
            fillValues(
              ...
              () =>
                ...
                (this.isLeft
                  ? initialRange.columns[0]
                  : _.last(initialRange.columns)),
            ),
          );
        }

        finished = this.rowPositionUtils.sameRow(
          currentRow,
          this.isUp ? finalRangeStartRow : finalRangeEndRow,
        );
    ...
    const fillValues = (
      ...
      if (withinInitialRange) {
        ...
        currentValue = this.processValues(
          e,
          values,
          initialValues,
          col,
    ...
    if (isVertical) {
      ...
      const columns = this.isLeft
        ? [...finalRange.columns].reverse()
        : finalRange.columns;
  ...
  private markPathFrom(
    ...
    if (this.dragAxis === 'y') {
      ...
      const isBefore = this.rowPositionUtils.before(
        currentPosition,
        initialPosition,
      );
      const rangeStartRow = this.getRangeStartRow();
      ...
      if (
        ...
        currentIndex >= displayedColumns.indexOf(this.getCellRange().columns[0])
      ) {
        this.reduceHorizontal(initialPosition, currentPosition);
        this.isReduce = true;
      } else {
        this.extendHorizontal(
          initialPosition,
          currentPosition,
          currentIndex < initialIndex,
        );
  ...
  private extendVertical(
    ...
    do {
      ...
      for (let i = 0; i < colLen; i++) {
        ...
        const cellInRange = rangeController.isCellInSpecificRange(
          cellPos,
          cellRange,
        );
        const isInitialRow = this.rowPositionUtils.sameRow(
          row,
          initialPosition,
        );
  ...
  private extendHorizontal(
    ...
    const startCol = allCols.indexOf(
      isMovingLeft ? endPosition.column : initialPosition.column,
    );
