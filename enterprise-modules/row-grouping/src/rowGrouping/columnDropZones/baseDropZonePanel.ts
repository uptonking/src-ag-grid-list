...
export abstract class BaseDropZonePanel extends Component {
  ...
  private isInterestedIn(type: DragSourceType): boolean {
    ...
    return (
      type === DragSourceType.HeaderCell || type === DragSourceType.ToolPanel
    );
  }

  private checkInsertIndex(draggingEvent: DraggingEvent): boolean {
    const newIndex = this.horizontal
      ? this.getNewHorizontalInsertIndex(draggingEvent)
      : this.getNewVerticalInsertIndex(draggingEvent);
  ...
  private getNewVerticalInsertIndex(draggingEvent: DraggingEvent): number {
    ...
    this.childColumnComponents.forEach((childColumn) => {
      ...
      const verticalFit =
        mouseEvent.clientY >=
        (draggingEvent.vDirection === VerticalDirection.Down
          ? rect.top
          : rect.bottom);
  ...
  private checkDragStartedBySelf(draggingEvent: DraggingEvent): void {
    ...
    this.potentialDndColumns =
      draggingEvent.dragSource.getDragItem().columns || [];
    this.refreshGui();
  ...
  private onDragEnter(draggingEvent: DraggingEvent): void {
    ...
    const goodDragColumns = dragColumns.filter(
      this.isColumnDroppable.bind(this),
    );
  ...
  private removeColumns(columnsToRemove: Column[]): void {
    const newColumnList = this.getExistingColumns().filter(
      (col) => !_.includes(columnsToRemove, col),
    );
  ...
  private getNonGhostColumns(): Column[] {
    ...
    if (this.isPotentialDndColumns()) {
      return existingColumns.filter(
        (column) => !_.includes(this.potentialDndColumns, column),
      );
    } else {
  ...
  private createColumnComponent(
    ...
    const columnComponent = new DropZoneColumnComp(
      column,
      this.dropTarget,
      ghost,
      this.valueColumn,
    ...
    columnComponent.addEventListener(
      DropZoneColumnComp.EVENT_COLUMN_REMOVE,
      this.removeColumns.bind(this, [column]),
    );
  ...
  private addArrow(eParent: HTMLElement): void {
    ...
    if (this.horizontal) {
      ...
      const enableRtl = this.beans.gridOptionsWrapper.isEnableRtl();
      const icon = _.createIconNoSpan(
        enableRtl ? 'smallLeft' : 'smallRight',
        this.beans.gridOptionsWrapper,
      );
