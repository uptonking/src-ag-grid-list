...
export class DropZoneColumnComp extends Component {
  ...
  public init(): void {
    ...
    this.eButton.appendChild(
      _.createIconNoSpan('cancel', this.gridOptionsWrapper),
    );
  ...
  private setupRemove(): void {
    ...
    this.addManagedListener(this.eButton, 'click', (mouseEvent: MouseEvent) => {
      const agEvent: ColumnRemoveEvent = {
  ...
  private setTextValue(): void {
    ...
    if (this.valueColumn) {
      ...
      const aggFuncString =
        typeof aggFunc === 'string' ? (aggFunc as string) : 'agg';
