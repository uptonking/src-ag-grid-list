...
export class GroupStage extends BeanStub implements IRowNodeStage {
  ...
  private createGroupingDetails(params: StageExecuteParams): GroupingDetails {
    ...
    const groupedCols = this.usingTreeData
      ? null
    ...
    const details: GroupingDetails = {
      ...
      groupedColCount:
        this.usingTreeData || !groupedCols ? 0 : groupedCols.length,
  ...
  private sortGroupsWithComparator(rootNode: RowNode): void {
    ...
    function recursiveSort(rowNode: RowNode): void {
      ...
      if (doSort) {
        ...
        rowNode.childrenAfterGroup.forEach((childNode: RowNode) =>
          recursiveSort(childNode),
        );
      }
    }
  }
  ...
  private moveNodesInWrongPath(
    ...
    childNodes.forEach((childNode) => {
      ...
      const oldPath: string[] = this.getExistingPathForNode(
        childNode,
        details,
      ).map(infoToKeyMapper);
      const newPath: string[] = this.getGroupInfo(childNode, details).map(
        infoToKeyMapper,
      );
  ...
  private postRemoveCreateFillerNodes(
    ...
    nodesToRemove.forEach((nodeToRemove) => {
      ...
      if (replaceWithGroup) {
        ...
        const newGroupNode = this.findParentForNode(
          nodeToRemove,
          oldPath,
          details,
        );
        ...
        newGroupNode.childrenAfterGroup.forEach(
          (rowNode) => (rowNode.parent = newGroupNode),
        );
      }
    });
  }
  ...
  private postRemoveRemoveEmptyGroups(
    ...
    const groupShouldBeRemoved = (rowNode: RowNode): boolean => {
      ...
      const mapKey = this.getChildrenMappedKey(
        rowNode.key,
        rowNode.rowGroupColumn,
      );
      const parentRowNode = rowNode.parent;
      const groupAlreadyRemoved =
        parentRowNode && parentRowNode.childrenMapped
  ...
  private shotgunResetEverything(
    ...
    const skipStage = afterColumnsChanged
      ? this.usingTreeData ||
        this.areGroupColsEqual(details, this.oldGroupingDetails)
      : false;
    ...
    details.rootNode.leafGroup = this.usingTreeData
      ? false
      : details.groupedCols.length === 0;
  ...
  private swapGroupWithUserNode(
    ...
    userGroup.childrenAfterGroup.forEach(
      (rowNode: RowNode) => (rowNode.parent = userGroup),
    );
  ...
  private getOrCreateNextNode(
    ...
    const mapKey = this.getChildrenMappedKey(
      groupInfo.key,
      groupInfo.rowGroupColumn,
    );
    let nextNode = parentGroup.childrenMapped
      ? (parentGroup.childrenMapped[mapKey] as RowNode)
      : undefined;
    if (!nextNode) {
  ...
  private createGroup(
    ...
    const groupDisplayCols: Column[] =
      this.columnController.getGroupDisplayColumns();

    groupDisplayCols.forEach((col) => {
      // newGroup.rowGroupColumn=null when working off GroupInfo, and we always display the group in the group column
      // if rowGroupColumn is present, then it's grid row grouping and we only include if configuration says so
      const displayGroupForCol =
        this.usingTreeData ||
        (groupNode.rowGroupColumn
          ? col.isRowGroupDisplayed(groupNode.rowGroupColumn.getId())
          : false);
    ...
    groupNode.leafGroup = this.usingTreeData
      ? false
      : level === details.groupedColCount - 1;
  ...
  private getGroupInfoFromCallback(rowNode: RowNode): GroupInfo[] {
    const keys: string[] | null = this.getDataPath
      ? this.getDataPath(rowNode.data)
      : null;
    ...
    const groupInfoMapper = (key: string | null) =>
      ({ key, field: null, rowGroupColumn: null } as GroupInfo);
