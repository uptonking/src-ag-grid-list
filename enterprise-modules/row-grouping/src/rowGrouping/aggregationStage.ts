...
export class AggregationStage extends BeanStub implements IRowNodeStage {
  ...
  public execute(params: StageExecuteParams): any {
    ...
    const noValueColumns = _.missingOrEmpty(
      this.columnController.getValueColumns(),
    );
    const noUserAgg = !this.gridOptionsWrapper.getGroupRowAggNodesFunc();
    const changedPathActive =
      params.changedPath && params.changedPath.isActive();
  ...
  private createAggDetails(params: StageExecuteParams): AggregationDetails {
    ...
    const pivotColumns = pivotActive
  ...
  private recursivelyCreateAggData(aggDetails: AggregationDetails) {
    const callback = (rowNode: RowNode) => {
      ...
      if (isRootNode) {
        ...
        const suppressAggAtRootLevel =
          this.gridOptionsWrapper.isSuppressAggAtRootLevel();
  ...
  private aggregateRowNodeUsingValuesAndPivot(rowNode: RowNode): any {
    ...
    pivotColumnDefs
      ...
      .forEach((valueColDef) => {
        ...
        result[colId] = this.aggregateValues(
          values,
    ...
    pivotColumnDefs
      ...
      .forEach((totalColDef) => {
        ...
        result[colId as string] = this.aggregateValues(
          aggResults,
          (pivotValueColumn as Column).getAggFunc(),
          pivotValueColumn,
          rowNode,
        );
      });
  ...
  private aggregateRowNodeUsingValuesOnly(
    ...
    const changedValueColumns = aggDetails.changedPath.isActive()
      ? aggDetails.changedPath.getValueColumnsForNode(
          rowNode,
    ...
    const notChangedValueColumns = aggDetails.changedPath.isActive()
      ? aggDetails.changedPath.getNotValueColumnsForNode(
          rowNode,
          aggDetails.valueColumns,
        )
      : null;
  ...
  public aggregateValues(
    ...
    if (typeof aggFunction !== 'function') {
      console.error(
        `ag-Grid: unrecognised aggregation function ${aggFuncOrString}`,
      );
      return null;
    }
