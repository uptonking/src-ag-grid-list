...
export class PivotColDefService extends BeanStub {
  ...
  public createPivotColumnDefs(uniqueValues: any): PivotColDefServiceResult {
    ...
    const pivotColumnDefsClone: ColDef[] = pivotColumnDefs.map((colDef) =>
      _.cloneObject(colDef),
    );
  ...
  private recursivelyAddPivotTotal(
    ...
    group.children.forEach((grp: ColDef | ColGroupDef) => {
      const childColIds = this.recursivelyAddPivotTotal(
        grp,
        pivotColumnDefs,
        columnIdSequence,
        valueColumn,
    ...
    if (group.children.length > 1) {
      ...
      const totalColDef = this.createColDef(
        valueColumn,
        'Total',
        groupDef.pivotKeys,
        columnIdSequence,
  ...
  private addRowGroupTotals(
    ...
    const valueCols = insertAfter
      ? valueColumns.slice()
      : valueColumns.slice().reverse();
    ...
    for (let i = 0; i < valueCols.length; i++) {
      ...
      let colIds: any[] = [];
      pivotColumnGroupDefs.forEach((groupDef: ColGroupDef | ColDef) => {
        colIds = colIds.concat(
          this.extractColIdsForValueColumn(groupDef, valueCol),
        );
      });

      const levelsDeep = pivotColumns.length;
      this.createRowGroupTotal(
        pivotColumnGroupDefs,
        pivotColumnDefs,
        1,
        [],
  ...
  private createRowGroupTotal(
    ...
    if (createGroup) {
      ...
      insertAfter
        ? parentChildren.push(groupDef)
        : parentChildren.unshift(groupDef);
      ...
      if (measureColumns.length === 0) {
        ...
        const columnName: string | null =
          this.columnController.getDisplayNameForColumn(valueColumn, 'header');
        const colDef = this.createColDef(
          valueColumn,
          columnName,
          newPivotKeys,
      ...
      insertAfter
        ? parentChildren.push(valueGroup)
        : parentChildren.unshift(valueGroup);
    }
  }
