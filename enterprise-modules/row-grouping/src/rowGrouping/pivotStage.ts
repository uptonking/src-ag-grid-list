...
export class PivotStage extends BeanStub implements IRowNodeStage {
  ...
  private executePivotOn(
    ...
    const aggregationColumnsHash = aggregationColumns
      ...
      .join('#');
    const aggregationFuncsHash = aggregationColumns
      .map((column) => column.getAggFunc().toString())
      .join('#');

    const aggregationColumnsChanged =
      this.aggregationColumnsHashLastTime !== aggregationColumnsHash;
    const aggregationFuncsChanged =
    ...
    if (
      ...
      const result = this.pivotColDefService.createPivotColumnDefs(
        this.uniqueValues,
      );
  ...
  private bucketRowNode(rowNode: RowNode, uniqueValues: any): void {
    ...
    rowNode.childrenMapped = this.bucketChildren(
  ...
  private bucketChildren(
    ...
    if (pivotIndex === pivotColumns.length - 1) {
      ...
      _.iterateObject(mappedChildren, (key: string, value: RowNode[]) => {
        result[key] = this.bucketChildren(
          value,
          pivotColumns,
          pivotIndex + 1,
          uniqueValues[key],
