...
const resetStylesheetValues = (): void => {
  ...
  registeredFonts = [
    { name: 'Calibri', size: 14, colorTheme: '1', family: 2, scheme: 'minor' },
  ];
...
const registerNumberFmt = (format: string): number => {
  ...
  let pos = _.findIndex(
    registeredNumberFmts,
    (currentFormat) => currentFormat.formatCode === format,
  );
...
const registerBorders = (borders: ExcelBorders): number => {
  ...
  let bottomStyle: BorderProperty,
    topStyle: BorderProperty,
    leftStyle: BorderProperty,
    rightStyle: BorderProperty;
  let bottomColor: BorderProperty,
    topColor: BorderProperty,
    leftColor: BorderProperty,
    rightColor: BorderProperty;
  ...
  if (borderBottom) {
    bottomStyle = convertLegacyBorder(
...
const registerStyle = (config: ExcelStyle): void => {
  const { id, alignment, borders, font, interior, numberFormat, protection } =
    config;
  let currentFill = 0;
  let currentBorder = 0;
  let currentFont = 0;
...
const stylesheetFactory: ExcelOOXMLTemplate = {
  getTemplate() {
    ...
    const cellStylesXfs = cellStylesXfsFactory.getTemplate(
