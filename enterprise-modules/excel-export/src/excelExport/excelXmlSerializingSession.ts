...
export class ExcelXmlSerializingSession extends BaseGridSerializingSession<
  ExcelCell[][]
> {
  ...
  public prepare(columnsToExport: Column[]): void {
    ...
    this.cols = columnsToExport.map((col, i) =>
      this.convertColumnToExcel(col, i),
    );
  }
      ...
      onColumn: (header: string, index: number, span: number) => {
        ...
        currentCells.push(
          this.createMergedCell(
            styleIds && styleIds.length > 0 ? styleIds[0] : undefined,
            'String',
            header,
            span,
          ),
  ...
  onNewRow(
    ...
    return {
      onColumn: onNewColumnAccumulator.bind(
        this,
        this.rows.length,
        currentCells,
      )(),
  ...
  public parse(): string {
    ...
    const longestRow = this.rows.reduce(
      (a, b) => Math.max(a, b.cells.length),
      0,
    );
  ...
  onNewBodyColumn(
    ...
    return (column, index, node) => {
      const valueForCell = this.extractRowCellValue(
        column,
        index,
        Constants.EXPORT_TYPE_EXCEL,
        node,
      );
      const styleIds: string[] = this.config.styleLinker(
        RowType.BODY,
  ...
  protected getDataTypeForValue(
    valueForCell: any,
  ): ExcelOOXMLDataType | ExcelDataType {
    ...
    function getType(): ExcelDataType {
      if (styleExists && actualStyle.dataType) {
        switch (actualStyle.dataType) {
          ...
          default:
            console.warn(
              `ag-grid: Unrecognized data type for excel export [${actualStyle.id}.dataType=${actualStyle.dataType}]`,
            );
        }
      }
