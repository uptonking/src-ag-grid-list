...
export class ServerSideBlock extends RowNodeBlock {
  ...
  constructor(
    ...
    params: ServerSideCacheParams,
    parentCache: ServerSideCache,
  ) {
    ...
    this.groupLevel = params.rowGroupCols
      ? this.level < params.rowGroupCols.length
      : undefined;
    this.leafGroup = params.rowGroupCols
      ? this.level === params.rowGroupCols.length - 1
      : false;
  }
  ...
  public getRow(displayRowIndex: number): RowNode | null {
    ...
    if (_.missing(topPointer) || _.missing(bottomPointer)) {
      console.warn(
        `ag-grid: error: topPointer = ${topPointer}, bottomPointer = ${bottomPointer}`,
      );
      return null;
    }
    ...
    while (true) {
      ...
      const expandedMasterRow =
        currentRowNode.master && currentRowNode.expanded;
      if (
        expandedMasterRow &&
        currentRowNode.detailNode.rowIndex === displayRowIndex
      ) {
      ...
      if (
        currentRowNode.expanded &&
        childrenCache &&
        childrenCache.isDisplayIndexInCache(displayRowIndex)
      ) {
      ...
      if (currentRowNode.rowIndex < displayRowIndex) {
        ...
        console.warn(
          `ag-Grid: error: unable to locate rowIndex = ${displayRowIndex} in cache`,
        );
        return null;
      }
    }
  ...
  protected setDataAndId(rowNode: RowNode, data: any, index: number): void {
    ...
    if (_.exists(data)) {
      ...
      rowNode.setRowHeight(
        this.gridOptionsWrapper.getRowHeightForNode(rowNode).height,
      );

      if (this.usingTreeData) {
        const getServerSideGroupKey =
          this.gridOptionsWrapper.getServerSideGroupKeyFunc();
        if (_.exists(getServerSideGroupKey) && getServerSideGroupKey) {
          rowNode.key = getServerSideGroupKey(rowNode.data);
        }

        const isServerSideGroup =
          this.gridOptionsWrapper.getIsServerSideGroupFunc();
  ...
  private setGroupDataIntoRowNode(rowNode: RowNode): void {
    ...
    groupDisplayCols.forEach((col) => {
      if (usingTreeData) {
        ...
        const groupValue = this.valueService.getValue(
          this.rowGroupColumn,
          rowNode,
        );
        if (_.missing(rowNode.groupData)) {
  ...
  public getRowBounds(
    ...
    console.error(
      ` ag-Grid: looking for invalid row index in Server Side Row Model, index=${index}`,
    );
  ...
  public isDisplayIndexInBlock(displayIndex: number): boolean {
    return (
      displayIndex >= this.displayIndexStart &&
      displayIndex < this.displayIndexEnd
    );
  }
