...
@Bean('rowModel')
export class ServerSideRowModel
  extends BeanStub
  implements IServerSideRowModel
{
  ...
  private onColumnEverything(): void {
    ...
    if (!this.cacheParams) {
      ...
      const rowGroupColumnVos = this.toValueObjects(
        this.columnController.getRowGroupColumns(),
      );
      const valueColumnVos = this.toValueObjects(
        this.columnController.getValueColumns(),
      );
      const pivotColumnVos = this.toValueObjects(
        this.columnController.getPivotColumns(),
      );

      const sortModelDifferent = !_.jsonEquals(
        this.cacheParams.sortModel,
        this.sortController.getSortModel(),
      );
      const rowGroupDifferent = !_.jsonEquals(
        this.cacheParams.rowGroupCols,
        rowGroupColumnVos,
      );
      const pivotDifferent = !_.jsonEquals(
        this.cacheParams.pivotCols,
        pivotColumnVos,
      );
      const valuesDifferent = !_.jsonEquals(
        this.cacheParams.valueCols,
        valueColumnVos,
      );
      resetRequired =
        sortModelDifferent ||
  ...
  private findChangedColumnsInSort(
    ...
    oldSortModel: { colId: string; sort: string }[],
  ): string[] {
    ...
    return allColsInBothSorts.filter((colId) => {
      ...
      return (
        differentSorts(oldSortItem, newSortItem) ||
        differentIndexes(oldSortItem, newSortItem)
      );
    });
  }
  ...
  private onSortChanged(): void {
    ...
    const changedColumnsInSort = this.findChangedColumnsInSort(
      newSortModel,
      oldSortModel,
    );
    ...
    const rowGroupColIds = this.columnController
      .getRowGroupColumns()
      .map((col) => col.getId());
    const serverSideCache = this.rootNode.childrenCache as ServerSideCache;

    const sortingWithValueCol =
      this.isSortingWithValueColumn(changedColumnsInSort);
    const sortingWithSecondaryCol =
      this.isSortingWithSecondaryColumn(changedColumnsInSort);

    const sortAlwaysResets =
      this.gridOptionsWrapper.isServerSideSortingAlwaysResets();
    if (sortAlwaysResets || sortingWithValueCol || sortingWithSecondaryCol) {
      this.reset();
    } else {
      serverSideCache.refreshCacheAfterSort(
  ...
  private onRowGroupOpened(event: any): void {
    ...
    if (rowNode.expanded) {
      ...
      _.exists(rowNode.childrenCache)
    ) {
    ...
    return columns.map(
      (col) =>
        ({
          ...
          displayName: this.columnController.getDisplayNameForColumn(
            col,
            'model',
          ),
  ...
  private createCacheParams(): ServerSideCacheParams {
    ...
    if (
      (maxBlocksInCache as number) >= 0 &&
      this.columnController.isAutoRowHeightActive()
    ) {
      console.warn(
  ...
  private executeOnCache(
    route: string[],
    callback: (cache: ServerSideCache) => void,
  ) {
    if (!this.cacheExists()) {
      return;
    }
  ...
  public getNodesInRangeForSelection(
    ...
    return firstInRange.parent!.childrenCache!.getRowNodesInRange(
      lastInRange,
      firstInRange,
    );
  }
  ...
  private extractSortModel(): { colId: string; sort: string }[] {
    ...
    const rowGroupCols = this.toValueObjects(
      this.columnController.getRowGroupColumns(),
    );
    ...
    if (autoGroupIndex > -1) {
      const individualGroupCols = rowGroupCols.map((group) => {
        return {
          colId: group.id,
      ...
      for (let i = 0; i < individualGroupCols.length; i++) {
        ...
        const sameNonGroupColumnExists = sortModel.some(
          (sm) => sm.colId === individualGroupCol.colId,
        );
    ...
    if (this.gridOptionsWrapper.isGroupMultiAutoColumn()) {
      ...
      for (let i = 0; i < sortModel.length; ++i) {
        if (sortModel[i].colId.indexOf(multiColumnPrefix) > -1) {
          sortModel[i].colId = sortModel[i].colId.substr(
            multiColumnPrefix.length,
          );
        }
      }
    }
  ...
  private isSortingWithValueColumn(changedColumnsInSort: string[]): boolean {
    const valueColIds = this.columnController
      .getValueColumns()
  ...
  private isSortingWithSecondaryColumn(
    ...
    const secondaryColIds = this.columnController
      .getSecondaryColumns()!
      .map((col) => col.getColId());
  ...
  private createDetailNode(masterNode: RowNode): RowNode {
    ...
    const rowHeight =
      this.gridOptionsWrapper.getRowHeightForNode(detailNode).height;
