...
export class ServerSideCache
  extends RowNodeCache<ServerSideBlock, ServerSideCacheParams>
  implements IServerSideCache
{
  ...
  public clearDisplayIndexes(): void {
    ...
    this.forEachBlockInOrder((block) =>
      block.clearDisplayIndexes(this.getVirtualRowCount()),
    );
  }
  ...
  public setDisplayIndexes(
    ...
    this.forEachBlockInOrder(
      (currentBlock: ServerSideBlock, blockId: number) => {
        ...
        currentBlock.setDisplayIndexes(
          displayIndexSeq,
          this.getVirtualRowCount(),
          nextRowTop,
        );
    ...
    if (_.missing(block)) {
      ...
      if (beforeBlock) {
        ...
        const isInRange = (): boolean => {
          return (
            displayRowIndex >= displayIndexStart &&
            displayRowIndex < displayIndexStart + blockSize
          );
        };
        ...
        nextRowTop =
          this.cacheTop + blockNumber * blockSize * this.cacheParams.rowHeight;
      }
      ...
      this.logger.log(
        `block missing, rowIndex = ${displayRowIndex}, creating #${blockNumber}, displayIndexStart = ${displayIndexStart}`,
      );
    }
  ...
  public getTopLevelRowDisplayedIndex(topLevelIndex: number): number {
    ...
    if (block) {
      ...
      this.forEachBlockInOrder(
        (currentBlock: ServerSideBlock, currentId: number) => {
      ...
      if (blockBefore) {
        ...
        const lastRowTopLevelIndex =
          blockSize * (blockBefore.getBlockNumber() + 1) - 1;

        // get the last top level node in the block before the wanted block. this will be the last
        // loaded displayed top level node.
        const lastRowNode = blockBefore!.getRowUsingLocalIndex(
          lastRowTopLevelIndex,
          true,
        );
        ...
        if (lastRowNode.expanded && lastRowNode.childrenCache) {
          ...
          lastDisplayedNodeIndexInBlockBefore =
            serverSideCache.getDisplayIndexEnd() - 1;
  ...
  private createBlock(
    ...
    const newBlock = new ServerSideBlock(
      blockNumber,
      this.parentRowNode,
      this.cacheParams,
      this,
  ...
  public getChildCache(keys: string[]): ServerSideCache | null {
    ...
    if (nextServerSideCache) {
      ...
      return nextServerSideCache
        ? nextServerSideCache.getChildCache(keyListForNextLevel)
        : null;
    } else {
      return null;
    }
      ...
      this.forEachBlockInOrder((block) => {
        if (block.isGroupLevel()) {
          const callback = (rowNode: RowNode) => {
            ...
            if (nextCache) {
              nextCache.refreshCacheAfterSort(
