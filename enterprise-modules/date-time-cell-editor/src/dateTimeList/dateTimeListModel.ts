...
const monthlyCalendarOptions: IDateTimeListModelOptions = {
  ...
  columnTitle(value) {
    return new Intl.DateTimeFormat('default', { weekday: 'narrow' }).format(
      value,
    );
  },
};
...
export class DateTimeListModel implements IDateTimeListModel {
  constructor(
    private options: IDateTimeListModelOptions = monthlyCalendarOptions,
  ) {}
  ...
  public getPage(date: Date, offset: number): Page {
    ...
    const paddingAtStart = modulo(
      options.columnForValue(firstValue),
      columnCount,
    ...
    const itemsInLastColumn =
      modulo(options.columnForValue(lastValue), columnCount) + 1;
    ...
    const titleFormat = new Intl.DateTimeFormat('default', {
      month: 'long',
      year: 'numeric',
    });
    return {
...
const modulo = (value: number, modulo: number) =>
