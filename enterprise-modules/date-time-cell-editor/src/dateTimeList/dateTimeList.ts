...
export class DateTimeList extends Component {
  private static TEMPLATE /*html*/ = `<div class="ag-date-time-list" tabindex="-1">
  ...
  constructor(options: DateTimeListOptions) {
    ...
    this.initialValue = this.model.roundToValue(
  ...
  public init(): void {
    ...
    this.addManagedListener(
      this.ePrevPageButton,
      'click',
      this.navigatePage.bind(this, -1),
    );
    this.addManagedListener(
      this.eNextPageButton,
      'click',
      this.navigatePage.bind(this, 1),
    );
    this.addManagedListener(
      this.getGui(),
      'focus',
      this.handleFocusChange.bind(this, true),
    );
    this.addManagedListener(
      this.getGui(),
      'blur',
      this.handleFocusChange.bind(this, false),
    );
    this.addManagedListener(
      this.getGui(),
      'keydown',
  ...
  private showPage(number: number) {
    ...
    this.rowComps.forEach((comp, i) =>
      comp.setDisplayed(i < page.entries.length),
    );

    page.columns.forEach(
      (column, i) => (this.getColumnLabel(i).textContent = column.label),
    );
    this.columnLabels.forEach((columnLabel, i) =>
      _.setDisplayed(columnLabel, i < page.columns.length),
    );
  }
  ...
  private getColumnLabel(index: number) {
    if (!this.columnLabels[index]) {
      const label = _.loadTemplate(
        `<div class="ag-date-time-list-page-column-label"></div>`,
      );
...
class DateTimeListPageEntriesRowComp extends Component {
  ...
  public setEntries(entries: Entry[], currentValue?: Date) {
    entries.forEach((entry, i) =>
      this.getEntryComponent(i).setEntry(entry, currentValue),
    );
  ...
  private getEntryComponent(index: number) {
    if (!this.entryComps[index]) {
      this.entryComps[index] = new DateTimeListPageEntryComp(
        this.onValueSelect,
      );
...
class DateTimeListPageEntryComp extends Component {
  ...
  private onDataChange() {
    ...
    _.addOrRemoveCssClass(
      ...
      this.entry.value &&
        this.currentValue &&
        this.entry.value.getTime() === this.currentValue.getTime(),
    );
  }
}
