...
const frameworkModules = ['react', 'angular', 'vue', 'polymer'];

const modules = glob
  .sync('../../community-modules/*')
  .concat(glob.sync('../../enterprise-modules/*'))
  .filter(
    (module) =>
      !frameworkModules.includes(
        module.replace(`../../community-modules/`, ''),
      ),
  )
  .filter(
    (module) =>
      !frameworkModules.includes(
        module.replace(`../../enterprise-modules/`, ''),
      ),
  )
...
const moduleRequireLines = modules.map(
  (module) => `var ${module.moduleName} = require('${module.directory}');`,
);
const moduleRegisterLines = modules
  .filter((module) => module.directory.indexOf('core') === -1) // exclude core - we don't register core
  .map(
    (module) =>
      `agGrid.ModuleRegistry.register(${module.moduleName}.${module.moduleName});`,
  );

const css = glob
  .sync('./dist/styles/*.css')
  .filter((css) => css.indexOf('.min.css') === -1)
  .map((css) => `require('${css}');`);

const webpackBase = fs
  .readFileSync('./webpack-base.config.js', 'UTF-8')
  .toString();
