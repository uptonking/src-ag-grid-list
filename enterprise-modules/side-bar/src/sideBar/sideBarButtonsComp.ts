...
export class SideBarButtonsComp extends Component {
  ...
  private handleKeyDown(e: KeyboardEvent): void {
    ...
    const prevEl = this.focusController.findNextFocusableElement(
      this.getFocusableElement(),
      null,
      true,
    );
    ...
    if (!prevEl) {
      const headerPosition = this.headerPositionUtils.findColAtEdgeForHeaderRow(
        0,
        'start',
      );
      if (!headerPosition) {
  ...
  private addButtonComp(def: ToolPanelDef): void {
    ...
    buttonComp.addEventListener(
      SideBarButtonComp.EVENT_TOGGLE_BUTTON_CLICKED,
      () => {
        this.dispatchEvent({
...
class SideBarButtonComp extends Component {
  ...
  private createTemplate(): string {
    ...
    const res = `<div class="ag-side-button">
