...
export class ToolPanelWrapper extends Component {
  ...
  private static TEMPLATE /* html */ = `<div class="ag-tool-panel-wrapper"/>`;
  ...
  public setToolPanelDef(toolPanelDef: ToolPanelDef): void {
    ...
    const componentPromise: Promise<IToolPanelComp> =
      this.userComponentFactory.newToolPanelComponent(toolPanelDef, params);

    if (componentPromise == null) {
      console.warn(
        `ag-grid: error processing tool panel component ${toolPanelDef.id}. You need to specify either 'toolPanel' or 'toolPanelFramework'`,
      );
      return;
    }
