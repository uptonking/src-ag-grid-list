...
export class SideBarComp extends Component implements ISideBar {
  ...
  private setupToolPanels(defs: ToolPanelDef[]): void {
    defs.forEach((def) => {
      ...
      if (def.toolPanel === 'agColumnsToolPanel') {
        const moduleMissing = !ModuleRegistry.assertRegistered(
          ModuleNames.ColumnToolPanelModule,
          'Column Tool Panel',
        );
        if (moduleMissing) {
          return;
  ...
  public getToolPanelInstance(key: string): IToolPanel | undefined {
    const toolPanelWrapper = this.toolPanelWrappers.filter(
      (toolPanel) => toolPanel.getToolPanelId() === key,
    )[0];

    if (!toolPanelWrapper) {
      console.warn(
        `ag-grid: unable to lookup Tool Panel as invalid key supplied: ${key}`,
      );
      return;
    }
