...
export class ToolPanelColDefService extends BeanStub {
  ...
  public createColumnTree(
    ...
    const createDummyColGroup = (
      ...
      if (this.isColGroupDef(abstractColDef)) {
        ...
        const groupId =
          typeof groupDef.groupId !== 'undefined'
            ? groupDef.groupId
            : groupDef.headerName;
        const group = new OriginalColumnGroup(
          groupDef,
          groupId as string,
          false,
          depth,
        ...
        const column = this.columnController.getPrimaryColumn(
          key,
        ) as OriginalColumnGroupChild;
  ...
  private getLeafPathTrees(): AbstractColDef[] {
    ...
    const getLeafPathTree = (
      node: Column | OriginalColumnGroup,
    ...
    return allPrimaryGridColumns.map((col) =>
      getLeafPathTree(col, col.getColDef()),
    );
  }
  ...
  private mergeLeafPathTrees(leafPathTrees: AbstractColDef[]) {
    const getLeafPathString = (leafPath: ColDef | ColGroupDef): string => {
      ...
      return group.children
        ? group.groupId + getLeafPathString(group.children[0])
        : '';
    };

    const matchingRootGroupIds = (
      pathA: AbstractColDef,
      pathB: AbstractColDef,
    ) => {
      const bothPathsAreGroups =
        this.isColGroupDef(pathA) && this.isColGroupDef(pathB);
      return bothPathsAreGroups && this.getId(pathA) === this.getId(pathB);
    };

    const mergeTrees = (
      treeA: AbstractColDef,
      treeB: AbstractColDef,
    ): AbstractColDef => {
      ...
      if (groupToMerge.children && groupToMerge.groupId) {
        const added = this.addChildrenToGroup(
          mergeResult,
  ...
  private addChildrenToGroup(
    ...
    const subGroupIsSplit = (
      currentGroup: ColGroupDef,
      groupToAdd: ColGroupDef,
    ) => {
      const existingChildIds = currentGroup.children.map(this.getId);
      const childGroupAlreadyExists = _.includes(
        existingChildIds,
        this.getId(groupToAdd),
      );
      const lastChild = _.last(currentGroup.children);
      const lastChildIsDifferent =
        lastChild && this.getId(lastChild) !== this.getId(groupToAdd);
    ...
    if (currentGroup.groupId === groupId) {
      ...
      const existingChildIds = currentGroup.children.map(this.getId);
      const colDefAlreadyPresent = _.includes(
        existingChildIds,
        this.getId(groupToAdd),
      );
    ...
    currentGroup.children.forEach((subGroup) =>
      this.addChildrenToGroup(subGroup, groupId, colDef),
    );
    return false;
  }
  ...
  private getId = (colDef: AbstractColDef): string | undefined => {
    return this.isColGroupDef(colDef)
      ? (colDef as ColGroupDef).groupId
      : (colDef as ColDef).colId;
  };
}
