...
export class PrimaryColsListPanel extends ManagedFocusComponent {
  ...
  public init(params: ToolPanelColumnCompParams, allowDragging: boolean): void {
    ...
    eventsImpactingCheckedState.forEach((event) => {
      ...
      this.addManagedListener(
        this.eventService,
        event,
        this.fireSelectionChangedEvent.bind(this),
      );
    });
  ...
  private recursivelyAddGroupComps(
    ...
    if (
      ...
      columnGroup.getColGroupDef().suppressColumnsToolPanel
    ) {
      return;
    }
  ...
  public setGroupsExpanded(expand: boolean, groupIds?: string[]): void {
    ...
    const unrecognisedGroupIds = groupIds.filter(
      (groupId) => !_.includes(expandedGroupIds, groupId),
    );
  ...
  public updateSelections(): void {
    if (this.columnApi.isPivotMode()) {
      ...
      this.columnComps.forEach((column) =>
      ...
      const colsToChange = primaryCols.filter(
        (col) => !col.getColDef().lockVisible,
      );
      ...
      if (!_.exists(this.filterText)) {
        this.columnController.setColumnsVisible(
          colsToChange,
      ...
      if (filteredCols.length > 0) {
        const filteredColsToChange = colsToChange.filter((col) =>
          _.includes(filteredCols, col.getColId()),
        );

        // update visibility of columns currently filtered
        this.columnController.setColumnsVisible(
          filteredColsToChange,
          this.selectAllChecked,
          'columnMenu',
        );
    ...
    columns.forEach((col) => {
      ...
      if (pivotMode) {
        const noPivotModeOptionsAllowed =
          !col.isAllowPivot() && !col.isAllowRowGroup() && !col.isAllowValue();
  ...
  private filterColumns(): void {
    ...
    const passesFilter = (item: OriginalColumnGroupChild) => {
      ...
      const isPaddingGroup =
        item instanceof OriginalColumnGroup && item.isPadding();
      if (isPaddingGroup) return false;

      const displayName = comp.getDisplayName();
      return displayName !== null
        ? displayName.toLowerCase().indexOf(this.filterText as string) >= 0
        : true;
    };

    const recursivelyCheckFilter = (
      item: OriginalColumnGroupChild,
      ...
      if (item instanceof OriginalColumnGroup) {
        ...
        item.getChildren().forEach((child) => {
          const childPasses = recursivelyCheckFilter(
            child,
            groupPasses || parentPasses,
          );
          if (childPasses) {
      ...
      const filterPasses =
        parentPasses || atLeastOneChildPassed ? true : passesFilter(item);
      const columnCompId = this.getColumnCompId(item);
  ...
  private recursivelySetVisibility(
    columnTree: any[],
    parentGroupsOpen: boolean,
  ): void {
    columnTree.forEach((child) => {
      const compId = this.getColumnCompId(child);
      let comp: ColumnItem = this.columnComps.get(compId);
      if (comp) {
        const filterResultExists =
          this.filterResults && _.exists(this.filterResults[compId]);
        const passesFilter = filterResultExists
          ? this.filterResults[compId]
          : true;
  ...
  private getColumnCompId = (columnGroupChild: OriginalColumnGroupChild) => {
    if (columnGroupChild instanceof OriginalColumnGroup) {
      // group comps are stored using a custom key (groupId + child colIds concatenated) as we need
      // to distinguish individual column groups after they have been split by user
      const childIds = columnGroupChild
        .getLeafColumns()
