...
export class ToolPanelColumnComp
  extends ManagedFocusComponent
  implements BaseColumnItem
{
  private static TEMPLATE /* html */ = `<div class="ag-column-select-column" tabindex="-1">
  ...
  private actionUnCheckedPivotMode(): void {
    ...
    if (column.isPivotActive()) {
      if (functionPassive) {
        const copyOfPivotColumns = this.columnController
          .getPivotColumns()
          .slice();
    ...
    if (column.isValueActive()) {
      if (functionPassive) {
        const copyOfValueColumns = this.columnController
          .getValueColumns()
          .slice();
    ...
    if (column.isRowGroupActive()) {
      if (functionPassive) {
        const copyOfRowGroupColumns = this.columnController
          .getRowGroupColumns()
          .slice();
  ...
  private onColumnStateChanged(): void {
    ...
    const checkboxPassive =
      isPivotMode && this.gridOptionsWrapper.isFunctionsPassive();
