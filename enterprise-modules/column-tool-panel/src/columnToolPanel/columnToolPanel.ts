...
export class ColumnToolPanel
  extends Component
  implements IColumnToolPanel, IToolPanelComp
{
    ...
    if (this.isRowGroupingModuleLoaded()) {
      if (!this.params.suppressRowGroups) {
        this.rowGroupDropZonePanel = this.createManagedBean(
          new RowGroupDropZonePanel(false),
        );
  ...
  private setLastVisible(): void {
    ...
    const columnDrops: HTMLElement[] = Array.prototype.slice.call(
      eGui.querySelectorAll('.ag-column-drop'),
    );

    columnDrops.forEach((columnDrop) =>
      _.removeCssClass(columnDrop, 'ag-last-column-drop'),
    );

    const lastVisible = _.last(
      eGui.querySelectorAll('.ag-column-drop:not(.ag-hidden)') as any,
    ) as HTMLElement;
