...
export class ToolPanelColumnGroupComp
  extends ManagedFocusComponent
  implements BaseColumnItem
{
  private static TEMPLATE /* html */ = `<div class="ag-column-select-column-group" tabindex="-1">
  ...
  public init(): void {
    ...
    this.eDragHandle = _.createIconNoSpan(
      'columnDrag',
    ...
    CssClassApplier.addToolPanelClassesFromColDef(
  ...
  private addVisibilityListenersToAllChildren(): void {
    this.columnGroup.getLeafColumns().forEach((column) => {
      ...
      this.addManagedListener(
        column,
        Column.EVENT_VALUE_CHANGED,
        this.onColumnStateChanged.bind(this),
      );
      this.addManagedListener(
        column,
        Column.EVENT_PIVOT_CHANGED,
        this.onColumnStateChanged.bind(this),
      );
      this.addManagedListener(
        column,
        Column.EVENT_ROW_GROUP_CHANGED,
        this.onColumnStateChanged.bind(this),
      );
    });
  }
  ...
  private setupExpandContract(): void {
    this.eGroupClosedIcon.appendChild(
      _.createIcon('columnSelectClosed', this.gridOptionsWrapper, null),
    );
    this.eGroupOpenedIcon.appendChild(
      _.createIcon('columnSelectOpen', this.gridOptionsWrapper, null),
    );
  ...
  private onChangeCommon(nextState: boolean): void {
    ...
    if (this.columnController.isPivotMode()) {
      ...
      const isAllowedColumn = (c: Column) =>
        !c.getColDef().lockVisible && !c.getColDef().suppressColumnsToolPanel;
      const allowedColumns = childColumns.filter(isAllowedColumn);

      const filterResults = this.getFilterResultsCallback();
      const passesFilter = (c: Column) =>
        !filterResults || filterResults[c.getColId()];
  ...
  private actionUnCheckedReduce(columns: Column[]): void {
    ...
    if (columnsToUnGroup.length > 0) {
      this.columnController.removeRowGroupColumns(
        columnsToUnGroup,
        'toolPanelUi',
      );
    }
  ...
  private workOutSelectedValue(): boolean | undefined {
    ...
    for (let i = 0; i < len; i++) {
      ...
      const type = this.isColumnVisible(column, pivotMode)
        ? 'visible'
        : 'hidden';
