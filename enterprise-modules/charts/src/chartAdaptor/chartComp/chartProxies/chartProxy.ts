...
export abstract class ChartProxy<
  ...
  TOptions extends ChartOptions<any>,
> {
  ...
  protected getFontColor = (): string =>
    this.isDarkTheme() ? 'rgb(221, 221, 221)' : 'rgb(87, 87, 87)';
  protected getAxisGridColor = (): string =>
    this.isDarkTheme() ? 'rgb(100, 100, 100)' : 'rgb(219, 219, 219)';
  protected getBackgroundColor = (): string =>
    this.isDarkTheme() ? '#2d3436' : 'white';
  ...
  protected initChartOptions(): void {
    ...
    if (processChartOptions) {
      const params: ProcessChartOptionsParams = {
        type: this.chartType,
  ...
  private overridePalette(chartOptions: TOptions): void {
    ...
    const { fills: defaultFills, strokes: defaultStrokes } =
      this.getPredefinedPalette();
    const { seriesDefaults } = chartOptions;
    const {
      fill: { colors: fills },
      stroke: { colors: strokes },
    } = seriesDefaults;
    const fillsOverridden = fills && fills.length > 0 && fills !== defaultFills;
    const strokesOverridden =
      strokes && strokes.length > 0 && strokes !== defaultStrokes;
  ...
  public setSeriesOption(expression: string, value: any): void {
    if (
      _.get(this.chartOptions.seriesDefaults, expression, undefined) === value
    ) {
      // option is already set to the specified value
      return;
    }
  ...
  public getChartPaddingOption = (property: keyof PaddingOptions): string =>
    this.chartOptions.padding ? `${this.chartOptions.padding[property]}` : '';

  public setChartPaddingOption(
    property: keyof PaddingOptions,
    value: number,
  ): void {
  ...
  public setShadowProperty(
    property: keyof DropShadowOptions,
    value: any,
  ): void {
    const { seriesDefaults } = this.chartOptions;
    ...
    const series = this.getChart().series as (
      | BarSeries
      | AreaSeries
      | PieSeries
    )[];
