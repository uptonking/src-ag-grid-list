...
export abstract class CartesianChartProxy<
  T extends SeriesOptions,
> extends ChartProxy<
  CartesianChart | GroupedCategoryChart,
  CartesianChartOptions<T>
> {
  ...
  protected updateLabelRotation(categoryId: string, isHorizontalChart = false) {
    ...
    if (
      categoryId !== ChartDataModel.DEFAULT_CATEGORY &&
      !this.chartProxyParams.grouping
    ) {
    ...
    const axisPosition = isHorizontalChart
      ? ChartAxisPosition.Left
      : ChartAxisPosition.Bottom;
    const axis = find(
      this.chart.axes,
      (axis) => axis.position === axisPosition,
    );
  ...
  protected getDefaultCartesianChartOptions(): CartesianChartOptions<SeriesOptions> {
    const options =
      this.getDefaultChartOptions() as CartesianChartOptions<SeriesOptions>;
  ...
  protected updateAxes(
    baseAxisType: AxisType = 'category',
  ...
  protected getXAxis(): ChartAxis {
    return find(
      this.chart.axes,
      (a) => a.position === ChartAxisPosition.Bottom,
    );
  }
