...
export class AreaChartProxy extends CartesianChartProxy<AreaSeriesOptions> {
  ...
  protected createChart(
    ...
    const chart = ChartBuilder[
      grouping ? 'createGroupedAreaChart' : 'createAreaChart'
    ](parentElement, options || this.chartOptions);

    chart.axes
      .filter(
        (axis) =>
          axis.position === ChartAxisPosition.Bottom &&
          axis instanceof CategoryAxis,
      )
      .forEach((axis) => {
  ...
  public update(params: UpdateChartParams): void {
    ...
    if (this.chartType === ChartType.Area) {
      ...
      if (!areaSeries) {
        areaSeries = ChartBuilder.createSeries(
  ...
  private updateAreaChart(params: UpdateChartParams): void {
    ...
    const existingSeriesById = (chart.series as AreaSeries[]).reduceRight(
      (map, series, i) => {
  ...
  protected getDefaultOptions(): CartesianChartOptions<AreaSeriesOptions> {
    const options =
      this.getDefaultCartesianChartOptions() as CartesianChartOptions<AreaSeriesOptions>;
  ...
  private getSeriesDefaults(): InternalAreaSeriesOptions {
    return {
      ...
      type: 'area',
      normalizedTo:
        this.chartType === ChartType.NormalizedArea ? 100 : undefined,
    };
  }
}
