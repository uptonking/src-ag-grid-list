...
export class ScatterChartProxy extends CartesianChartProxy<ScatterSeriesOptions> {
  ...
  protected createChart(
    ...
    return ChartBuilder.createScatterChart(
  ...
  public update(params: UpdateChartParams): void {
    ...
    const seriesDefinitions = this.getSeriesDefinitions(
      fields,
      seriesDefaults.paired,
    );
    ...
    const labelFieldDefinition =
      params.category.id === ChartDataModel.DEFAULT_CATEGORY
        ? undefined
        : params.category;

    const existingSeriesById = (chart.series as ScatterSeries[]).reduceRight(
      (map, series, i) => {
        const matchingIndex = _.findIndex(
          ...
          (s: any) =>
            ...
            ((!s.sizeField && !series.sizeKey) ||
              (s.sizeField && s.sizeField.colId === series.sizeKey)),
        );
    ...
    seriesDefinitions.forEach((seriesDefinition, index) => {
      const existingSeries = existingSeriesById.get(
        seriesDefinition.yField.colId,
      );
      const series =
        existingSeries ||
        (ChartBuilder.createSeries(seriesOptions) as ScatterSeries);
  ...
  public getTooltipsEnabled(): boolean {
    return (
      this.chartOptions.seriesDefaults.tooltip != null &&
  ...
  protected getDefaultOptions(): CartesianChartOptions<ScatterSeriesOptions> {
    ...
    const options =
      this.getDefaultCartesianChartOptions() as CartesianChartOptions<ScatterSeriesOptions>;
