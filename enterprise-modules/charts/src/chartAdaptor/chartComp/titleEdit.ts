...
export class TitleEdit extends Component {
  private static TEMPLATE /* html */ = `<input
  ...
  public setChartProxy(chartProxy: ChartProxy<Chart, any>) {
    ...
    const destroyDbleClickListener = this.addManagedListener(
      canvas,
      'dblclick',
      (event) => {
        const { title } = chart;
    ...
    const destroyMouseMoveListener = this.addManagedListener(
      canvas,
      'mousemove',
      (event) => {
        const { title } = chart;
        ...
        const inTitle =
          title && title.node.containsPoint(event.offsetX, event.offsetY);
  ...
  private startEditing(titleBBox: BBox): void {
    ...
    const inputWidth = Math.max(
      Math.min(titleBBox.width + 20, maximumInputWidth),
      minimumTargetInputWidth,
    );
    ...
    const inputValue =
      oldTitle === this.chartTranslator.translate('titlePlaceholder')
        ? ''
        : oldTitle;
    ...
    inputStyle.left =
      Math.round(titleBBox.x + titleBBox.width / 2 - inputWidth / 2) + 'px';
    inputStyle.top =
      Math.round(titleBBox.y + titleBBox.height / 2 - inputRect.height / 2) +
      'px';
