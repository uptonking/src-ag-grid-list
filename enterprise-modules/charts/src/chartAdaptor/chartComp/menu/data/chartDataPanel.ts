...
export class ChartDataPanel extends Component {
  ...
  private columnComps: Map<string, AgRadioButton | AgCheckbox> = new Map<
    string,
    AgRadioButton | AgCheckbox
  >();
  ...
  private updatePanels() {
    ...
    const { dimensionCols, valueCols } =
      this.chartController.getColStateForMenu();
    const colIds = dimensionCols
      .map((c) => c.colId)
      .concat(valueCols.map((c) => c.colId));

    this.chartType = this.chartController.getChartType();

    if (
      _.areEqual(_.keys(this.columnComps), colIds) &&
  ...
  private addChangeListener(
    component: AgRadioButton | AgCheckbox,
  ...
  private createCategoriesGroupComponent(columns: ColState[]): void {
    ...
    columns.forEach((col) => {
      const comp = this.categoriesGroupComp!.createManagedBean(
        new AgRadioButton(),
      );
  ...
  private createSeriesGroupComponent(columns: ColState[]): void {
    ...
    if (this.chartController.isActiveXYChart()) {
      const pairedModeToggle = this.seriesGroupComp.createManagedBean(
        new AgToggleButton(),
      );
  ...
  private addDragHandle(comp: AgCheckbox, col: ColState): void {
    const eDragHandle = _.createIconNoSpan(
      'columnDrag',
  ...
  private generateGetSeriesLabel(): (col: ColState) => string {
    ...
    return (col: ColState): string => {
      ...
      if (isInPairedMode) {
        axisLabel = indexToAxisLabel.get(
          selectedValuesCount % (isBubble ? 3 : 2),
        );
      } else {
  ...
  private checkInsertIndex(draggingEvent: DraggingEvent): boolean {
    ...
    const changed =
      this.insertIndex !== undefined && newIndex !== this.insertIndex;
