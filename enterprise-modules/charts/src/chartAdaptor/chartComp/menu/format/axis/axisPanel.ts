...
export class AxisPanel extends Component {
  ...
  private initAxis() {
    ...
    this.axisColorInput
      ...
      .onValueChange((newColor) =>
        this.getChartProxy().setAxisProperty('line.color', newColor),
      );
    ...
    this.axisLineWidthSlider
      ...
      .onValueChange((newValue) =>
        this.getChartProxy().setAxisProperty('line.width', newValue),
      );
  ...
  private initAxisTicks() {
    const axisTicksComp = this.createBean(
      new AxisTicksPanel(this.chartController),
    );
  ...
  private addAdditionalLabelComps(labelPanelComp: FontPanel) {
    const createAngleComp = (
      label: string,
      initialValue: number,
      updateFunc: (value: number) => void,
    ) => {
      const rotationInput = this.createBean(
        new AgAngleSelect()
          .setLabel(label)
          .setLabelWidth('flex')
    ...
    const createLabelUpdateFunc =
      (axisPosition: ChartAxisPosition) => (newValue: number) => {
        const chartProxy = this.getChartProxy();
        ...
        if (axis) {
          ...
          if (axis.position === ChartAxisPosition.Bottom) {
            _.set(
              chartProxy.getChartOptions().xAxis,
              'label.rotation',
              newValue,
            );
          } else if (axis.position === ChartAxisPosition.Left) {
            _.set(
              chartProxy.getChartOptions().yAxis,
              'label.rotation',
              newValue,
            );
          }
    ...
    const xRotationLabel = `${this.chartTranslator.translate(
      'xRotation',
    )} ${degreesSymbol}`;
    const yRotationLabel = `${this.chartTranslator.translate(
      'yRotation',
    )} ${degreesSymbol}`;
    ...
    createAngleComp(
      ...
      createLabelUpdateFunc(ChartAxisPosition.Bottom),
    );
    createAngleComp(
      yRotationLabel,
      this.getChartProxy().getChartOption('yAxis.label.rotation'),
      createLabelUpdateFunc(ChartAxisPosition.Left),
    );
    ...
    labelPaddingSlider
      ...
      .setTextFieldWidth(45)
      .onValueChange((newValue) =>
        this.getChartProxy().setAxisProperty('label.padding', newValue),
      );
