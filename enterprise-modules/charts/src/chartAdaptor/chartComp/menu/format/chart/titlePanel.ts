...
export default class TitlePanel extends Component {
  ...
  private initFontPanel(): void {
    ...
    const initialFont = {
      family: hasTitle
        ? chartProxy.getChartOption('title.fontFamily')
        : 'Verdana, sans-serif',
      style: hasTitle
        ? chartProxy.getChartOption<FontStyle>('title.fontStyle')
        : undefined,
      weight: hasTitle
        ? chartProxy.getChartOption<FontWeight>('title.fontWeight')
        : undefined,
    ...
    const fontPanelParams: FontPanelParams = {
      ...
      setEnabled: (enabled) => {
        ...
        if (enabled) {
          const newTitle =
            this.disabledTitle ||
            this.chartTranslator.translate('titlePlaceholder');
          chartProxy.setTitleOption('text', newTitle);
          this.disabledTitle = '';
        } else {
          this.disabledTitle = this.chartController
            .getChartProxy()
            .getTitleOption('text');
