...
export class ChartDataModel extends BeanStub {
  ...
  public isGrouping(): boolean {
    ...
    const groupedCols = usingTreeData
      ? null
      : this.columnController.getRowGroupColumns();
    const groupActive =
      usingTreeData || ((groupedCols && groupedCols.length > 0) as boolean);
  ...
  private displayNameMapper(col: ColState): ColState {
    ...
    col.displayName = columnNames
      ? columnNames.join(' - ')
      : this.getColDisplayName(col.column);
  ...
  private resetColumnState(): void {
    ...
    const valueColumnsFromReferenceRange =
      this.referenceCellRange.columns.filter((c) => valueCols.has(c));

    valueCols.forEach((column) => {
      // first time the value cell range is set, preserve the column order from the supplied range
      if (
        isInitialising &&
        _.includes(this.referenceCellRange.columns, column)
      ) {
  ...
  private updateColumnState(updatedCol: ColState): void {
    ...
    if (dimensionColState.filter(idsMatch).length > 0) {
      ...
      valueColState
        .filter(idsMatch)
        .forEach((cs) => (cs.selected = updatedCol.selected));
    }
    ...
    allColumns.forEach((col) => {
      ...
      col.order =
        order >= 0 ? orderedColIds.indexOf(col.colId) : allColumns.length - 1;
    });
  ...
  private setDimensionCellRange(
    ...
    if (
      !selectedDimensionColState ||
      !dimensionCols.has(selectedDimensionColState.column)
    ) {
      selectedDimensionColState = this.dimensionColState.filter(
        (cs) => cs.selected,
      )[0];
    }
    ...
    if (
      selectedDimensionColState &&
      selectedDimensionColState.colId !== ChartDataModel.DEFAULT_CATEGORY
    ) {
      this.dimensionCellRange = this.createCellRange(
        CellRangeType.DIMENSION,
        selectedDimensionColState.column,
      );
    }
  }
  ...
  private setValueCellRange(
    ...
    if (selectedValueCols.length > 0) {
      ...
      selectedValueCols.sort(
        (a, b) =>
          orderedColIds.indexOf(a.getColId()) -
          orderedColIds.indexOf(b.getColId()),
      );

      this.valueCellRange = this.createCellRange(
        CellRangeType.VALUE,
        ...selectedValueCols,
      );
    }
