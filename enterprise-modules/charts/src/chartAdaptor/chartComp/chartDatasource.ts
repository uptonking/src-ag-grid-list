...
export class ChartDatasource extends BeanStub {
  ...
  private extractRowsFromGridRowModel(params: ChartDatasourceParams): IData {
    ...
    const rangeLastRow =
      params.endRow >= 0 ? Math.min(params.endRow, modelLastRow) : modelLastRow;
    ...
    for (let i = 0; i < numRows; i++) {
      ...
      params.valueCols.forEach((col) => {
        ...
        data[col.getId()] =
          value != null && typeof value.toNumber === 'function'
            ? value.toNumber()
            : value;
      });
    ...
    if (params.grouping) {
      ...
      extractedRowData = extractedRowData.filter(
        (_1, index) => !_.includes(groupIndexesToRemove, index),
      );
    }
  ...
  private aggregateRowsByDimension(
    ...
    dataAggregated.forEach((groupItem) =>
      params.valueCols.forEach((col) => {
        const dataToAgg = groupItem.__children.map(
          (child: any) => child[col.getId()],
        );
        let aggResult: any = 0;
        ...
        if (
          ...
          aggResult = this.aggregationStage.aggregateValues(
            dataToAgg,
            params.aggFunc!,
          );
        }
        ...
        groupItem[col.getId()] =
          aggResult && typeof aggResult.value !== 'undefined'
            ? aggResult.value
            : aggResult;
      }),
    );
