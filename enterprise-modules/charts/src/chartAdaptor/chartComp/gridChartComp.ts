...
export class GridChartComp extends Component {
  ...
  private eTitleEditContainer: HTMLDivElement;
  ...
  public init(): void {
    ...
    this.chartController = this.createManagedBean(
      new ChartController(this.model, this.params.chartPaletteName),
    );
    ...
    this.addManagedListener(
      this.chartMenu,
      ChartMenu.EVENT_DOWNLOAD_CHART,
  ...
  private addDialog(): void {
    const title = this.chartTranslator.translate(
  ...
  private handleEmptyChart(data: any[], fields: any[]): boolean {
    ...
    const pivotModeDisabled =
      this.model.isPivotChart() && !this.model.isPivotMode();
    let minFieldsRequired = 1;

    if (this.chartController.isActiveXYChart()) {
      minFieldsRequired =
        this.model.getChartType() === ChartType.Bubble ? 3 : 2;
    }
