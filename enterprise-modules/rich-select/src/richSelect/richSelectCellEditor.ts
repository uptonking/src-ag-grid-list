...
export class RichSelectCellEditor
  extends PopupComponent
  implements ICellEditor
{
  // tab index is needed so we can focus, which is needed for keyboard events
  private static TEMPLATE /* html */ = `<div class="ag-rich-select" tabindex="-1">
  ...
  private runSearch() {
    ...
    const topSuggestion = _.fuzzySuggestions(
      this.searchString,
      searchStrings,
      true,
      true,
  ...
  private renderSelectedValue(): void {
    ...
    const promise: Promise<ICellRendererComp> =
      this.userComponentFactory.newCellRenderer(this.params, params);
    if (promise != null) {
