...
export class FloatingFilterWrapper extends AbstractHeaderWrapper {
  ...
  private static TEMPLATE /* html */ = `<div class="ag-header-cell" role="presentation" tabindex="-1">
  ...
  protected onTabKeyDown(e: KeyboardEvent) {
    ...
    const nextFocusableEl = this.focusController.findNextFocusableElement(
      eGui,
      null,
      e.shiftKey,
    );
  ...
  private setupLeftPositioning(): void {
    const setLeftFeature = new SetLeftFeature(
      this.column,
      this.getGui(),
      this.beans,
    );
  ...
  private setupSyncWithFilter(): void {
    const syncWithFilter = (filterChangedEvent: FilterChangedEvent) => {
      const parentModel = this.getFilterComponent().resolveNow(null, (filter) =>
        filter.getModel(),
      );
  ...
  private setupWithFloatingFilter(
    floatingFilterComp: IFloatingFilterComp,
  ): void {
    const disposeFunc = () => {
  ...
  private getFloatingFilterInstance(): Promise<IFloatingFilterComp> {
    ...
    if (typeof colDef.filter === 'string') {
      ...
      defaultFloatingFilterType =
        FloatingFilterWrapper.filterToFloatingFilterNames[colDef.filter];
      ...
      const setFilterModuleLoaded = ModuleRegistry.isRegistered(
        ModuleNames.SetFilterModule,
      );
      defaultFloatingFilterType = setFilterModuleLoaded
        ? 'agSetColumnFloatingFilter'
        : 'agTextColumnFloatingFilter';
    }

    const filterParams = this.filterManager.createFilterParams(
      this.column,
    ...
    this.suppressFilterButton = colDef.floatingFilterComponentParams
      ? !!colDef.floatingFilterComponentParams.suppressFilterButton
      : false;

    let promise = this.userComponentFactory.newFloatingFilterComponent(
      colDef,
      params,
      defaultFloatingFilterType,
    );
  ...
  private getFilterComponentPrototype(colDef: ColDef): { new (): any } {
    const resolvedComponent = this.userComponentFactory.lookupComponentClassDef(
      colDef,
      'filter',
  ...
  private currentParentModel(): any {
    return this.getFilterComponent().resolveNow(null, (filter) =>
      filter.getModel(),
    );
  }
  ...
  private onParentModelChanged(
    ...
    this.floatingFilterCompPromise.then((comp) =>
      comp.onParentModelChanged(model, filterChangedEvent),
    );
  }
