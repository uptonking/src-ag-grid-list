...
export class FilterManager extends BeanStub {
  ...
  public setFilterModel(model: { [key: string]: any }): void {
    ...
    if (model) {
      ...
      modelKeys.forEach((colId) => {
        ...
        allPromises.push(
          this.setModelOnFilterWrapper(
            filterWrapper.filterPromise,
            model[colId],
          ),
        );
      });
  ...
  private setAdvancedFilterPresent(): void {
    ...
    this.allFilters.forEach((filterWrapper) => {
      ...
      if (
        filterWrapper.filterPromise.resolveNow(false, (filter) =>
  ...
  private updateFilterFlagInColumns(
    ...
    this.allFilters.forEach((filterWrapper) => {
      const isFilterActive = filterWrapper.filterPromise.resolveNow(
        false,
        (filter) => filter.isFilterActive(),
      );

      filterWrapper.column.setFilterActive(
        isFilterActive,
        source,
  ...
  private doesFilterPass(node: RowNode, filterToSkip?: any): boolean {
    ...
    this.allFilters.forEach((filterWrapper) => {
      ...
      const filter = filterWrapper.filterPromise.resolveNow(
        undefined,
        (filter) => filter,
      );
  ...
  private doesRowPassQuickFilter(node: RowNode): boolean {
    ...
    return every(this.quickFilterParts, (part) =>
      usingCache
        ? this.doesRowPassQuickFilterCache(node, part)
        : this.doesRowPassQuickFilterNoCache(node, part),
    );
  }
  ...
  public doesRowPassFilter(node: any, filterToSkip?: any): boolean {
    ...
    if (
      this.externalFilterPresent &&
      !this.gridOptionsWrapper.doesExternalFilterPass(node)
    ) {
      return false;
    }
    ...
    if (
      this.advancedFilterPresent &&
      !this.doesFilterPass(node, filterToSkip)
    ) {
      return false;
    }
  ...
  private aggregateRowForQuickFilter(node: RowNode): void {
    ...
    node.quickFilterAggregateText = stringParts.join(
  ...
  public getOrCreateFilterWrapper(
    column: Column,
    source: FilterRequestSource,
  ): FilterWrapper {
  ...
  private createFilterInstance(
    ...
    const defaultFilter = ModuleRegistry.isRegistered(
    ...
    const modifyParamsCallback = (params: any, filterInstance: IFilterComp) =>
      assign(params, {
        doesRowPassOtherFilter: (node: RowNode) =>
          this.doesRowPassOtherFilters(filterInstance, node),
        filterChangedCallback: (additionalEventAttributes?: any) =>
          this.onFilterChanged(filterInstance, additionalEventAttributes),
      });

    const res = this.userComponentFactory.newFilterComponent(
      sanitisedColDef,
      params,
      defaultFilter,
  ...
  private createFilterWrapper(
    ...
    filterWrapper.scope = this.gridOptionsWrapper.isAngularCompileFilters()
      ? this.$scope.$new()
      : null;
    filterWrapper.filterPromise = this.createFilterInstance(
      column,
      filterWrapper.scope,
    );
    ...
    filterWrapper.guiPromise = new Promise<HTMLElement>((resolve) => {
      filterWrapper.filterPromise.then((filter) => {
        ...
        if (!exists(guiFromFilter)) {
          console.warn(
            `getGui method from filter returned ${guiFromFilter}, it should be a DOM element or an HTML template string.`,
          );
        }
        ...
        if (filterWrapper.scope) {
          const compiledElement = this.$compile(eFilterGui)(
            filterWrapper.scope,
          );
  ...
  private onNewColumnsLoaded(): void {
    ...
    this.allFilters.forEach((filterWrapper) => {
      const oldColumn = !this.columnController.getPrimaryColumn(
        filterWrapper.column,
      );
