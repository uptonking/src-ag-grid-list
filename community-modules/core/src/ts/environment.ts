...
export class Environment extends BeanStub {
  ...
  public getSassVariable(theme: string, key: SASS_PROPERTIES): number {
    ...
    if (SASS_PROPERTY_BUILDER[key]) {
      ...
      const el: HTMLDivElement = classList.reduce(
        (el: HTMLDivElement, currentClass: string, idx: number) => {
          if (idx === 0) {
      ...
      if (document.body) {
        ...
        const sizeName =
          key.toLowerCase().indexOf('height') !== -1 ? 'height' : 'width';
  ...
  public getTheme(): {
    ...
    if (usingOldTheme) {
      ...
      _.doOnce(
        () =>
          console.warn(
            `ag-Grid: As of v19 old theme are no longer provided. Please replace ${theme} with ${newTheme}.`,
          ),
        'using-old-theme',
      );
    }
