...
export class PaginationProxy extends BeanStub {
  ...
  private onModelUpdated(modelUpdatedEvent?: ModelUpdatedEvent): void {
    ...
    const paginationChangedEvent: PaginationChangedEvent = {
      ...
      keepRenderedRows: modelUpdatedEvent
        ? modelUpdatedEvent.keepRenderedRows
        : false,
      api: this.gridApi,
  ...
  public isRowPresent(rowNode: RowNode): boolean {
    ...
    const nodeIsInPage =
      rowNode.rowIndex >= this.topDisplayedRowIndex &&
      rowNode.rowIndex <= this.bottomDisplayedRowIndex;
    return nodeIsInPage;
  }
  ...
  private calculatePages(): void {
    ...
    this.pixelOffset = _.exists(this.topRowBounds)
      ? this.topRowBounds.rowTop
      : 0;
  }
  ...
  private calculatePagesMasterRowsOnly(): void {
    ...
    if (masterPageEndIndex === masterLastRowIndex) {
      ...
      const firstIndexNotToShow = this.rowModel.getTopLevelRowDisplayedIndex(
        masterPageEndIndex + 1,
      );
