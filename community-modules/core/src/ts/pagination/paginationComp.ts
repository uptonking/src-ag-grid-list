...
export class PaginationComp extends Component {
  ...
  private postConstruct(): void {
    ...
    this.btFirst.insertAdjacentElement(
      'afterbegin',
      _.createIconNoSpan(isRtl ? 'last' : 'first', this.gridOptionsWrapper),
    );
    this.btPrevious.insertAdjacentElement(
      'afterbegin',
      _.createIconNoSpan(isRtl ? 'next' : 'previous', this.gridOptionsWrapper),
    );
    this.btNext.insertAdjacentElement(
      'afterbegin',
      _.createIconNoSpan(isRtl ? 'previous' : 'next', this.gridOptionsWrapper),
    );
    this.btLast.insertAdjacentElement(
      'afterbegin',
      _.createIconNoSpan(isRtl ? 'first' : 'last', this.gridOptionsWrapper),
    );
    ...
    const paginationPanelEnabled =
      isPaging && !this.gridOptionsWrapper.isSuppressPaginationPanel();
  ...
  private enableOrDisableButtons() {
    ...
    this.lastButtonDisabled =
      !maxRowFound || zeroPagesToDisplay || currentPage === totalPages - 1;
  ...
  private setTotalLabels() {
    ...
    if (lastPageFound) {
      ...
      const moreText = this.gridOptionsWrapper.getLocaleTextFunc()(
        'more',
        'more',
      );
