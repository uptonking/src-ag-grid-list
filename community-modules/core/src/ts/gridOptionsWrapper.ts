...
export class GridOptionsWrapper {
  ...
  public static PROP_GROUP_REMOVE_LOWEST_SINGLE_CHILDREN =
  ...
  public static PROP_SUPPRESS_MOVE_WHEN_ROW_DRAG =
  ...
  private readonly autoHeightCalculator: AutoHeightCalculator;
  ...
  private agWire(
    ...
    @Qualifier('columnApi') columnApi: ColumnApi,
  ): void {
  ...
  public init(): void {
    ...
    if (this.isRowModelServerSide()) {
      const msg = (prop: string) =>
        `ag-Grid: '${prop}' is not supported on the Server-Side Row Model`;
    ...
    const warnOfDeprecaredIcon = (name: string) => {
      if (this.gridOptions.icons && this.gridOptions.icons[name]) {
        console.warn(
          `gridOptions.icons.${name} is no longer supported. For information on how to style checkboxes and radio buttons, see https://www.ag-grid.com/javascript-grid-icons/`,
        );
      }
    };
  ...
  private checkProperties(
    ...
    _.iterateObject<any>(invalidProperties, (key, value) => {
      console.warn(
        `ag-grid: invalid ${containerName} property '${key}' did you mean any of these: ${value
          .slice(0, 8)
          .join(', ')}`,
      );
    });
    ...
    if (Object.keys(invalidProperties).length > 0) {
      console.warn(
        `ag-grid: to see all the valid ${containerName} properties please check: ${docsUrl}`,
      );
    }
  }
  ...
  public isRowModelDefault() {
    return (
      ...
      this.gridOptions.rowModelType ===
        Constants.DEPRECATED_ROW_MODEL_TYPE_NORMAL
    );
  }
  ...
  public isShowToolPanel() {
    return isTrue(
      this.gridOptions.sideBar && Array.isArray(this.getSideBar().toolPanels),
    );
  }
  ...
  public isGroupMultiAutoColumn() {
    return (
      isTrue(this.gridOptions.groupMultiAutoColumn) ||
      isTrue(this.gridOptions.groupHideOpenParents)
    );
  }
  ...
  public getPostProcessPopupFunc():
    | ((params: PostProcessPopupParams) => void)
    | undefined {
  ...
  public isEmbedFullWidthRows() {
    return (
      isTrue(this.gridOptions.embedFullWidthRows) ||
      isTrue(this.gridOptions.deprecatedEmbedFullWidthRows)
    );
  }
  ...
  public isEnableCharts() {
    if (isTrue(this.gridOptions.enableCharts)) {
      return ModuleRegistry.assertRegistered(
  ...
  public getProcessDataFromClipboardFunc():
    | ((params: ProcessDataFromClipboardParams) => string[][])
    | undefined {
    return this.gridOptions.processDataFromClipboard;
  }

  public getAsyncTransactionWaitMillis(): number | undefined {
    return _.exists(this.gridOptions.asyncTransactionWaitMillis)
  ...
  public isEnableRangeSelection(): boolean {
    return (
      ...
      isTrue(this.gridOptions.enableRangeSelection)
    );
  }
  ...
  public getNodeChildDetailsFunc():
  ...
  public getProcessHeaderForClipboardFunc():
    | ((params: ProcessHeaderForExportParams) => any)
    | undefined {
  ...
  public getViewportRowModelPageSize(): number {
    return oneOrGreater(
      this.gridOptions.viewportRowModelPageSize,
      DEFAULT_VIEWPORT_ROW_MODEL_PAGE_SIZE,
    );
  }

  public getViewportRowModelBufferSize(): number {
    return zeroOrGreater(
      this.gridOptions.viewportRowModelBufferSize,
      DEFAULT_VIEWPORT_ROW_MODEL_BUFFER_SIZE,
    );
  }
  ...
  public getProcessChartOptionsFunc(): (
  ...
  public getClipboardDeliminator() {
    return _.exists(this.gridOptions.clipboardDeliminator)
  ...
  public static checkEventDeprecation(eventName: string): void {
    if (eventName === 'floatingRowDataChanged') {
      console.warn(
        'ag-Grid: floatingRowDataChanged is now called pinnedRowDataChanged',
      );
    }
  }
  ...
  public getAutoSizePadding(): number {
    return this.gridOptions.autoSizePadding &&
      this.gridOptions.autoSizePadding > 0
      ? this.gridOptions.autoSizePadding
      : 20;
  }
  ...
  public getDocument(): Document {
    ...
    if (
      ...
      _.exists(this.gridOptions.getDocument)
    ) {
  ...
  public getMaxColWidth() {
    if (
      ...
      this.gridOptions.maxColWidth > GridOptionsWrapper.MIN_COL_WIDTH
    ) {
  ...
  public getColWidth() {
    if (
      ...
      this.gridOptions.colWidth < GridOptionsWrapper.MIN_COL_WIDTH
    ) {
      return 200;
    }
  ...
  public getScrollbarWidth() {
    if (this.scrollWidth == null) {
      ...
      this.scrollWidth = useGridOptions
  ...
  private checkForDeprecated() {
    ...
    if (options.suppressUnSort) {
      console.warn(
        'ag-grid: as of v1.12.4 suppressUnSort is not used. Please use sortingOrder instead.',
      );
    }
    if (options.suppressDescSort) {
      console.warn(
        'ag-grid: as of v1.12.4 suppressDescSort is not used. Please use sortingOrder instead.',
      );
    }
    if (options.groupAggFields) {
      console.warn(
        'ag-grid: as of v3 groupAggFields is not used. Please add appropriate agg fields to your columns.',
      );
    }
    if (options.groupHidePivotColumns) {
      console.warn(
        'ag-grid: as of v3 groupHidePivotColumns is not used as pivot columns are now called rowGroup columns. Please refer to the documentation',
      );
    }
    if (options.groupKeys) {
      console.warn(
        'ag-grid: as of v3 groupKeys is not used. You need to set rowGroupIndex on the columns to group. Please refer to the documentation',
      );
    }
    if (typeof options.groupDefaultExpanded === 'boolean') {
      console.warn(
        'ag-grid: groupDefaultExpanded can no longer be boolean. for groupDefaultExpanded=true, use groupDefaultExpanded=9999 instead, to expand all the groups',
      );
    }
    if (options.onRowDeselected || options.rowDeselected) {
      console.warn(
        'ag-grid: since version 3.4 event rowDeselected no longer exists, please check the docs',
      );
    }
    if (options.rowsAlreadyGrouped) {
      console.warn(
        'ag-grid: since version 3.4 rowsAlreadyGrouped no longer exists, please use getNodeChildDetails() instead',
      );
    }
    if (options.groupAggFunction) {
      console.warn(
        'ag-grid: since version 4.3.x groupAggFunction is now called groupRowAggNodes',
      );
    }
    if (options.checkboxSelection) {
      console.warn(
        'ag-grid: since version 8.0.x checkboxSelection is not supported as a grid option. ' +
          'If you want this on all columns, use defaultColDef instead and set it there',
      );
    }
    if (options.paginationInitialRowCount) {
      console.warn(
        'ag-grid: since version 9.0.x paginationInitialRowCount is now called infiniteInitialRowCount',
      );
    }
    if (options.infinitePageSize) {
      console.warn(
        'ag-grid: since version 9.0.x infinitePageSize is now called cacheBlockSize',
      );
    }
    if (options.infiniteBlockSize) {
      console.warn(
        'ag-grid: since version 10.0.x infiniteBlockSize is now called cacheBlockSize',
      );
    }
    if (options.maxPagesInCache) {
      console.warn(
        'ag-grid: since version 10.0.x maxPagesInCache is now called maxBlocksInCache',
      );
    }
    if (options.paginationOverflowSize) {
      console.warn(
        'ag-grid: since version 10.0.x paginationOverflowSize is now called cacheOverflowSize',
      );
    }
    // if (options.forPrint) {
    ...
    if (options.suppressMenuFilterPanel) {
      console.warn(
        `ag-grid: since version 11.0.x, use property colDef.menuTabs=['generalMenuTab','columnsMenuTab'] instead of suppressMenuFilterPanel=true`,
      );
    }
    if (options.suppressMenuMainPanel) {
      console.warn(
        `ag-grid: since version 11.0.x, use property colDef.menuTabs=['filterMenuTab','columnsMenuTab'] instead of suppressMenuMainPanel=true`,
      );
    }
    if (options.suppressMenuColumnPanel) {
      console.warn(
        `ag-grid: since version 11.0.x, use property colDef.menuTabs=['generalMenuTab','filterMenuTab'] instead of suppressMenuColumnPanel=true`,
      );
    }
    if (options.suppressUseColIdForGroups) {
      console.warn(
        `ag-grid: since version 11.0.x, this is not in use anymore. You should be able to remove it from your definition`,
      );
    }
    if (options.groupSuppressRow) {
      console.warn(
        `ag-grid: since version 18.2.x, 'groupSuppressRow' should not be used anymore. Instead remove row groups and perform custom sorting.`,
      );
    }
    ...
    if (options.slaveGrids) {
      console.warn(
        `ag-grid: since version 12.x, slaveGrids has been renamed, this property is now called alignedGrids. Please change your configuration accordingly`,
      );
    }
    if (options.floatingTopRowData) {
      console.warn(
        `ag-grid: since version 12.x, floatingTopRowData is now called pinnedTopRowData`,
      );
    }
    if (options.floatingBottomRowData) {
      console.warn(
        `ag-grid: since version 12.x, floatingBottomRowData is now called pinnedBottomRowData`,
      );
    }
    if (options.paginationStartPage) {
      console.warn(
        `ag-grid: since version 12.x, paginationStartPage is gone, please call api.paginationGoToPage(${options.paginationStartPage}) instead.`,
      );
    }

    if (options.getHeaderCellTemplate) {
      console.warn(
        `ag-grid: since version 15.x, getHeaderCellTemplate is gone, please check the header documentation on how to set header templates.`,
      );
    }
    if (options.headerCellTemplate) {
      console.warn(
        `ag-grid: since version 15.x, headerCellTemplate is gone, please check the header documentation on how to set header templates.`,
      );
    }
    if (options.headerCellRenderer) {
      console.warn(
        `ag-grid: since version 15.x, headerCellRenderer is gone, please check the header documentation on how to set header templates.`,
      );
    }
    if (options.angularCompileHeaders) {
      console.warn(
        `ag-grid: since version 15.x, angularCompileHeaders is gone, please see the getting started for Angular 1 docs to see how to do headers in Angular 1.x.`,
      );
    }
    if (options.pivotTotals) {
      console.warn(
        `ag-grid: since version 18.x, pivotTotals has been removed, instead if using pivotTotals, set pivotColumnGroupTotals='before'|'after'.`,
      );
      options.pivotColumnGroupTotals = 'before';
    }
    if (options.rowModelType === 'inMemory') {
      console.warn(
        `ag-grid: since version 18.x, The In Memory Row Model has been renamed to the Client Side Row Model, set rowModelType='clientSide' instead.`,
      );
      options.rowModelType = 'clientSide';
    }
    if (options.rowModelType === 'enterprise') {
      console.warn(
        `ag-grid: since version 18.x, The Enterprise Row Model has been renamed to the Server Side Row Model, set rowModelType='serverSide' instead.`,
      );
      options.rowModelType = 'serverSide';
    }
    if (options.layoutInterval) {
      console.warn(
        `ag-grid: since version 18.x, layoutInterval is no longer a property. This is because the grid now uses CSS Flex for layout.`,
      );
    }
    if (options.gridAutoHeight) {
      console.warn(
        `ag-grid: since version 19.x, gridAutoHeight is gone, please use domLayout=autoHeight instead`,
      );
      options.domLayout = 'autoHeight';
    }
    if (options.showToolPanel === true) {
      console.warn(
        `ag-grid: since version 19.x, showToolPanel is gone, please specify toolPanel components. See https://www.ag-grid.com/javascript-grid-tool-panel/`,
      );
    ...
    Object.keys(oldToolPanelProperties).forEach((key) => {
      ...
      if (value !== undefined) {
        if (key === 'toolPanelSuppressSideButtons') {
          console.warn(
            'ag-grid: since v19.0 toolPanelSuppressSideButtons has been completely removed. See https://www.ag-grid.com/javascript-grid-tool-panel/',
          );
          return;
        }

        console.warn(
          `ag-grid: since v19.0 gridOptions.${key} is deprecated, please use gridOptions.sideBar.toolPanel[columnsIndex].componentParams.${translation}`,
        );
    ...
    if (
      Object.keys(toolPanelColumnsCompProps).length > 0 &&
      !_.exists(options.sideBar)
    ) {
      console.warn(
        `ag-grid: since version 19.x, sideBar is mandatory if using toolPanel related properties. See https://www.ag-grid.com/javascript-grid-tool-panel/`,
      );
    ...
    if (
      ...
      const columnsDef: ToolPanelDef[] = sideBarDef.toolPanels.filter(
        (it: ToolPanelDef) => it.id === 'columns',
      ) as ToolPanelDef[];
    ...
    if (options.enableStatusBar) {
      console.warn(
        `ag-grid: since version 19.x, enableStatusBar is gone, please specify statusBar components`,
      );
    ...
    const checkRenamedProperty = (
      ...
      if (options[oldProp] != null) {
        console.warn(
          `ag-grid: since version ${version}, '${oldProp}' is deprecated / renamed, please use the new property name '${newProp}' instead.`,
        );
