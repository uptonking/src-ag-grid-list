...
> extends BeanStub {
  ...
  protected onPageLoaded(event: any): void {
    ...
    this.logger.log(
      `onPageLoaded: page = ${event.page.getBlockNumber()}, lastRow = ${
        event.lastRow
      }`,
    );
  ...
  private purgeBlocksIfNeeded(blockToExclude: T): void {
    ...
    blocksForPurging.sort(
      (a: T, b: T) => b.getLastAccessed() - a.getLastAccessed(),
    );
    ...
    const blocksToKeep = maxBlocksProvided
      ? this.cacheParams.maxBlocksInCache - 1
      : null;
    ...
    blocksForPurging.forEach((block: T, index: number) => {
      const purgeBecauseBlockEmpty =
        block.getState() === RowNodeBlock.STATE_DIRTY &&
        index >= emptyBlocksToKeep;

      const purgeBecauseCacheFull = maxBlocksProvided
        ? index >= blocksToKeep
        : false;
      ...
      if (purgeBecauseBlockEmpty || purgeBecauseCacheFull) {
        ...
        if (block.isAnyNodeOpen(this.virtualRowCount)) {
          return;
        }
  ...
  protected postCreateBlock(newBlock: T): void {
    newBlock.addEventListener(
      RowNodeBlock.EVENT_LOAD_COMPLETE,
  ...
  protected checkVirtualRowCount(block: T, lastRow: any): void {
    ...
    if (typeof lastRow === 'number' && lastRow >= 0) {
      ...
      const lastRowIndex =
        (block.getBlockNumber() + 1) * this.cacheParams.blockSize;
      const lastRowIndexPlusOverflow =
        lastRowIndex + this.cacheParams.overflowSize;
  ...
  protected forEachBlockInReverseOrder(
    callback: (block: T, id: number) => void,
  ): void {
  ...
  public getRowNodesInRange(
    ...
    this.forEachBlockInOrder((block: IRowNodeBlock, id: number) => {
      ...
      block.forEachNodeShallow(
        (rowNode) => {
          const hitFirstOrLast =
            rowNode === firstInRange || rowNode === lastInRange;
