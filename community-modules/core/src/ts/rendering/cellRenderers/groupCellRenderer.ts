...
export class GroupCellRenderer extends Component implements ICellRendererComp {
  ...
  public init(params: GroupCellRendererParams): void {
    ...
    if (
      ...
      if (node.footer) {
        ...
        const rowGroupColumnId =
          node.rowGroupColumn && node.rowGroupColumn.getColId();
  ...
  private setPaddingDeprecatedWay(paddingCount: number, padding: number): void {
    ...
    const paddingSide = this.gridOptionsWrapper.isEnableRtl()
      ? 'paddingRight'
      : 'paddingLeft';
  ...
  private setupIndent(): void {
    ...
    if (!suppressPadding) {
      this.addManagedListener(
        node,
        RowNode.EVENT_UI_LEVEL_CHANGED,
  ...
  private createFooterCell(): void {
    ...
    if (footerValueGetter) {
      ...
      if (typeof footerValueGetter === 'function') {
        ...
        footerValue = this.expressionService.evaluate(
          footerValueGetter,
          paramsClone,
        );
      } else {
  ...
  private useInnerRenderer(
    ...
    if (
      ...
      groupInnerRendererClass.component != null &&
      groupInnerRendererClass.source != ComponentSource.DEFAULT
    ) {
      // use the renderer defined in cellRendererParams.innerRenderer
      cellRendererPromise = this.userComponentFactory.newInnerCellRenderer(
        groupCellRendererParams,
        params,
      );
    } else {
      ...
      if (
        ...
        cellRendererPromise = this.userComponentFactory.newCellRenderer(
          groupedColumnDef,
          params,
        );
      } else if (
  ...
  private updateChildCount(): void {
    ...
    this.eChildCount.innerHTML =
      allChildrenCount >= 0 ? `(${allChildrenCount})` : ``;
  }

  private createLeafCell(): void {
    if (_.exists(this.params.value)) {
      this.eValue.innerText = this.params.valueFormatted
  ...
  private addCheckboxIfNeeded(): void {
    ...
    if (checkboxNeeded) {
      ...
      this.addDestroyFunc(() =>
        this.getContext().destroyBean(cbSelectionComponent),
      );
    }
  ...
  private addExpandAndContract(): void {
    ...
    const eExpandedIcon = _.createIconNoSpan(
      'groupExpanded',
      this.gridOptionsWrapper,
      null,
    );
    const eContractedIcon = _.createIconNoSpan(
      'groupContracted',
      this.gridOptionsWrapper,
      null,
    );
    ...
    if (
      ...
      this.isExpandable() &&
      !params.suppressDoubleClickExpand
    ) {
      this.addManagedListener(
        eGroupCell,
        'dblclick',
  ...
  private onKeyDown(event: KeyboardEvent): void {
    ...
    const cellEditable =
      this.params.column && this.params.column.isCellEditable(this.params.node);
  ...
  private setupDragOpenParents(): void {
    ...
    if (!this.gridOptionsWrapper.isGroupHideOpenParents()) {
      ...
      if (rowGroupColumn) {
        ...
        this.draggedFromHideOpenParents = !column.isRowGroupDisplayed(
          rowGroupColumn.getId(),
        );
      } else {
    ...
    if (this.draggedFromHideOpenParents) {
      ...
      while (true) {
        ...
        if (
          ...
          column.isRowGroupDisplayed(pointer.rowGroupColumn.getId())
        ) {
  ...
  private isExpandable(): boolean {
    ...
    const reducedLeafNode =
      this.columnController.isPivotMode() && rowNode.leafGroup;
  ...
  private showExpandAndContractIcons(): void {
    ...
    if (this.isExpandable()) {
      ...
      const expanded = this.draggedFromHideOpenParents
        ? true
        : rowNode.expanded;
    ...
    const pivotModeAndLeafGroup =
      this.columnController.isPivotMode() && displayedGroup.leafGroup;
    const notExpandable = !displayedGroup.isExpandable();
    const addLeafIndentClass =
      displayedGroup.footer || notExpandable || pivotModeAndLeafGroup;
