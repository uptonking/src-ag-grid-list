...
export class RowComp extends Component {
    ...
    if (this.fadeRowIn) {
      this.createSecondPassFuncs.push(() => {
        this.eAllRowContainers.forEach((eRow) =>
          _.removeCssClass(eRow, 'ag-opacity-zero'),
        );
      });
    }
  }
  ...
  private createTemplate(
    ...
    templateParts.push(
      ` row-index="${rowIdx}" aria-rowindex="${
        headerRowCount + this.rowNode.rowIndex + 1
      }"`,
    );
    templateParts.push(rowIdSanitised ? ` row-id="${rowIdSanitised}"` : ``);
    templateParts.push(
      businessKey ? ` row-business-key="${businessKeySanitised}"` : ``,
    );
    templateParts.push(` comp-id="${this.getCompId()}"`);
    templateParts.push(` class="${rowClasses}"`);
    templateParts.push(
      ` style="height: ${rowHeight}px; ${rowTopStyle} ${userRowStyles}">`,
    );
  ...
  private getInitialRowTopStyle() {
    ...
    return isSuppressRowTransform
      ...
      : `transform: translateY(${afterScalingPixels}px);`;
  }
  ...
  private createRowContainer(
    ...
    const cellTemplatesAndComps: CellTemplate = useAnimationsFrameForCreate
      ? { cellComps: [], template: '' }
    ...
    rowContainerComp.appendRowTemplate(rowTemplate, () => {
      const eRow: HTMLElement = rowContainerComp.getRowElement(
        this.getCompId(),
      );
  ...
  private setupRowContainers(): void {
    ...
    const isFullWidthGroup =
      isGroupRow &&
      this.beans.gridOptionsWrapper.isGroupUseEntireRow(pivotMode);

    if (this.rowNode.stub) {
      this.createFullWidthRows(
        RowComp.LOADING_CELL_RENDERER,
        RowComp.LOADING_CELL_RENDERER_COMP_NAME,
      );
    } else if (isDetailCell) {
      this.createFullWidthRows(
        RowComp.DETAIL_CELL_RENDERER,
        RowComp.DETAIL_CELL_RENDERER_COMP_NAME,
      );
    } else if (isFullWidthCell) {
      this.createFullWidthRows(RowComp.FULL_WIDTH_CELL_RENDERER, null);
    } else if (isFullWidthGroup) {
      this.createFullWidthRows(
        RowComp.GROUP_ROW_RENDERER,
        RowComp.GROUP_ROW_RENDERER_COMP_NAME,
      );
    } else {
  ...
  private setupNormalRowContainers(): void {
    ...
    if (this.printLayout) {
      ...
      rightCols = this.beans.columnController.getDisplayedRightColumnsForRow(
        this.rowNode,
      );
    }
    ...
    this.createRowContainer(
      ...
      centerCols,
      (eRow) => (this.eBodyRow = eRow),
    );
    this.createRowContainer(
      this.pinnedRightContainerComp,
      rightCols,
      (eRow) => (this.ePinnedRightRow = eRow),
    );
    this.createRowContainer(
      this.pinnedLeftContainerComp,
      leftCols,
      (eRow) => (this.ePinnedLeftRow = eRow),
    );
  }
  ...
  public refreshFullWidth(): boolean {
    ...
    const normalSuccess = tryRefresh(
      this.eFullWidthRow,
      this.fullWidthRowComponent,
      null,
    );
    const bodySuccess = tryRefresh(
      this.eFullWidthRowBody,
      this.fullWidthRowComponentBody,
      null,
    );
    const leftSuccess = tryRefresh(
      this.eFullWidthRowLeft,
      this.fullWidthRowComponentLeft,
      Constants.PINNED_LEFT,
    );
    const rightSuccess = tryRefresh(
      this.eFullWidthRowRight,
      this.fullWidthRowComponentRight,
      Constants.PINNED_RIGHT,
    );

    const allFullWidthRowsRefreshed =
      normalSuccess && bodySuccess && leftSuccess && rightSuccess;
  ...
  private addListeners(): void {
    ...
    this.addManagedListener(
      this.rowNode,
      RowNode.EVENT_ROW_INDEX_CHANGED,
      this.onRowIndexChanged.bind(this),
    );
    this.addManagedListener(
      this.rowNode,
      RowNode.EVENT_TOP_CHANGED,
    ...
    this.addManagedListener(
      this.rowNode,
      RowNode.EVENT_DRAGGING_CHANGED,
  ...
  private postProcessRowDragging(): void {
    ...
    this.eAllRowContainers.forEach((row) =>
      _.addOrRemoveCssClass(row, 'ag-row-dragging', dragging),
    );
  }
  ...
  private onExpandedChanged(): void {
    ...
    this.eAllRowContainers.forEach((row) =>
      _.addOrRemoveCssClass(row, 'ag-row-group-expanded', rowNode.expanded),
    );
    this.eAllRowContainers.forEach((row) =>
      _.addOrRemoveCssClass(row, 'ag-row-group-contracted', !rowNode.expanded),
    );
  }
  ...
  private refreshCellsInAnimationFrame() {
    ...
    const eligibleToBeRemoved = colIdsToRemove.filter(
      this.isCellEligibleToBeRemoved.bind(this),
    );
  ...
  private isCellEligibleToBeRemoved(indexStr: string): boolean {
    ...
    const focused = this.beans.focusController.isCellFocused(
      renderedCell.getCellPosition(),
    );
  ...
  private createNewCell(
    ...
    cellTemplates: string[],
    newCellComps: CellComp[],
  ): void {
    const newCellComp = new CellComp(
      this.scope,
      this.beans,
      col,
      this.rowNode,
  ...
  private onRowMouseDown(mouseEvent: MouseEvent) {
    this.lastMouseDownOnDragger = _.isElementChildOfClass(
  ...
  public onRowClick(mouseEvent: MouseEvent) {
    const stop =
      _.isStopPropagationForAgGrid(mouseEvent) || this.lastMouseDownOnDragger;

    if (stop) {
      return;
    }

    const agEvent: RowClickedEvent = this.createRowEventWithSource(
      Events.EVENT_ROW_CLICKED,
      mouseEvent,
    );
    ...
    if (this.rowNode.isSelected()) {
      ...
      const clearSelection = multiSelectOnClick
        ? false
  ...
  private createFullWidthRowContainer(
    ...
    rowContainerComp.appendRowTemplate(rowTemplate, () => {
      ...
      const cachedRowComp = this.beans.detailRowCompCache.get(
        this.rowNode,
        pinned,
      );
      if (cachedRowComp) {
        ...
        if (!res) {
          const masterDetailModuleLoaded = ModuleRegistry.isRegistered(
            ModuleNames.MasterDetailModule,
          );
          if (
            cellRendererName === 'agDetailCellRenderer' &&
            !masterDetailModuleLoaded
          ) {
            console.warn(
            ...
            console.error(
              `ag-Grid: fullWidthCellRenderer ${cellRendererName} not found`,
            );
          }
          return;
        }
  ...
  private getInitialRowClasses(extraCssClass: string): string[] {
    ...
    if (rowNode.group) {
      ...
      classes.push(
        'ag-row-level-' + (rowNode.parent ? rowNode.parent.level + 1 : '0'),
      );
    }
    ...
    if (addExpandedClass) {
      classes.push(
        rowNode.expanded ? 'ag-row-group-expanded' : 'ag-row-group-contracted',
      );
    }
  ...
  private isFirstRowOnPage(): boolean {
    return (
      this.rowNode.rowIndex === this.beans.paginationProxy.getPageFirstRow()
    );
  }

  private isLastRowOnPage(): boolean {
    return (
      this.rowNode.rowIndex === this.beans.paginationProxy.getPageLastRow()
    );
  }
  ...
  private onModelUpdated(): void {
    ...
    if (this.firstRowOnPage !== newFirst) {
      ...
      this.eAllRowContainers.forEach((row) =>
        _.addOrRemoveCssClass(row, 'ag-row-first', newFirst),
      );
    }
    if (this.lastRowOnPage !== newLast) {
      this.lastRowOnPage = newLast;
      this.eAllRowContainers.forEach((row) =>
        _.addOrRemoveCssClass(row, 'ag-row-last', newLast),
      );
    }
  }
  ...
  public stopEditing(cancel = false): void {
    ...
    if (!cancel) {
      const event: RowValueChangedEvent = this.createRowEvent(
        Events.EVENT_ROW_VALUE_CHANGED,
      );
  ...
  private setEditingRow(value: boolean): void {
    ...
    this.eAllRowContainers.forEach((row) =>
      _.addOrRemoveCssClass(row, 'ag-row-editing', value),
    );

    const event: RowEvent = value
      ? (this.createRowEvent(
          Events.EVENT_ROW_EDITING_STARTED,
        ) as RowEditingStartedEvent)
      : (this.createRowEvent(
          Events.EVENT_ROW_EDITING_STOPPED,
        ) as RowEditingStoppedEvent);
  ...
  public startRowEditing(
    ...
    sourceRenderedCell: CellComp | null = null,
  ): void {
  ...
  private postProcessRowClassRules(): void {
    this.processRowClassRules(
      ...
      (className: string) => {
        this.eAllRowContainers.forEach((row) =>
          _.removeCssClass(row, className),
        );
      },
    );
  }
  ...
  private addHoverFunctionality(eRow: HTMLElement): void {
    ...
    this.addManagedListener(eRow, 'mouseenter', () =>
      this.rowNode.onMouseEnter(),
    );
    this.addManagedListener(eRow, 'mouseleave', () =>
      this.rowNode.onMouseLeave(),
    );
  ...
  public destroy(animate = false): void {
    ...
    const event: VirtualRowRemovedEvent = this.createRowEvent(
      Events.EVENT_VIRTUAL_ROW_REMOVED,
    );
  ...
  private onCellFocusChanged(): void {
    const rowFocused = this.beans.focusController.isRowFocused(
      this.rowNode.rowIndex,
      this.rowNode.rowPinned,
    );
    ...
    if (rowFocused !== this.rowFocused) {
      this.eAllRowContainers.forEach((row) =>
        _.addOrRemoveCssClass(row, 'ag-row-focus', rowFocused),
      );
      this.eAllRowContainers.forEach((row) =>
        _.addOrRemoveCssClass(row, 'ag-row-no-focus', !rowFocused),
      );
  ...
  private setRowTop(pixels: number): void {
    ...
    if (_.exists(pixels)) {
      ...
      if (this.beans.gridOptionsWrapper.isSuppressRowTransform()) {
        ...
        this.eAllRowContainers.forEach(
          (row) => (row.style.transform = `translateY(${topPx})`),
        );
      }
    }
  }
  ...
  private updateRowIndexes(): void {
    ...
    this.eAllRowContainers.forEach((eRow) => {
      ...
      eRow.setAttribute(
        'aria-rowindex',
        (headerRowCount + this.rowNode.rowIndex + 1).toString(),
      );
