...
export class CellComp extends Component implements TooltipParentComp {
  ...
  constructor(
    ...
    this.rangeSelectionEnabled =
      this.beans.rangeController &&
      beans.gridOptionsWrapper.isEnableRangeSelection();
  ...
  public getCreateTemplate(): string {
    ...
    const valueSanitised = _.get(this.column, 'colDef.template', null)
      ? valueToRender
      : _.escape(valueToRender);
    ...
    if (
      ...
      _.exists(tooltipSanitised)
    ) {
      templateParts.push(`title="${tooltipSanitised}"`);
    }

    templateParts.push(
      ` style="width: ${Number(width)}px; left: ${Number(left)}px; ${_.escape(
        stylesFromColDef,
      )} ${_.escape(stylesForRowSpanning)}" >`,
    );
  ...
  public afterAttached(): void {
    ...
    if (
      _.exists(this.tooltip) &&
      !this.beans.gridOptionsWrapper.isEnableBrowserTooltips()
    ) {
      this.createManagedBean(
        new TooltipFeature(this, 'cell'),
        this.beans.context,
      );
    }
  }
  ...
  private getCellWidth(): number {
    ...
    return this.colsSpanning.reduce(
      (width, col) => width + col.getActualWidth(),
      0,
    );
  }
  ...
  public getInitialValueToRender(): string {
    ...
    if (colDef.templateUrl) {
      ...
      const template = this.beans.templateService.getTemplate(
        colDef.templateUrl,
  ...
  public refreshCell(params?: {
    ...
    if (dataNeedsUpdating) {
      ...
      const cellRendererRefreshed = newData
        ? false
        : this.attemptCellRendererRefresh();
  ...
  private angular1Compile(): void {
    ...
    if (this.beans.gridOptionsWrapper.isAngularCompileRows()) {
      ...
      if (
        !eGui.classList.contains('ng-scope') ||
        eGui.childElementCount === 0
      ) {
  ...
  private postProcessClassesFromColDef() {
    this.processClassesFromColDef((className) =>
  ...
  public attemptCellRendererRefresh(): boolean {
    if (
      _.missing(this.cellRenderer) ||
      !this.cellRenderer ||
      _.missing(this.cellRenderer.refresh)
    ) {
      return false;
    }
    ...
    const finalParams = this.beans.userComponentFactory.createFinalParams(
  ...
  public setUsingWrapper(): void {
    ...
    this.includeSelectionComponent =
      cbSelectionIsFunc || colDef.checkboxSelection === true;
    this.includeRowDraggingComponent =
      rowDraggableIsFunc || colDef.rowDrag === true;
  ...
  private afterCellRendererCreated(
    ...
    const cellRendererNotRequired =
      !this.isAlive() || cellRendererVersion !== this.cellRendererVersion;
  ...
  private formatValue(value: any): any {
    const valueFormatted = this.beans.valueFormatterService.formatValue(
      this.column,
      this.rowNode,
      this.scope,
      value,
  ...
  private getValue(): any {
    ...
    const ignoreAggData =
      isOpenGroup && groupFootersEnabled && !groupAlwaysShowAggData;
  ...
  public dispatchCellContextMenuEvent(event: Event) {
    const colDef = this.getComponentHolder();
    const cellContextMenuEvent: CellContextMenuEvent = this.createEvent(
      event,
      Events.EVENT_CELL_CONTEXT_MENU,
    );
  ...
  private onMouseOut(mouseEvent: MouseEvent): void {
    const cellMouseOutEvent: CellMouseOutEvent = this.createEvent(
      mouseEvent,
      Events.EVENT_CELL_MOUSE_OUT,
    );
  ...
  private onMouseOver(mouseEvent: MouseEvent): void {
    const cellMouseOverEvent: CellMouseOverEvent = this.createEvent(
      mouseEvent,
  ...
  private onCellDoubleClicked(mouseEvent: MouseEvent) {
    ...
    const cellDoubleClickedEvent: CellDoubleClickedEvent = this.createEvent(
      mouseEvent,
  ...
  public startEditingIfEnabled(
    keyPress: number | null = null,
    ...
    const callback = this.afterCellEditorCreated.bind(
      this,
      this.cellEditorVersion,
    );

    const params = this.createCellEditorParams(
      keyPress,
      charPress,
      cellStartedEdit,
    );
  ...
  private createCellEditor(
    ...
    const cellEditorPromise = this.beans.userComponentFactory.newCellEditor(
  ...
  private afterCellEditorCreated(
    ...
    if (!cellEditor.getGui) {
      console.warn(
        `ag-Grid: cellEditor for column ${this.column.getId()} is missing getGui() method`,
      );
    ...
    this.cellEditorInPopup =
      cellEditor.isPopup !== undefined && cellEditor.isPopup();
    ...
    const event: CellEditingStartedEvent = this.createEvent(
      null,
  ...
  private setInlineEditingClass(): void {
    ...
    _.addOrRemoveCssClass(
      this.getGui().parentNode as HTMLElement,
  ...
  private parseValue(newValue: any): any {
    ...
    return _.exists(valueParser)
      ? this.beans.expressionService.evaluate(valueParser, params)
      : newValue;
  }
  ...
  private onMouseDown(mouseEvent: MouseEvent): void {
    ...
    if (
      !shiftKey ||
      (rangeController && !rangeController.getCellRanges().length)
    ) {
      ...
      const forceBrowserFocus =
        (_.isBrowserIE() || _.isBrowserEdge()) && !this.editingCell;
    ...
    eventService.dispatchEvent(
      this.createEvent(mouseEvent, Events.EVENT_CELL_MOUSE_DOWN),
    );
  }
  ...
  private isRightClickInExistingRange(mouseEvent: MouseEvent): boolean {
    ...
    if (rangeController) {
      const cellInRange = rangeController.isCellInAnyRange(
  ...
  private onCellClicked(mouseEvent: MouseEvent): void {
    ...
    const cellClickedEvent: CellClickedEvent = this.createEvent(
      mouseEvent,
      Events.EVENT_CELL_CLICKED,
    );
    ...
    const editOnSingleClick =
      (gridOptionsWrapper.isSingleClickEdit() || colDef.singleClickEdit) &&
  ...
  private modifyLeftForPrintLayout(leftPosition: number): number {
    if (
      !this.printLayout ||
      this.column.getPinned() === Constants.PINNED_LEFT
    ) {
      return leftPosition;
    }
  ...
  private getRangeBorders(): {
    ...
    const ranges = rangeController
      .getCellRanges()
      .filter((range) =>
        rangeController.isCellInSpecificRange(this.cellPosition, range),
      );
    ...
    for (let i = 0; i < ranges.length; i++) {
      ...
      if (
        !top &&
        this.beans.rowPositionUtils.sameRow(startRow, this.cellPosition)
      ) {
        top = true;
      }
      ...
      if (
        !bottom &&
        this.beans.rowPositionUtils.sameRow(endRow, this.cellPosition)
      ) {
        bottom = true;
      }
  ...
  private getHasChartRange(): boolean {
    ...
    return (
      cellRanges.length > 0 &&
      cellRanges.every((range) =>
        _.includes([CellRangeType.DIMENSION, CellRangeType.VALUE], range.type),
      )
    );
  }
  ...
  private addSelectionHandle() {
    ...
    const selectionHandleFill =
      gridOptionsWrapper.isEnableFillHandle() && _.missing(cellRangeType);
    const type = selectionHandleFill
      ? SelectionHandleType.FILL
      : SelectionHandleType.RANGE;
  ...
  private addRowDragging(): void {
    ...
    const rowDraggingComp = new RowDragComp(
      this.rowNode,
      this.column,
      this.getValueToUse(),
      this.beans,
    ...
    this.eCellWrapper.insertBefore(
      rowDraggingComp.getGui(),
      this.eParentOfValue,
    );
  }

  private addDndSource(): void {
    const dndSourceComp = new DndSourceComp(
      this.rowNode,
      this.column,
      this.getValueToUse(),
      this.beans,
  ...
  private addSelectionCheckbox(): void {
    ...
    this.addDestroyFunc(() =>
      this.beans.context.destroyBean(cbSelectionComponent),
    );

    // put the checkbox in before the value
    this.eCellWrapper.insertBefore(
      cbSelectionComponent.getGui(),
      this.eParentOfValue,
    );
  }
  ...
  public onCellFocused(event?: any): void {
    const cellFocused = this.beans.focusController.isCellFocused(
      this.cellPosition,
    );
  ...
  public stopEditing(cancel = false): void {
    ...
    if (!cancel) {
      ...
      const userWantsToCancel =
        this.cellEditor.isCancelAfterEnd && this.cellEditor.isCancelAfterEnd();
    ...
    const event: CellEditingStoppedEvent = this.createEvent(
      null,
      Events.EVENT_CELL_EDITING_STOPPED,
    );
    this.beans.eventService.dispatchEvent(event);
  }
}
