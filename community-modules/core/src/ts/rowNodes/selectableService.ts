...
export class SelectableService extends BeanStub {
  ...
  public updateSelectableAfterFiltering(rowNode: RowNode): void {
    if (this.isRowSelectableFunc) {
      const nextChildrenFunc = (rowNode: RowNode) =>
  ...
  private recurseDown(
    children: RowNode[],
    nextChildrenFunc: (rowNode: RowNode) => RowNode[],
  ): void {
    children.forEach((child: RowNode) => {
      ...
      if (this.groupSelectsChildren) {
        ...
        const firstSelectable = _.find(
          nextChildrenFunc(child),
          'selectable',
          true,
        );
        ...
        rowSelectable = this.isRowSelectableFunc
          ? this.isRowSelectableFunc(child)
          : false;
      }
