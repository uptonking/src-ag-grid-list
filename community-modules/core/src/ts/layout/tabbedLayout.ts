...
export class TabbedLayout extends ManagedFocusComponent {
  ...
  protected onTabKeyDown(e: KeyboardEvent) {
    ...
    if (focusInHeader) {
      ...
      if (isFocusManaged) {
        ...
        const nextEl = focusController.findNextFocusableElement(
          eBody,
          false,
          e.shiftKey,
        );
  ...
  public getMinDimensions(): { width: number; height: number } {
    ...
    const eDummyBody = eDummyContainer.querySelector(
      '[ref="eBody"]',
    ) as HTMLElement;
    ...
    this.items.forEach((itemWrapper: TabbedItemWrapper) => {
      ...
      const eClone: HTMLElement = itemWrapper.tabbedItem.bodyPromise.resolveNow(
        null,
  ...
  private addItem(item: TabbedItem): void {
    ...
    eHeaderButton.addEventListener(
      'click',
      this.showItemWrapper.bind(this, wrapper),
    );
  }
  ...
  private showItemWrapper(wrapper: TabbedItemWrapper): void {
    ...
    wrapper.tabbedItem.bodyPromise.then((body) => {
      ...
      this.focusController.focusFirstFocusableElement(
        this.eBody,
        onlyUnmanaged,
      );
    });
