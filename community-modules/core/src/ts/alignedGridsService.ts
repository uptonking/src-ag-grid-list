...
export class AlignedGridsService extends BeanStub {
  ...
  private fireEvent(
    callback: (alignedGridService: AlignedGridsService) => void,
  ): void {
  ...
  private processGroupOpenedEvent(
    groupOpenedEvent: ColumnGroupOpenedEvent,
  ): void {
  ...
  private processColumnEvent(colEvent: ColumnEvent): void {
    ...
    if (masterColumn) {
      otherColumn = this.columnController.getPrimaryColumn(
        masterColumn.getColId(),
      );
    }
    ...
    switch (colEvent.type) {
      case Events.EVENT_COLUMN_MOVED:
        ...
        this.logger.log(
          `onColumnEvent-> processing ${colEvent.type} toIndex = ${movedEvent.toIndex}`,
        );
      ...
      case Events.EVENT_COLUMN_VISIBLE:
        ...
        this.logger.log(
          `onColumnEvent-> processing ${colEvent.type} visible = ${visibleEvent.visible}`,
        );
      ...
      case Events.EVENT_COLUMN_PINNED:
        ...
        this.logger.log(
          `onColumnEvent-> processing ${colEvent.type} pinned = ${pinnedEvent.pinned}`,
        );
        this.columnController.setColumnsPinned(
          columnIds,
          pinnedEvent.pinned,
          'alignedGridChanged',
        );
      ...
      case Events.EVENT_COLUMN_RESIZED:
        ...
        masterColumns.forEach((column: Column) => {
          ...
          const columnWidths = [
            { key: column.getColId(), newWidth: column.getActualWidth() },
          ];
