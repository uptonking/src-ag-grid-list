...
export class DragAndDropService extends BeanStub {
  ...
  public static GHOST_TEMPLATE = `<div class="ag-dnd-ghost ag-unselectable">
  ...
  @PostConstruct
  private init(): void {
    this.ePinnedIcon = _.createIcon(
      'columnMovePin',
      this.gridOptionsWrapper,
      null,
    );
    this.eHideIcon = _.createIcon(
      'columnMoveHide',
      this.gridOptionsWrapper,
      null,
    );
    this.eMoveIcon = _.createIcon(
      'columnMoveMove',
      this.gridOptionsWrapper,
      null,
    );
    this.eLeftIcon = _.createIcon(
      'columnMoveLeft',
      this.gridOptionsWrapper,
      null,
    );
    this.eRightIcon = _.createIcon(
      'columnMoveRight',
      this.gridOptionsWrapper,
      null,
    );
    this.eGroupIcon = _.createIcon(
      'columnMoveGroup',
      this.gridOptionsWrapper,
      null,
    );
    this.eAggregateIcon = _.createIcon(
      'columnMoveValue',
      this.gridOptionsWrapper,
      null,
    );
    this.ePivotIcon = _.createIcon(
      'columnMovePivot',
      this.gridOptionsWrapper,
      null,
    );
    this.eDropNotAllowedIcon = _.createIcon(
      'dropNotAllowed',
      this.gridOptionsWrapper,
      null,
    );
  ...
  private onDragStop(mouseEvent: MouseEvent): void {
    ...
    if (this.lastDropTarget && this.lastDropTarget.onDragStop) {
      const draggingEvent = this.createDropTargetEvent(
        this.lastDropTarget,
        mouseEvent,
        null,
        null,
  ...
  private onDragging(mouseEvent: MouseEvent, fromNudge: boolean): void {
    ...
    const dropTarget = _.find(
      this.dropTargets,
      this.isMouseOnDropTarget.bind(this, mouseEvent),
    );

    if (dropTarget !== this.lastDropTarget) {
      this.leaveLastTargetIfExists(
        mouseEvent,
        hDirection,
        vDirection,
        fromNudge,
      );
      this.enterDragTargetIfExists(
        dropTarget,
        mouseEvent,
        hDirection,
        vDirection,
  ...
  private getAllContainersFromDropTarget(
    ...
    const secondaryContainers = dropTarget.getSecondaryContainers
      ? dropTarget.getSecondaryContainers()
      : null;
  ...
  public removeDropTarget(dropTarget: DropTarget) {
    this.dropTargets = this.dropTargets.filter(
      (target) => target.getContainer() !== dropTarget.getContainer(),
    );
  }
  ...
  public findExternalZone(params: RowDropZoneParams): DropTarget {
    const externalTargets = this.dropTargets.filter(
      (target) => target.external,
    );

    return _.find(
      externalTargets,
      (zone) => zone.getContainer() === params.getContainer(),
    );
  }
  ...
  private positionGhost(event: MouseEvent): void {
    ...
    const windowScrollY =
      window.pageYOffset || usrDocument.documentElement.scrollTop;
    const windowScrollX =
      window.pageXOffset || usrDocument.documentElement.scrollLeft;

    // check ghost is not positioned outside of the browser
    if (
      browserWidth > 0 &&
      left + ghost.clientWidth > browserWidth + windowScrollX
    ) {
    ...
    if (
      browserHeight > 0 &&
      top + ghost.clientHeight > browserHeight + windowScrollY
    ) {
  ...
  private createGhost(): void {
    ...
    this.eGhostIcon = this.eGhost.querySelector(
      '.ag-dnd-ghost-icon',
    ) as HTMLElement;

    this.setGhostIcon(null);

    const eText = this.eGhost.querySelector(
      '.ag-dnd-ghost-label',
    ) as HTMLElement;
  ...
  public setGhostIcon(iconName: string, shake = false): void {
    ...
    if (!iconName) {
      iconName =
        this.dragSource.defaultIconName || DragAndDropService.ICON_NOT_ALLOWED;
    }
