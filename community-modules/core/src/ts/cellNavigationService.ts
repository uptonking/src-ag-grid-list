...
export class CellNavigationService extends BeanStub {
  ...
  private getCellToLeft(lastCell: CellPosition | null): CellPosition | null {
    ...
    const colToLeft = this.columnController.getDisplayedColBefore(
      lastCell.column,
    );
    if (!colToLeft) {
      return null;
  ...
  private getCellToRight(lastCell: CellPosition | null): CellPosition | null {
    ...
    const colToRight = this.columnController.getDisplayedColAfter(
      lastCell.column,
    );
      ...
      switch (pinned) {
        ...
        case Constants.PINNED_TOP:
          ...
          if (this.rowModel.isRowsToRender()) {
            ...
            this.pinnedRowModel.isRowsToRender(Constants.PINNED_BOTTOM)
          ) {
            return {
              rowIndex: 0,
  ...
  private isLastRowInContainer(rowPosition: RowPosition): boolean {
    ...
    if (pinned === Constants.PINNED_TOP) {
      ...
      const lastBottomIndex =
        this.pinnedRowModel.getPinnedBottomRowData().length - 1;
  ...
  private getLastFloatingTopRow(): RowPosition {
    const lastFloatingRow =
      this.pinnedRowModel.getPinnedTopRowData().length - 1;
    return {
  ...
  public getNextTabbedCellForwards(
    gridCell: CellPosition,
  ): CellPosition | null {
  ...
  public getNextTabbedCellBackwards(
    gridCell: CellPosition,
  ): CellPosition | null {
    const displayedColumns = this.columnController.getAllDisplayedColumns();
    ...
    // move along to the next cell
    let newColumn = this.columnController.getDisplayedColBefore(
      gridCell.column,
    );
