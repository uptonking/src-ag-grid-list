...
import {
  ColumnController,
  ColumnResizeSet,
} from '../../columnController/columnController';
...
export class HeaderGroupWrapperComp extends AbstractHeaderWrapper {
  private static TEMPLATE /* html */ = `<div class="ag-header-group-cell" role="presentation" tabindex="-1">
  ...
  protected postConstruct(): void {
    ...
    CssClassApplier.addHeaderClassesFromColDef(
    ...
    const displayName = this.columnController.getDisplayNameForColumnGroup(
      this.column,
      'header',
    );
    ...
    this.createManagedBean(
      new SetLeftFeature(this.column, this.getGui(), this.beans),
    );
  }
  ...
  private appendHeaderGroupComp(displayName: string): void {
    const params: IHeaderGroupParams = {
      ...
      setExpanded: (expanded: boolean) => {
        this.columnController.setColumnGroupOpened(
          this.column.getOriginalColumnGroup(),
          expanded,
          'gridInitializing',
        );
      },
    ...
    if (!displayName) {
      ...
      while (
        ...
        columnGroup.getParent().getLeafColumns().length === leafCols.length
      ) {
  ...
  private afterHeaderCompCreated(
    displayName: string,
    headerGroupComp: IHeaderGroupComp,
  ): void {
  ...
  private setupMove(eHeaderGroup: HTMLElement, displayName: string): void {
    ...
    const allLeafColumns = this.column
    ...
    const dragSource: DragSource = {
      ...
      onDragStopped: () =>
        allLeafColumns.forEach((col) =>
          col.setMoving(false, 'uiColumnDragged'),
        ),
    };

    this.dragAndDropService.addDragSource(dragSource, true);
    this.addDestroyFunc(() =>
      this.dragAndDropService.removeDragSource(dragSource),
    );
  }
  ...
  public getDragItemForGroup(): DragItem {
    const allColumnsOriginalOrder = this.column
      .getOriginalColumnGroup()
      .getLeafColumns();

    // capture visible state, used when re-entering grid to dictate which columns should be visible
    const visibleState: { [key: string]: boolean } = {};
    allColumnsOriginalOrder.forEach(
    ...
    allColumnsOriginalOrder.forEach((column) =>
      allColumnsCurrentOrder.push(column),
    );
  ...
  private isSuppressMoving(): boolean {
    ...
    this.column.getLeafColumns().forEach((column: Column) => {
      if (
        column.getColDef().suppressMovable ||
        column.getColDef().lockPosition
      ) {
    ...
    if (!this.gridOptionsWrapper.isSuppressAutoSize()) {
      ...
      this.eHeaderCellResize.addEventListener(
        ...
        (event: MouseEvent) => {
          ...
          if (keys.length > 0) {
            this.columnController.autoSizeColumns(
              keys,
  ...
  public onResizeStart(shiftKey: boolean): void {
    ...
    this.resizeCols.forEach(
      (col) => (this.resizeStartWidth += col.getActualWidth()),
    );
    this.resizeRatios = [];
    this.resizeCols.forEach((col) =>
      this.resizeRatios.push(col.getActualWidth() / this.resizeStartWidth),
    );
    ...
    if (takeFromGroup) {
      ...
      this.resizeTakeFromCols = takeFromLeafCols.filter((col) =>
        col.isResizable(),
      );

      this.resizeTakeFromStartWidth = 0;
      this.resizeTakeFromCols.forEach(
        (col) => (this.resizeTakeFromStartWidth += col.getActualWidth()),
      );
      this.resizeTakeFromRatios = [];
      this.resizeTakeFromCols.forEach((col) =>
        this.resizeTakeFromRatios.push(
          col.getActualWidth() / this.resizeTakeFromStartWidth,
        ),
      );
    } else {
  ...
  public onResizing(finished: boolean, resizeAmount: any): void {
    ...
    this.columnController.resizeColumnSets(
      resizeSets,
      finished,
      'uiColumnDragged',
    );
