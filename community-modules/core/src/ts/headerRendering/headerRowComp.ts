...
export class HeaderRowComp extends Component {
  ...
  private getWidthForRow(): number {
    ...
    if (printLayout) {
      ...
      if (centerRow) {
        return (
          this.columnController.getContainerWidth(Constants.PINNED_RIGHT) +
  ...
  private getItemsAtDepth(): ColumnGroupChild[] {
    ...
    if (printLayout) {
      ...
      if (centerContainer) {
        ...
        [Constants.PINNED_LEFT, null, Constants.PINNED_RIGHT].forEach(
          (pinned) => {
    ...
    return this.columnController.getVirtualHeaderGroupRow(
      this.pinned,
      this.type == HeaderRowType.FLOATING_FILTER ? this.dept - 1 : this.dept,
    );
  }
  ...
  private onVirtualColumnsChanged(): void {
    ...
    if (ensureDomOrder) {
      const correctChildOrder = correctChildIds.map((id) =>
        this.headerComps[id].getGui(),
      );
  ...
  private createHeaderComp(
    ...
    switch (this.type) {
      case HeaderRowType.COLUMN:
        result = new HeaderWrapperComp(
          columnGroupChild as Column,
          this.dropTarget,
          this.pinned,
        );
        break;
      case HeaderRowType.COLUMN_GROUP:
        result = new HeaderGroupWrapperComp(
          columnGroupChild as ColumnGroup,
          this.dropTarget,
          this.pinned,
        );
        break;
      case HeaderRowType.FLOATING_FILTER:
        result = new FloatingFilterWrapper(
          columnGroupChild as Column,
          this.pinned,
        );
        break;
