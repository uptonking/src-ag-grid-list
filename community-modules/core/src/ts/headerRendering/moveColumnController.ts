...
export class MoveColumnController implements DropListener {
  ...
  public getIconName(): string {
    return this.pinned
      ? DragAndDropService.ICON_PINNED
      : DragAndDropService.ICON_MOVE;
  }
  ...
  public onDragEnter(draggingEvent: DraggingEvent): void {
    ...
    const dragCameFromToolPanel =
      draggingEvent.dragSource.type === DragSourceType.ToolPanel;
    ...
    if (dragCameFromToolPanel) {
      ...
      const visibleColumns: Column[] = columns.filter(
        (column) => visibleState[column.getId()],
      );
  ...
  private checkCenterForScrolling(xAdjustedForScroll: number): void {
    if (this.centerContainer) {
      ...
      const lastVisiblePixel =
        firstVisiblePixel + this.gridPanel.getCenterWidth();
  ...
  public onDragging(draggingEvent: DraggingEvent, fromEnter = false): void {
    ...
    const hDirectionNormalised = this.normaliseDirection(
      draggingEvent.hDirection,
    );
    ...
    this.attemptMoveColumns(
      dragSourceType,
      columnsToMove,
      hDirectionNormalised,
      mouseXNormalised,
      fromEnter,
    );
  }
  ...
  private calculateOldIndex(movingCols: Column[]): number {
    ...
    const indexes = _.sortNumerically(
      movingCols.map((col) => gridCols.indexOf(col)),
    );
  ...
  private attemptMoveColumns(
    ...
    for (let i = 0; i < validMoves.length; i++) {
      ...
      this.columnController.moveColumns(
        allMovingColumns,
        move,
        'uiColumnDragged',
      );
  ...
  private calculateValidMoves(
    ...
    const isMoveBlocked =
      ...
      movingCols.some((col) => col.getColDef().suppressMovable);

    if (isMoveBlocked) {
      return [];
    }
    // this is the list of cols on the screen, so it's these we use when comparing the x mouse position
    const allDisplayedCols = this.columnController.getDisplayedColumns(
      this.pinned,
    );
    ...
    const movingDisplayedCols = allDisplayedCols.filter((col) =>
      _.includes(movingCols, col),
    );
    const otherDisplayedCols = allDisplayedCols.filter(
      (col) => !_.includes(movingCols, col),
    );
    const otherGridCols = allGridCols.filter(
      (col) => !_.includes(movingCols, col),
    );
    ...
    if (draggingRight) {
      ...
      movingDisplayedCols.forEach(
        (col) => (widthOfMovingDisplayedCols += col.getActualWidth()),
      );
    ...
    if (draggingRight) {
      ...
      while (
        pointer <= lastIndex &&
        this.isColumnHidden(allDisplayedCols, displacedCol)
      ) {
        pointer++;
        validMoves.push(pointer);
    ...
    if (!this.movingIntervalId) {
      ...
      if (this.needToMoveLeft) {
        this.dragAndDropService.setGhostIcon(
          DragAndDropService.ICON_LEFT,
          true,
        );
      } else {
        this.dragAndDropService.setGhostIcon(
          DragAndDropService.ICON_RIGHT,
          true,
        );
      }
    }
  ...
  private moveInterval(): void {
    ...
    if (pixelsMoved !== 0) {
      ...
      const columnsThatCanPin = columns.filter(
        (c) => !c.getColDef().lockPinned,
      );

      if (columnsThatCanPin.length > 0) {
        this.dragAndDropService.setGhostIcon(DragAndDropService.ICON_PINNED);
        if (this.failedMoveAttempts > 7) {
          const pinType = this.needToMoveLeft
