...
export class HeaderWrapperComp extends AbstractHeaderWrapper {
  ...
  protected postConstruct(): void {
    ...
    const displayName = this.columnController.getDisplayNameForColumn(
      this.column,
      'header',
      true,
    );
    const enableSorting = colDef.sortable;
    const enableMenu = (this.menuEnabled =
      this.menuFactory.isMenuEnabled(this.column) && !colDef.suppressMenu);
    ...
    this.addManagedListener(
      this.column,
      Column.EVENT_FILTER_ACTIVE_CHANGED,
  ...
  protected handleKeyDown(e: KeyboardEvent): void {
    ...
    if (e.keyCode === Constants.KEY_SPACE) {
      ...
      if (
        ...
        !checkbox.getGui().contains(document.activeElement)
      ) {
  ...
  private appendHeaderComp(
    ...
    const params = {
      ...
      setSort: (sort: string, multiSort?: boolean) => {
        this.sortController.setSortForColumn(
          this.column,
          sort,
          !!multiSort,
          'uiColumnSorted',
  ...
  private setupMove(eHeaderCellLabel: HTMLElement, displayName: string): void {
    ...
    if (suppressMove && !colDef.enableRowGroup && !colDef.enablePivot) {
      return;
    }
    ...
    if (eHeaderCellLabel) {
      const dragSource: DragSource = {
        ...
        onDragStarted: () =>
          !suppressMove && this.column.setMoving(true, 'uiColumnMoved'),
        onDragStopped: () =>
          !suppressMove && this.column.setMoving(false, 'uiColumnMoved'),
      };
  ...
  private setupResize(): void {
    ...
    if (weWantAutoSize) {
      ...
      this.addManagedListener(
        touchListener,
        TouchListener.EVENT_DOUBLE_TAP,
        () => {
          this.columnController.autoSizeColumn(
            this.column,
            skipHeaderOnAutoSize,
