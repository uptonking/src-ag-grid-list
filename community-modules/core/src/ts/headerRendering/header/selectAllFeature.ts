...
export class SelectAllFeature extends BeanStub {
  ...
  constructor(cbSelectAll: AgCheckbox, column: Column) {
    ...
    this.filteredOnly = colDef
      ? !!colDef.headerCheckboxSelectionFilteredOnly
      : false;
  }
  ...
  private checkRightRowModelType(): void {
    ...
    const rowModelMatches =
      rowModelType === Constants.ROW_MODEL_TYPE_CLIENT_SIDE;

    if (!rowModelMatches) {
      console.warn(
        `ag-Grid: selectAllCheckbox is only available if using normal row model, you are using ${rowModelType}`,
      );
    }
  }
