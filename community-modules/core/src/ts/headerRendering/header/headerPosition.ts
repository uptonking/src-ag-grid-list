...
export class HeaderPositionUtils extends BeanStub {
  ...
  public findHeader(
    ...
    if (focusedHeader.column instanceof ColumnGroup) {
      ...
      nextColumn = this.columnController[getGroupMethod](
        focusedHeader.column as ColumnGroup,
      );
    } else {
      getColMethod = `getDisplayedCol${direction}` as any;
      nextColumn = this.columnController[getColMethod](
        focusedHeader.column as Column,
      );
    }
  ...
  public findColAtEdgeForHeaderRow(
    level: number,
    position: 'start' | 'end',
  ): HeaderPosition {
    const displayedColumns = this.columnController.getAllDisplayedColumns();
    const column =
      displayedColumns[position === 'start' ? 0 : displayedColumns.length - 1];
    ...
    if (type == HeaderRowType.COLUMN_GROUP) {
      const columnGroup = this.columnController.getColumnGroupAtLevel(
        column,
        level,
      );
      return {
