...
export class ValueService extends BeanStub {
  ...
  public getValue(
    ...
    const groupDataExists =
      rowNode.groupData && rowNode.groupData[colId] !== undefined;
    const aggDataExists =
      !ignoreAggData && rowNode.aggData && rowNode.aggData[colId] !== undefined;

    if (forFilter && colDef.filterValueGetter) {
      result = this.executeFilterValueGetter(
      ...
      result = this.executeValueGetter(
        colDef.valueGetter,
        data,
        column,
        rowNode,
  ...
  public setValue(
    ...
    if (
      _.missing(field) &&
      _.missing(newValueHandler) &&
      _.missing(valueSetter)
    ) {
    ...
    if (newValueHandler && _.exists(newValueHandler)) {
      ...
      valueWasDifferent = this.setValueUsingField(
        rowNode.data,
        field,
        newValue,
        column.isFieldContainsDots(),
      );
    }
