...
export class ColumnController extends BeanStub {
  ...
  private autoGroupColService: AutoGroupColService;
  ...
  private checkDisplayedVirtualColumns(): void {
    ...
    const hashBefore = this.allDisplayedVirtualColumns
      .map((column) => column.getId())
      .join('#');

    this.updateVirtualSets();

    const hashAfter = this.allDisplayedVirtualColumns
      .map((column) => column.getId())
      .join('#');
  ...
  public setVirtualViewportPosition(
    scrollWidth: number,
  ...
  public getSecondaryPivotColumn(
    pivotKeys: string[],
    valueColKey: Column | string,
  ): Column | null {
  ...
  private setFirstRightAndLastLeftPinned(source: ColumnEventType): void {
    ...
    if (this.gridOptionsWrapper.isEnableRtl()) {
      lastLeft = this.displayedLeftColumns
        ? this.displayedLeftColumns[0]
        : null;
      firstRight = this.displayedRightColumns
        ? _.last(this.displayedRightColumns)
        : null;
    } else {
      lastLeft = this.displayedLeftColumns
        ? _.last(this.displayedLeftColumns)
        : null;
      firstRight = this.displayedRightColumns
    ...
    for (let i = 0; i < displayedColumns.length; i++) {
      ...
      if (filterPasses) {
        if (result.length === 0 && lastConsideredCol) {
          const gapBeforeColumn = emptySpaceBeforeColumn
            ? emptySpaceBeforeColumn(col)
            : false;
          if (gapBeforeColumn) {
  ...
  public getAllDisplayedCenterVirtualColumnsForRow(rowNode: RowNode): Column[] {
    ...
    const emptySpaceBeforeColumn = (col: Column) =>
      col.getLeft() > this.viewportLeft;

    // if doing column virtualisation, then we filter based on the viewport.
    const filterCallback = this.suppressColumnVirtualisation
      ? null
  ...
  private isColumnInViewport(col: Column): boolean {
    ...
    const columnToMuchLeft =
      columnLeft < leftBounds && columnRight < leftBounds;
    const columnToMuchRight =
      columnLeft > rightBounds && columnRight > rightBounds;
  ...
  public addValueColumn(
    colKey: (string | Column) | null | undefined,
    source: ColumnEventType = 'api',
  ): void {
    if (colKey) {
  ...
  public setColumnWidths(
    ...
    columnWidths.forEach((columnWidth) => {
      ...
      const defaultIsShift =
        this.gridOptionsWrapper.getColResizeDefault() === 'shift';
  ...
  private checkMinAndMaxWidthsForSet(
    columnResizeSet: ColumnResizeSet,
  ): boolean {
  ...
  public resizeColumnSets(
    ...
    const passMinMaxCheck =
      !resizeSets ||
      resizeSets.every(this.checkMinAndMaxWidthsForSet.bind(this));
  ...
  public moveRowGroupColumn(
    ...
    const event: ColumnRowGroupChangedEvent = {
      ...
      column:
        this.rowGroupColumns.length === 1 ? this.rowGroupColumns[0] : null,
      api: this.gridApi,
      columnApi: this.columnApi,
      source: source,
    };
    ...
    this.columnUtils.depthFirstOriginalTreeSearch(
      ...
      (child) => {
        ...
        const marryChildren =
          ...
          columnGroup.getColGroupDef().marryChildren;
  ...
  public getDisplayedGroupAtDirection(
    columnGroup: ColumnGroup,
    direction: 'After' | 'Before',
  ): ColumnGroup | null {
    // pick the last displayed column in this group
    const requiredLevel =
      columnGroup.getOriginalColumnGroup().getLevel() +
      columnGroup.getPaddingLevel();
    const colGroupLeafColumns = columnGroup.getDisplayedLeafColumns();
    const col: Column | null =
      direction === 'After'
        ? _.last(colGroupLeafColumns)
        : colGroupLeafColumns[0];
    const getDisplayColMethod:
      | 'getDisplayedColAfter'
      | 'getDisplayedColBefore' = `getDisplayedCol${direction}` as any;
  ...
  private createStateItemFromColumn(column: Column): ColumnState {
    const rowGroupIndex = column.isRowGroupActive()
      ? this.rowGroupColumns.indexOf(column)
      : null;
    const pivotIndex = column.isPivotActive()
      ? this.pivotColumns.indexOf(column)
      : null;
  ...
  public getColumnState(): ColumnState[] {
    ...
    const primaryColumnState: ColumnState[] = this.primaryColumns.map(
      this.createStateItemFromColumn.bind(this),
    );

    const groupAutoColumnState: ColumnState[] = this.groupAutoColumns
  ...
  public setColumnState(
    ...
    this.rowGroupColumns.sort(
      this.sortColumnListUsingIndexes.bind(this, rowGroupIndexes),
    );
    this.pivotColumns.sort(
      this.sortColumnListUsingIndexes.bind(this, pivotIndexes),
    );
  ...
  private raiseColumnEvents(
    ...
    const raiseEventWithAllColumns = (
      eventType: string,
      idMapper: (cs: ColumnState) => string,
      columns: Column[],
    ) => {
      const unchanged = areEqual(
    ...
    const getChangedColumns = (
      changedPredicate: (cs: ColumnState, c: Column) => boolean,
    ): Column[] => {
    ...
    const valueColumnIdMapper = (cs: ColumnState) =>
      cs.colId + '-' + cs.aggFunc;
    raiseEventWithAllColumns(
      Events.EVENT_COLUMN_VALUE_CHANGED,
      valueColumnIdMapper,
      this.valueColumns,
    );

    const pivotColumnIdMapper = (cs: ColumnState) =>
      cs.colId + '-' + cs.pivotIndex;
    raiseEventWithAllColumns(
      Events.EVENT_COLUMN_PIVOT_CHANGED,
      pivotColumnIdMapper,
      this.pivotColumns,
    ...
    const rowGroupColumnIdMapper = (cs: ColumnState) =>
      cs.colId + '-' + cs.rowGroupIndex;
    raiseEventWithAllColumns(
      Events.EVENT_COLUMN_ROW_GROUP_CHANGED,
      rowGroupColumnIdMapper,
      this.rowGroupColumns,
    );

    // specific ColumnEvents which return what's changed
    const pinnedChangePredicate = (cs: ColumnState, c: Column) =>
      cs.pinned !== c.getPinned();
    this.raiseColumnPinnedEvent(
      getChangedColumns(pinnedChangePredicate),
      source,
    );

    const visibilityChangePredicate = (cs: ColumnState, c: Column) =>
      cs.hide === c.isVisible();
    const cols = getChangedColumns(visibilityChangePredicate);
    this.raiseColumnVisibleEvent(cols, source);

    const resizeChangePredicate = (cs: ColumnState, c: Column) =>
      cs.width !== c.getActualWidth();
    this.raiseColumnResizeEvent(
      getChangedColumns(resizeChangePredicate),
      source,
    );
  ...
  private getColumns(
    keys: (string | Column)[],
    columnLookupCallback: (key: string | Column) => Column,
  ): Column[] {
  ...
  public getColumnWithValidation(
  ...
  private getAutoColumn(key: string | Column): Column | null {
    ...
    return _.find(this.groupAutoColumns, (groupCol) =>
      this.columnsMatch(groupCol, key),
    );
  }
  ...
  public getDisplayNameForColumn(
    ...
    const headerName: string | null = this.getHeaderName(
      column.getColDef(),
      column,
      null,
      null,
  ...
  public getDisplayNameForOriginalColumnGroup(
    ...
    const colGroupDef = originalColumnGroup
      ? originalColumnGroup.getColGroupDef()
      : null;
  ...
  public getDisplayNameForColumnGroup(
    columnGroup: ColumnGroup,
    location: string,
  ): string | null {
    return this.getDisplayNameForOriginalColumnGroup(
      columnGroup,
      columnGroup.getOriginalColumnGroup(),
      location,
    );
  }
  ...
  public getColumnGroup(
    colId: string | ColumnGroup,
    instanceId?: number,
  ): ColumnGroup | null {
    if (!colId) {
      return null;
    ...
    this.columnUtils.depthFirstAllColumnTreeSearch(
      ...
      (child: ColumnGroupChild) => {
        if (child instanceof ColumnGroup) {
          ...
          if (checkInstanceId) {
            matched =
              colId === columnGroup.getGroupId() &&
              instanceId === columnGroup.getInstanceId();
          } else {
  ...
  private extractColumns(
    ...
    const colPresentInPrimaryFunc = (col: Column) =>
      this.primaryColumns.indexOf(col) >= 0;
    const colMissingFromPrimaryFunc = (col: Column) =>
      this.primaryColumns.indexOf(col) < 0;
  ...
  public setColumnGroupState(
    ...
    stateItems.forEach((stateItem) => {
      ...
      this.logger.log(
        'columnGroupOpened(' +
          originalColumnGroup.getGroupId() +
          ',' +
          newValue +
          ')',
      );
  ...
  public setColumnGroupOpened(
    key: OriginalColumnGroup | string | undefined,
    newValue: boolean,
    source: ColumnEventType = 'api',
  ): void {
    ...
    this.setColumnGroupState(
      [{ groupId: keyAsString, open: newValue }],
      source,
    );
  }

  public getOriginalColumnGroup(
    key: OriginalColumnGroup | string,
  ): OriginalColumnGroup | null {
    if (key instanceof OriginalColumnGroup) {
      return key;
    }
    ...
    if (this.pivotMode && !this.secondaryColumnsPresent) {
      ...
      columnsForDisplay = this.gridColumns.filter((column) => {
        const isAutoGroupCol =
          this.groupAutoColumns && _.includes(this.groupAutoColumns, column);
        const isValueCol =
          this.valueColumns && _.includes(this.valueColumns, column);
  ...
  private orderGridColsLikeLastPrimary(): void {
    ...
    const oldColsOrdered = this.lastPrimaryOrder.filter(
      (col) => this.gridColumns.indexOf(col) >= 0,
    );
    const newColsOrdered = this.gridColumns.filter(
      (col) => oldColsOrdered.indexOf(col) < 0,
    );
  ...
  private addToDisplayedColumns(
    displayedColumnTree: ColumnGroupChild[],
    displayedColumns: Column[],
  ): void {
  ...
  public getVirtualHeaderGroupRow(
    type: string,
    dept: number,
  ): ColumnGroupChild[] {
    ...
    const testGroup = (
      ...
      for (let i = 0; i < children.length; i++) {
        ...
        if (child instanceof Column) {
          ...
          addThisItem = testGroup(
            columnGroup.getDisplayedChildren(),
            result,
            dept + 1,
          );
        }
  ...
  public refreshFlexedColumns(
    updatedFlexViewportWidth?: number,
    source: ColumnEventType = 'flex',
    silent?: boolean,
  ): void {
    if (!this.flexActive) {
      return;
    ...
    const knownWidthColumns = this.displayedCenterColumns.filter(
      (col) => !col.getFlex(),
    );
    const flexingColumns = this.displayedCenterColumns.filter((col) =>
      col.getFlex(),
    );
    ...
    outer: while (true) {
      const totalFlex = flexingColumns.reduce(
        (count, col) => count + col.getFlex(),
        0,
      );
      spaceForFlexingColumns =
        this.flexViewportWidth - this.getWidthOfColsInList(knownWidthColumns);
      for (let i = 0; i < flexingColumns.length; i++) {
        const col = flexingColumns[i];
        const widthByFlexRule =
          (spaceForFlexingColumns * col.getFlex()) / totalFlex;
        let constrainedWidth: number;
        if (widthByFlexRule < col.getMinWidth()) {
          constrainedWidth = col.getMinWidth();
        } else if (
          col.getMaxWidth() != null &&
          widthByFlexRule > col.getMaxWidth()
        ) {
  ...
  public sizeColumnsToFit(
    ...
    while (!finishedResizing) {
      ...
      const availablePixels =
        gridWidth - this.getWidthOfColsInList(colsToNotSpread);
  ...
  private createGroupAutoColumnsIfNeeded(): void {
    ...
    const groupFullWidthRow = this.gridOptionsWrapper.isGroupUseEntireRow(
      this.pivotMode,
    );
    ...
    if (needAutoColumns) {
      const newAutoGroupCols = this.autoGroupColService.createAutoGroupColumns(
        this.rowGroupColumns,
      );
      const autoColsDifferent = !this.autoColsEqual(
        newAutoGroupCols,
        this.groupAutoColumns,
      );
  ...
  public hasFloatingFilters(): boolean {
    ...
    return (
      (defaultColDef != null && defaultColDef.floatingFilter === true) ||
      (this.columnDefs != null &&
        this.columnDefs.some((c: ColDef) => c.floatingFilter === true))
    );
  }
