...
export class AutoGroupColService extends BeanStub {
  ...
  public createAutoGroupColumns(rowGroupColumns: Column[]): Column[] {
    ...
    if (doingMultiAutoColumn) {
      rowGroupColumns.forEach((rowGroupCol: Column, index: number) => {
        groupAutoColumns.push(
          this.createOneAutoGroupColumn(rowGroupCol, index),
        );
      });
    } else {
  ...
  private createOneAutoGroupColumn(
    ...
    const userAutoColDef: ColDef =
      this.gridOptionsWrapper.getAutoGroupColumnDef();
    ...
    if (!this.gridOptionsWrapper.isTreeData()) {
      ...
      const noFieldOrValueGetter =
        _.missing(defaultAutoColDef.field) &&
        _.missing(defaultAutoColDef.valueGetter) &&
        _.missing(defaultAutoColDef.filterValueGetter);
  ...
  private generateDefaultColDef(rowGroupCol?: Column): ColDef {
    ...
    const userHasProvidedGroupCellRenderer =
      userAutoColDef &&
      (userAutoColDef.cellRenderer || userAutoColDef.cellRendererFramework);
