...
export class ColumnGroup implements ColumnGroupChild {
  ...
  public setLeft(left: number) {
    ...
    if (this.left !== left) {
      ...
      this.localEventService.dispatchEvent(
        this.createAgEvent(ColumnGroup.EVENT_LEFT_CHANGED),
      );
    }
  }
  ...
  public isChildInThisGroupDeepSearch(wantedChild: ColumnGroupChild): boolean {
    ...
    this.children.forEach((foundChild: ColumnGroupChild) => {
      ...
      if (foundChild instanceof ColumnGroup) {
        if (
          (foundChild as ColumnGroup).isChildInThisGroupDeepSearch(wantedChild)
        ) {
          result = true;
        }
      }
  ...
  public calculateDisplayedColumns() {
    ...
    this.localEventService.dispatchEvent(
      this.createAgEvent(ColumnGroup.EVENT_DISPLAYED_CHILDREN_CHANGED),
    );
  }
}
