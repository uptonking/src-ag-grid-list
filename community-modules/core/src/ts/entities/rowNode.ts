...
import {
  RowNodeCache,
  RowNodeCacheParams,
} from '../modules/rowNodeCache/rowNodeCache';
...
export class RowNode implements IEventEmitter {
  ...
  private checkRowSelectable() {
    ...
    this.setRowSelectable(
      shouldInvokeIsRowSelectable ? isRowSelectableFunc(this) : true,
    );
  }
    ...
    if (this.selectable !== newVal) {
      ...
      if (this.eventService) {
        this.eventService.dispatchEvent(
          this.createLocalRowEvent(RowNode.EVENT_SELECTABLE_CHANGED),
        );
      }
    }
  }
    ...
    if (getRowNodeId) {
      ...
      if (this.data) {
        ...
        if (
          ...
          console.error(
            `ag-Grid: Row ID's cannot start with ${RowNode.ID_PREFIX_ROW_GROUP}, this is a reserved prefix for ag-Grid's row grouping feature.`,
          );
        }
      } else {
  ...
  public setFirstChild(firstChild: boolean): void {
    ...
    if (this.eventService) {
      this.eventService.dispatchEvent(
        this.createLocalRowEvent(RowNode.EVENT_FIRST_CHILD_CHANGED),
      );
    }
  }
  ...
  public setLastChild(lastChild: boolean): void {
    ...
    if (this.eventService) {
      this.eventService.dispatchEvent(
        this.createLocalRowEvent(RowNode.EVENT_LAST_CHILD_CHANGED),
      );
    }
  }
  ...
  public setChildIndex(childIndex: number): void {
    ...
    if (this.eventService) {
      this.eventService.dispatchEvent(
        this.createLocalRowEvent(RowNode.EVENT_CHILD_INDEX_CHANGED),
      );
    }
  }
  ...
  public setRowTop(rowTop: number | null): void {
    ...
    if (this.eventService) {
      this.eventService.dispatchEvent(
        this.createLocalRowEvent(RowNode.EVENT_TOP_CHANGED),
      );
    }
  }
  ...
  public setDragging(dragging: boolean): void {
    ...
    if (this.eventService) {
      this.eventService.dispatchEvent(
        this.createLocalRowEvent(RowNode.EVENT_DRAGGING_CHANGED),
      );
    }
  }
  ...
  public setHighlighted(highlighted: 'above' | 'below' | null): void {
    ...
    if (this.eventService) {
      this.eventService.dispatchEvent(
        this.createLocalRowEvent(RowNode.EVENT_HIGHLIGHT_CHANGED),
      );
    }
  }
  ...
  public setAllChildrenCount(allChildrenCount: number | null): void {
    ...
    if (this.eventService) {
      this.eventService.dispatchEvent(
        this.createLocalRowEvent(RowNode.EVENT_ALL_CHILDREN_COUNT_CHANGED),
      );
    }
  }
  ...
  public setMaster(master: boolean): void {
    ...
    if (this.eventService) {
      this.eventService.dispatchEvent(
        this.createLocalRowEvent(RowNode.EVENT_MASTER_CHANGED),
      );
    }
  }
  ...
  public setRowHeight(
    ...
    if (this.eventService) {
      this.eventService.dispatchEvent(
        this.createLocalRowEvent(RowNode.EVENT_HEIGHT_CHANGED),
      );
    }
  }
  ...
  public setRowIndex(rowIndex: number): void {
    ...
    if (this.eventService) {
      this.eventService.dispatchEvent(
        this.createLocalRowEvent(RowNode.EVENT_ROW_INDEX_CHANGED),
      );
    }
  }
  ...
  public setUiLevel(uiLevel: number): void {
    ...
    if (this.eventService) {
      this.eventService.dispatchEvent(
        this.createLocalRowEvent(RowNode.EVENT_UI_LEVEL_CHANGED),
      );
    }
  }
  ...
  public setExpanded(expanded: boolean): void {
    ...
    if (this.eventService) {
      this.eventService.dispatchEvent(
        this.createLocalRowEvent(RowNode.EVENT_EXPANDED_CHANGED),
      );
    }

    const event = _.assign(
      {},
      this.createGlobalRowEvent(Events.EVENT_ROW_GROUP_OPENED),
      {
        expanded,
      },
    );
  ...
  public depthFirstSearch(callback: (rowNode: RowNode) => void): void {
    if (this.childrenAfterGroup) {
      this.childrenAfterGroup.forEach((child) =>
  ...
  public isRowPinned(): boolean {
    return (
      this.rowPinned === Constants.PINNED_TOP ||
      this.rowPinned === Constants.PINNED_BOTTOM
    );
  }
  ...
  public setSelectedParams(params: SetSelectedParams): number {
    ...
    const groupSelectsFiltered =
      groupSelectsChildren && params.groupSelectsFiltered === true;
    ...
    if (rangeSelect) {
      const newRowClicked =
        this.selectionController.getLastSelectedNode() !== this;
    ...
    if (!suppressFinishActions) {
      const clearOtherNodes =
        newValue &&
        (clearSelection || !this.gridOptionsWrapper.isRowSelectionMulti());
      if (clearOtherNodes) {
  ...
  public selectThisNode(newValue: boolean): boolean {
    ...
    if (this.eventService) {
      this.dispatchLocalEvent(
        this.createLocalRowEvent(RowNode.EVENT_ROW_SELECTED),
      );
    }

    const event: RowSelectedEvent = this.createGlobalRowEvent(
  ...
  private selectChildNodes(
    ...
    const children = groupSelectsFiltered
      ? this.childrenAfterFilter
  ...
  public onMouseEnter(): void {
    this.dispatchLocalEvent(
      this.createLocalRowEvent(RowNode.EVENT_MOUSE_ENTER),
    );
  }

  public onMouseLeave(): void {
    this.dispatchLocalEvent(
      this.createLocalRowEvent(RowNode.EVENT_MOUSE_LEAVE),
    );
  }
