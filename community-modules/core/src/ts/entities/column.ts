...
export class Column
  ...
  public initialise(): void {
    ...
    this.tooltipFieldContainsDots =
      _.exists(this.colDef.tooltipField) &&
  ...
  private validate(): void {
      ...
      rowGroupingItems.forEach((item) => {
        if (_.exists(colDefAny[item])) {
          if (ModuleRegistry.isPackageBased()) {
            warnOnce(
              `ag-Grid: ${item} is only valid in ag-grid-enterprise, your column definition should not have ${item}`,
              'ColumnRowGroupingMissing' + item,
            );
          } else {
            warnOnce(
              `ag-Grid: ${item} is only valid with ag-Grid Enterprise Module ${ModuleNames.RowGroupingModule} - your column definition should not have ${item}`,
              'ColumnRowGroupingMissing' + item,
            );
          }
        }
    ...
    if (!ModuleRegistry.isRegistered(ModuleNames.RichSelectModule)) {
      if (
        ...
        if (ModuleRegistry.isPackageBased()) {
          warnOnce(
            `ag-Grid: ${this.colDef.cellEditor} can only be used with ag-grid-enterprise`,
            'ColumnRichSelectMissing',
          );
        } else {
          warnOnce(
            `ag-Grid: ${this.colDef.cellEditor} can only be used with ag-Grid Enterprise Module ${ModuleNames.RichSelectModule}`,
            'ColumnRichSelectMissing',
          );
        }
      }
    ...
    if (!ModuleRegistry.isRegistered(ModuleNames.DateTimeCellEditorModule)) {
      if (
        this.colDef.cellEditor === 'agRichSelect' ||
        this.colDef.cellEditor === 'agDateTimeCellEditor'
      ) {
        if (ModuleRegistry.isPackageBased()) {
          warnOnce(
            `ag-Grid: ${this.colDef.cellEditor} can only be used with ag-grid-enterprise`,
          ...
          warnOnce(
            `ag-Grid: ${this.colDef.cellEditor} can only be used with ag-Grid Enterprise Module ${ModuleNames.DateTimeCellEditorModule}`,
            'ColumnDateTimeMissing',
          );
        }
      }
    ...
    if (this.gridOptionsWrapper.isTreeData()) {
      ...
      itemsNotAllowedWithTreeData.forEach((item) => {
        if (_.exists(colDefAny[item])) {
          warnOnce(
            `ag-Grid: ${item} is not possible when doing tree data, your column definition should not have ${item}`,
    ...
    if (colDefAny.floatingRendererFramework) {
      ...
      this.colDef.pinnedRowCellRendererFramework =
        colDefAny.floatingRendererFramework;
    }
    if (colDefAny.floatingRendererParams) {
      ...
      this.colDef.pinnedRowCellRendererParams =
        colDefAny.floatingRendererParams;
    }
  ...
  public setMoving(moving: boolean, source: ColumnEventType = 'api'): void {
    ...
    this.eventService.dispatchEvent(
      this.createColumnEvent(Column.EVENT_MOVING_CHANGED, source),
    );
  }
  ...
  public setSort(
    ...
    if (this.sort !== sort) {
      this.sort = sort;
      this.eventService.dispatchEvent(
        this.createColumnEvent(Column.EVENT_SORT_CHANGED, source),
      );
    }
  }

  public setMenuVisible(
    visible: boolean,
    source: ColumnEventType = 'api',
  ): void {
    if (this.menuVisible !== visible) {
      ...
      this.eventService.dispatchEvent(
        this.createColumnEvent(Column.EVENT_MENU_VISIBLE_CHANGED, source),
      );
    }
  }
  ...
  public setLeft(left: number | null, source: ColumnEventType = 'api') {
    ...
    if (this.left !== left) {
      this.left = left;
      this.eventService.dispatchEvent(
        this.createColumnEvent(Column.EVENT_LEFT_CHANGED, source),
      );
    }
  }
  ...
  public setFilterActive(
    ...
    if (this.filterActive !== active) {
      ...
      this.eventService.dispatchEvent(
        this.createColumnEvent(Column.EVENT_FILTER_ACTIVE_CHANGED, source),
      );
    }
    const filterChangedEvent = this.createColumnEvent(
  ...
  public setFirstRightPinned(
    ...
    if (this.firstRightPinned !== firstRightPinned) {
      ...
      this.eventService.dispatchEvent(
        this.createColumnEvent(Column.EVENT_FIRST_RIGHT_PINNED_CHANGED, source),
      );
    }
  }

  public setLastLeftPinned(
    ...
    if (this.lastLeftPinned !== lastLeftPinned) {
      ...
      this.eventService.dispatchEvent(
        this.createColumnEvent(Column.EVENT_LAST_LEFT_PINNED_CHANGED, source),
      );
    }
  }
  ...
  public setVisible(visible: boolean, source: ColumnEventType = 'api'): void {
    ...
    if (this.visible !== newValue) {
      ...
      this.eventService.dispatchEvent(
        this.createColumnEvent(Column.EVENT_VISIBLE_CHANGED, source),
      );
    }
  }
  ...
  public setActualWidth(
    ...
    if (this.actualWidth !== actualWidth) {
      ...
      this.eventService.dispatchEvent(
        this.createColumnEvent(Column.EVENT_WIDTH_CHANGED, source),
      );
    }
  }
  ...
  public setRowGroupActive(
    ...
    if (this.rowGroupActive !== rowGroup) {
      ...
      this.eventService.dispatchEvent(
        this.createColumnEvent(Column.EVENT_ROW_GROUP_CHANGED, source),
      );
    }
  }
  ...
  public setPivotActive(pivot: boolean, source: ColumnEventType = 'api'): void {
    if (this.pivotActive !== pivot) {
      ...
      this.eventService.dispatchEvent(
        this.createColumnEvent(Column.EVENT_PIVOT_CHANGED, source),
      );
    }
  }
  ...
  public setValueActive(value: boolean, source: ColumnEventType = 'api'): void {
    if (this.aggregationActive !== value) {
      ...
      this.eventService.dispatchEvent(
        this.createColumnEvent(Column.EVENT_VALUE_CHANGED, source),
      );
    }
  }
