...
export class OriginalColumnGroup
  ...
  public getColumnGroupShow(): string | undefined {
    return this.padding
      ? ColumnGroup.HEADER_GROUP_PADDING
  ...
  public setupExpandable() {
    ...
    this.getLeafColumns().forEach((col) =>
      col.addEventListener(
        ...
        this.onColumnVisibilityChanged.bind(this),
      ),
    );
  }
  ...
  public setExpandable() {
    ...
    for (let i = 0, j = children.length; i < j; i++) {
      ...
      if (headerGroupShow === ColumnGroup.HEADER_GROUP_SHOW_OPEN) {
        ...
        if (headerGroupShow === ColumnGroup.HEADER_GROUP_PADDING) {
          ...
          atLeastOneChangeable =
            atLeastOneChangeable ||
            column.children.some(
              (child) => child.getColumnGroupShow() !== undefined,
            );
        }
      }
    }
    ...
    const expandable =
      atLeastOneShowingWhenOpen &&
      atLeastOneShowingWhenClosed &&
      atLeastOneChangeable;
  ...
  public findChildren(): OriginalColumnGroupChild[] {
    ...
    while (
      children.length === 1 &&
      children[0] instanceof OriginalColumnGroup
    ) {
