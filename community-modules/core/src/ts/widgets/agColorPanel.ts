...
export class AgColorPanel extends Component {
  ...
  private static TEMPLATE /* html */ = `<div class="ag-color-panel">
  ...
  public setSpectrumValue(saturation: number, brightness: number) {
    ...
    if (valRect) {
      ...
      dragger.style.left =
        saturation * valRect.width - draggerRect.width / 2 + 'px';
      dragger.style.top =
        (1 - brightness) * valRect.height - draggerRect.height / 2 + 'px';
  ...
  public setValue(val: string) {
    ...
    this.spectrumHueSlider.style.left = `${
      (this.H - 1) * -spectrumHueRect.width
    }px`;
    this.spectrumAlphaSlider.style.left = `${
      this.A * spectrumAlphaRect.width
    }px`;
