...
export class PopupService extends BeanStub {
  ...
  public positionPopupForMenu(params: {
    ...
    const minWidth =
      params.ePopup.clientWidth > 0 ? params.ePopup.clientWidth : 200;
  ...
  private keepYWithinBounds(
    params: { ePopup: HTMLElement | null; minHeight?: number },
    y: number,
  ): number {
    const eDocument = this.gridOptionsWrapper.getDocument();
    ...
    let heightOfParent = isBody
      ? _.getAbsoluteHeight(docElement) + docElement!.scrollTop
      : parentRect.height;
    if (isBody) {
  ...
  private keepXWithinBounds(
    params: { minWidth?: number; ePopup: HTMLElement },
    x: number,
  ): number {
    const eDocument = this.gridOptionsWrapper.getDocument();
    const docElement = eDocument.documentElement;
    ...
    let widthOfParent = isBody
      ? _.getAbsoluteWidth(docElement!) + docElement!.scrollLeft
      : parentRect.width;
  ...
  public addPopup(
    ...
    const hidePopup = (params: PopupEventParams = {}) => {
      ...
      if (
        ...
        this.isEventSameChainAsOriginalEvent({
  ...
  private isEventFromCurrentPopup(
    ...
    const indexOfThisChild = _.findIndex(
      this.popupList,
      (popup) => popup.element === target,
    );
  ...
  public isElementWithinCustomPopup(el: HTMLElement): boolean {
    ...
    while (el && el !== document.body) {
      if (
        el.classList.contains('ag-custom-component-popup') ||
  ...
  public bringPopupToFront(ePopup: HTMLElement) {
    ...
    const popupList = Array.prototype.slice.call(
      parent.querySelectorAll('.ag-popup'),
    );
    ...
    if (onTopLength) {
      ...
      if (isPopupAlwaysOnTop) {
        if (pos !== popupLen - 1) {
          (_.last(alwaysOnTopList) as HTMLElement).insertAdjacentElement(
            'afterend',
            eWrapper,
          );
        }
      ...
      (_.last(popupList) as HTMLElement).insertAdjacentElement(
        'afterend',
        eWrapper,
      );
    }
