...
export class AgPanel extends Component {
  ...
  protected calculateMouseMovement(params: {
    ...
    let skipX =
      ...
      (parentRect.right <= e.clientX &&
        parentRect.right <= this.position.x + parentRect.left + width);
    ...
    const skipY =
      ...
      (parentRect.bottom <= e.clientY &&
        parentRect.bottom <= this.position.y + parentRect.top + height) ||
      (isTop &&
        // skip if we are moving to towards top and the cursor is
        // below the top anchor + topBuffer
        // note: topBuffer is used when moving the dialog using the title bar
        ((movementY < 0 &&
          e.clientY > this.position.y + parentRect.top + (topBuffer || 0)) ||
      ...
      (!isTop &&
        ...
        ((movementY < 0 &&
          e.clientY > this.position.y + parentRect.top + height) ||
  ...
  public setHeight(height: number | string) {
    ...
    if (typeof height === 'string' && height.indexOf('%') !== -1) {
      ...
      if (
        ...
        height + this.position.y > offsetParent.clientHeight
      ) {
  ...
  public setWidth(width: number | string) {
    ...
    if (typeof width === 'string' && width.indexOf('%') !== -1) {
      ...
      if (
        ...
        width + this.position.x > offsetParent.clientWidth
      ) {
  ...
  public setClosable(closable: boolean) {
    ...
    if (closable) {
      const closeButtonComp = (this.closeButtonComp = new Component(
      ...
      closeButtonComp.addManagedListener(
        eGui,
        'click',
        this.onBtClose.bind(this),
      );
