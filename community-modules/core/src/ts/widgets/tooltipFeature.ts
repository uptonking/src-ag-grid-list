...
export class TooltipFeature extends BeanStub {
  ...
  private postConstruct(): void {
    this.tooltipShowDelay =
      this.gridOptionsWrapper.getTooltipShowDelay() || 2000;
  ...
  public onMouseEnter(e: MouseEvent): void {
    ...
    const delay = this.isLastTooltipHiddenRecently()
      ? 200
      : this.tooltipShowDelay;
  ...
  private showTooltip(): void {
    ...
    const params: ITooltipParams = {
      ...
      rowIndex:
        this.parentComp.getCellPosition &&
        this.parentComp.getCellPosition().rowIndex,
      value: tooltipText,
    };
    ...
    // we disregard it
    const callback = this.newTooltipComponentCallback.bind(
      this,
      this.tooltipInstanceCount,
    );
  ...
  private newTooltipComponentCallback(
    tooltipInstanceCopy: number,
    tooltipComp: ITooltipComp,
  ): void {
    const compNoLongerNeeded =
      this.state !== TooltipStates.SHOWING ||
      this.tooltipInstanceCount !== tooltipInstanceCopy;
