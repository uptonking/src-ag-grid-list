...
export class SelectionController extends BeanStub {
  ...
  public clearOtherNodes(rowNodeToKeepSelected: RowNode): number {
    ...
    _.iterateObject(
      this.selectedNodes,
      (key: string, otherRowNode: RowNode) => {
  ...
  private syncInOldRowNode(rowNode: RowNode, oldNode: RowNode): void {
    const oldNodeHasDifferentId =
      _.exists(oldNode) && rowNode.id !== oldNode.id;
  ...
  public selectAllRowNodes(justFiltered = false) {
    if (this.rowModel.getType() !== Constants.ROW_MODEL_TYPE_CLIENT_SIDE) {
      throw new Error(
        `selectAll only available with normal row model, ie not ${this.rowModel.getType()}`,
      );
    }
