...
export class AnimationFrameService extends BeanStub {
  ...
  private verifyAnimationFrameOn(methodName: string): void {
    if (this.useAnimationFrame === false) {
      console.warn(
        `ag-Grid: AnimationFrameService.${methodName} called but animation frames are off`,
      );
    }
  }
  ...
  public createTask(
    task: () => void,
    index: number,
    list: 'createTasksP1' | 'createTasksP2',
  ) {
  ...
  private sortTaskList(taskList: TaskList) {
    ...
    taskList.list.sort((a, b) =>
      a.index !== b.index
        ? sortDirection * (b.index - a.index)
        : b.createOrder - a.createOrder,
    );
