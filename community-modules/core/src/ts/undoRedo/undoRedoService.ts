...
export class UndoRedoService extends BeanStub {
  ...
  public undo() {
    ...
    this.processAction(
      undoAction,
      (cellValueChange: CellValueChange) => cellValueChange.oldValue,
    );

    if (undoAction instanceof FillUndoRedoAction) {
      this.processRangeAndCellFocus(
        undoAction.cellValueChanges,
        undoAction.initialRange,
      );
    } else {
  ...
  public redo() {
    ...
    this.processAction(
      redoAction,
      (cellValueChange: CellValueChange) => cellValueChange.newValue,
    );

    if (redoAction instanceof FillUndoRedoAction) {
      this.processRangeAndCellFocus(
        redoAction.cellValueChanges,
        redoAction.finalRange,
      );
    } else {
    ...
    this.addManagedListener(
      ...
      () => {
        ...
        const shouldPushAction =
          !this.isRowEditing && !this.isPasting && !this.isFilling;
        if (shouldPushAction) {
          const action = new UndoRedoAction(this.cellValueChanges);
          this.pushActionsToUndoStack(action);
        }
  ...
  private addFillListeners(): void {
    ...
    this.addManagedListener(
      ...
      (event: FillEndEvent) => {
        const action = new FillUndoRedoAction(
