...
export class ComponentUtil {
  ...
  public static getEventCallbacks(): string[] {
    if (!ComponentUtil.EVENT_CALLBACKS) {
      ComponentUtil.EVENT_CALLBACKS = ComponentUtil.EVENTS.map((event) =>
  ...
  public static copyAttributesToGridOptions(
    ...
    component: any,
    skipEventDeprecationCheck: boolean = false,
  ): GridOptions {
    ...
    ComponentUtil.BOOLEAN_PROPERTIES.filter(keyExists).forEach(
      (key) => (pGridOptions[key] = ComponentUtil.toBoolean(component[key])),
    );

    ComponentUtil.NUMBER_PROPERTIES.filter(keyExists).forEach(
      (key) => (pGridOptions[key] = ComponentUtil.toNumber(component[key])),
    );
    ...
    if (!skipEventDeprecationCheck) {
      ComponentUtil.EVENTS
        ...
        .filter(
          (event) =>
            keyExists(event) ||
            keyExists(ComponentUtil.getCallbackForEvent(event)),
        )
  ...
  public static processOnChange(
    ...
    ComponentUtil.BOOLEAN_PROPERTIES.filter(keyExists).forEach(
      (key) =>
        (pGridOptions[key] = ComponentUtil.toBoolean(
    ...
    if (changes.enableCellTextSelection) {
      api.setEnableCellTextSelection(
        ComponentUtil.toBoolean(changes.enableCellTextSelection.currentValue),
      );
    }

    if (changes.showToolPanel) {
      api.showToolPanel(
        ComponentUtil.toBoolean(changes.showToolPanel.currentValue),
      );
    }
    ...
    if (changes.autoGroupColumnDef) {
      api.setAutoGroupColumnDef(
        changes.autoGroupColumnDef.currentValue,
        'gridOptionsChanged',
      );
    }
    ...
    if (changes.headerHeight) {
      api.setHeaderHeight(
        ComponentUtil.toNumber(changes.headerHeight.currentValue),
      );
    }

    if (changes.paginationPageSize) {
      api.paginationSetPageSize(
        ComponentUtil.toNumber(changes.paginationPageSize.currentValue),
      );
    }

    if (changes.pivotMode) {
      columnApi.setPivotMode(
        ComponentUtil.toBoolean(changes.pivotMode.currentValue),
      );
    }

    if (changes.groupRemoveSingleChildren) {
      api.setGroupRemoveSingleChildren(
        ComponentUtil.toBoolean(changes.groupRemoveSingleChildren.currentValue),
      );
    }

    if (changes.suppressRowDrag) {
      api.setSuppressRowDrag(
        ComponentUtil.toBoolean(changes.suppressRowDrag.currentValue),
      );
    }

    if (changes.suppressMoveWhenRowDragging) {
      api.setSuppressMoveWhenRowDragging(
        ComponentUtil.toBoolean(
          changes.suppressMoveWhenRowDragging.currentValue,
        ),
      );
    }

    if (changes.suppressRowClickSelection) {
      api.setSuppressRowClickSelection(
        ComponentUtil.toBoolean(changes.suppressRowClickSelection.currentValue),
      );
    }

    if (changes.gridAutoHeight) {
      api.setGridAutoHeight(
        ComponentUtil.toBoolean(changes.gridAutoHeight.currentValue),
      );
    }

    if (changes.suppressClipboardPaste) {
      api.setSuppressClipboardPaste(
        ComponentUtil.toBoolean(changes.suppressClipboardPaste.currentValue),
      );
    }
