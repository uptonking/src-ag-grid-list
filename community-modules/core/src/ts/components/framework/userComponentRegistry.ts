...
export type RegisteredComponentInput<A extends IComponent<any> & B, B> =
  ...
  | { new (): B };
export type AgGridRegisteredComponentInput<A extends IComponent<any>> =
  | AgGridComponentFunctionInput
  | { new (): A };
export type AgGridComponentFunctionInput = (
  params: any,
) => string | HTMLElement;
...
export class UserComponentRegistry extends BeanStub {
  ...
  public registerComponent<A extends IComponent<any>>(
    rawName: string,
    component: AgGridRegisteredComponentInput<A>,
  ) {
    const name: string = this.translateIfDeprecated(rawName);
    if (this.frameworkComponents[name]) {
      console.error(
        `Trying to register a component that you have already registered for frameworks: ${name}`,
      );
      return;
    }
   ...
   * A the agGridComponent interface (ie IHeaderComp). The final object acceptable by ag-grid
   */
  public registerFwComponent<A extends IComponent<any> & B, B>(
    rawName: string,
    component: { new (): IComponent<B> },
  ) {
    const name: string = this.translateIfDeprecated(rawName);
    if (this.jsComponents[name]) {
      console.error(
        `Trying to register a component that you have already registered for plain javascript: ${name}`,
      );
      return;
    }
   ...
   * A the agGridComponent interface (ie IHeaderComp). The final object acceptable by ag-grid
   */
  public retrieve<A extends IComponent<any> & B, B>(
    rawName: string,
    ...
    if (Object.keys(this.agGridDefaults).indexOf(name) < 0) {
      console.warn(
        `ag-Grid: Looking for component [${name}] but it wasn't found.`,
      );
    }
    return null;
  }
  ...
  private translateIfDeprecated(raw: string): string {
    ...
    if (deprecatedInfo != null) {
      _.doOnce(() => {
        console.warn(
          `ag-grid. Since v15.0 component names have been renamed to be namespaced. You should rename ${deprecatedInfo.propertyHolder}:${raw} to ${deprecatedInfo.propertyHolder}:${deprecatedInfo.newComponentName}`,
        );
