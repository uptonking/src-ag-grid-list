...
export class FilterService extends BeanStub {
  ...
  private filterNodes(filterActive: boolean, changedPath: ChangedPath): void {
    const filterCallback = (rowNode: RowNode, includeChildNodes: boolean) => {
      ...
      if (rowNode.hasChildren()) {
        ...
        if (filterActive && !includeChildNodes) {
          rowNode.childrenAfterFilter = rowNode.childrenAfterGroup.filter(
            (childNode) => {
              // a group is included in the result if it has any children of it's own.
              // by this stage, the child groups are already filtered
              const passBecauseChildren =
                childNode.childrenAfterFilter &&
                childNode.childrenAfterFilter.length > 0;

              // both leaf level nodes and tree data nodes have data. these get added if
              // the data passes the filter
              const passBecauseDataPasses =
                childNode.data &&
                this.filterManager.doesRowPassFilter(childNode);
    ...
    if (this.doingTreeDataFiltering()) {
      const treeDataDepthFirstFilter = (
        rowNode: RowNode,
        alreadyFoundInParent: boolean,
      ) => {
        ...
        if (rowNode.childrenAfterGroup) {
          for (let i = 0; i < rowNode.childrenAfterGroup.length; i++) {
            ...
            const foundInParent =
              alreadyFoundInParent ||
              this.filterManager.doesRowPassFilter(childNode);
            if (childNode.childrenAfterGroup) {
              treeDataDepthFirstFilter(
                rowNode.childrenAfterGroup[i],
                foundInParent,
              );
            } else {
      ...
      const treeDataFilterCallback = (rowNode: RowNode) =>
        treeDataDepthFirstFilter(rowNode, false);
      changedPath.executeFromRootNode(treeDataFilterCallback);
    } else {
      const defaultFilterCallback = (rowNode: RowNode) =>
        filterCallback(rowNode, false);
