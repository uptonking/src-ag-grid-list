...
export class SortService extends BeanStub {
  ...
  public sort(
    ...
    const callback = (rowNode: RowNode) => {
      ...
      this.pullDownGroupDataForHideOpenParents(
      ...
      if (sortActive) {
        ...
        rowNode.childrenAfterSort = sortedRowNodes.map(
          (sorted) => sorted.rowNode,
        );
      } else {
  ...
  private doFullSort(
    ...
    const sortedRowNodes: SortedRowNode[] = rowNode.childrenAfterFilter.map(
      this.mapNodeToSortedNode.bind(this),
    );
  ...
  private doDeltaSort(
    ...
    const cleanNodes: SortedRowNode[] = rowNode.childrenAfterSort
      .filter((rowNode) => {
        ...
        const passesChangedPathCheck =
          noAggregations || changedPath.canSkip(rowNode);
    ...
    cleanNodes.forEach(
      (sortedRowNode) =>
        (cleanNodesMapped[sortedRowNode.rowNode.id] = sortedRowNode.rowNode),
    );
  ...
  private compareRowNodes(
    sortOptions: any,
    sortedNodeA: SortedRowNode,
    sortedNodeB: SortedRowNode,
  ) {
    ...
    for (let i = 0, len = sortOptions.length; i < len; i++) {
      ...
      if (sortOption.column.getColDef().comparator) {
        ...
        comparatorResult = sortOption.column
          .getColDef()
          .comparator(valueA, valueB, nodeA, nodeB, isInverted);
      } else {
        //otherwise do our own comparison
        comparatorResult = _.defaultComparator(
          valueA,
          valueB,
