...
export class ZipContainer extends BeanStub {
  ...
  private buildFileStream(fData: string = ''): string {
    ...
    for (const currentFile of totalFiles) {
      const { fileHeader, folderHeader, content } = this.getHeader(
        currentFile,
        lL,
      );
  ...
  private getHeader(currentFile: ZipFile, offset: number) {
    ...
    let extraFields = '';
    if (isUTF8) {
      const uExtraFieldPath =
        decToHex(1, 1) + decToHex(this.getFromCrc32Table(utfPath), 4) + utfPath;
      extraFields =
        '\x75\x70' + decToHex(uExtraFieldPath.length, 2) + uExtraFieldPath;
    }
