  ...
  implements GridSerializingSession<T>
{
  ...
  public processHeaderCallback?: (
    params: ProcessHeaderForExportParams,
  ) => string;
  public processGroupHeaderCallback?: (
    params: ProcessGroupHeaderForExportParams,
  ) => string;
  public processRowGroupCallback?: (
  ...
  public prepare(columnsToExport: Column[]): void {
    this.firstGroupColumn = _.find(
      columnsToExport,
      (col) => !!col.getColDef().showRowGroup,
    );
  }
...
export class GridSerializer extends BeanStub {
  ...
  public serialize<T>(
    ...
    const rowSkipper: (params: ShouldRowBeSkippedParams) => boolean =
    ...
    if (_.existsAndNotEmpty(params.columnKeys)) {
      columnsToExport = this.columnController.getGridColumns(
        params.columnKeys!,
      );
      ...
      columnsToExport = columnsToExport.concat(
    ...
    if (isPivotMode) {
      ...
      if (params.onlySelectedAllPages || onlySelectedNonStandardModel) {
        ...
        if (rowModelNormal) {
          (this.rowModel as IClientSideRowModel).forEachNodeAfterFilterAndSort(
            processRow,
          );
        } else {
          this.rowModel.forEachNode(processRow);
  ...
  recursivelyAddHeaderGroups<T>(
    ...
    displayedGroups.forEach((columnGroupChild: ColumnGroupChild) => {
      ...
      columnGroup
        .getChildren()
        .forEach((it) => directChildrenHeaderGroups.push(it));
    });

    if (
      displayedGroups.length > 0 &&
      displayedGroups[0] instanceof ColumnGroup
    ) {
      this.doAddHeaderHeader(
        gridSerializingSession,
        displayedGroups,
        processGroupHeaderCallback,
      );
  ...
  private doAddHeaderHeader<T>(
    ...
    displayedGroups.forEach((columnGroupChild: ColumnGroupChild) => {
      ...
      if (processGroupHeaderCallback) {
        ...
        name = this.columnController.getDisplayNameForColumnGroup(
          columnGroup,
          'header',
        );
      }
...
type ProcessGroupHeaderCallback = (
  params: ProcessGroupHeaderForExportParams,
) => string;
