...
var AgGridReact = /** @class */ (function (_super) {
  ...
  AgGridReact.prototype.componentDidMount = function () {
    ...
    if (agGridColumn_1.AgGridColumn.hasChildColumns(this.props)) {
      gridOptions.columnDefs = agGridColumn_1.AgGridColumn.mapChildColumnDefs(
        this.props,
      );
    }
    this.gridOptions = core_1.ComponentUtil.copyAttributesToGridOptions(
      gridOptions,
      this.props,
    );
  ...
  AgGridReact.prototype.waitForInstance = function (
    reactComponent,
    resolve,
    runningTime,
  ) {
      ...
      if (runningTime >= AgGridReact.MAX_COMPONENT_CREATION_TIME) {
        console.error(
          "ag-Grid: React Component '" +
            reactComponent.getReactComponentName() +
            "' not created within " +
   ...
   */
  AgGridReact.prototype.mountReactPortal = function (
    portal,
    reactComponent,
    resolve,
  ) {
  ...
  AgGridReact.prototype.getStrategyTypeForProp = function (propKey) {
    if (propKey === 'rowData') {
      ...
      if (!!this.props.rowDataChangeDetectionStrategy) {
        ...
        return changeDetectionService_1.ChangeDetectionStrategyType
          .IdentityCheck;
      }
    }
  ...
  AgGridReact.prototype.processPropsChanges = function (prevProps, nextProps) {
    ...
    if (Object.keys(changes).length > 0) {
      core_1.ComponentUtil.processOnChange(
        changes,
        this.gridOptions,
        this.api,
        this.columnApi,
  ...
  AgGridReact.prototype.extractDeclarativeColDefChanges = function (
    nextProps,
    changes,
  ) {
    var debugLogging = !!nextProps.debug;
    if (agGridColumn_1.AgGridColumn.hasChildColumns(nextProps)) {
      var detectionStrategy = this.changeDetectionService.getStrategy(
        changeDetectionService_1.ChangeDetectionStrategyType.DeepValueCheck,
      );
      var currentColDefs = this.gridOptions.columnDefs;
      var newColDefs = agGridColumn_1.AgGridColumn.mapChildColumnDefs(
        nextProps,
      );
      if (!detectionStrategy.areEqual(currentColDefs, newColDefs)) {
        ...
        changes['columnDefs'] = {
          ...
          currentValue: agGridColumn_1.AgGridColumn.mapChildColumnDefs(
            nextProps,
          ),
        };
      }
  ...
  AgGridReact.prototype.extractGridPropertyChanges = function (
    prevProps,
    nextProps,
    changes,
  ) {
    ...
    changedKeys.forEach(function (propKey) {
      if (core_1.ComponentUtil.ALL_PROPERTIES.indexOf(propKey) !== -1) {
        var changeDetectionStrategy = _this.changeDetectionService.getStrategy(
          _this.getStrategyTypeForProp(propKey),
        );
        if (
...
var ReactFrameworkComponentWrapper = /** @class */ (function (_super) {
  ...
  ReactFrameworkComponentWrapper.prototype.createWrapper = function (
    UserReactComponent,
    componentType,
  ) {
    return new reactComponent_1.ReactComponent(
      UserReactComponent,
      this.agGridReact,
      componentType,
    );
