...
it('stateful component returns a valid component instance', () => {
  const instances = agGridReact.api.getCellRendererInstances({
    columns: ['age'],
  });
...
it('cell should be editable and editor component usable', async () => {
  ...
  await waitForAsyncCondition(
    () =>
      ...
      agGridReact.api.getCellEditorInstances().length > 0,
    5,
  ).then(
    () => null,
    () => fail('Editor instance not created within expected time'),
  );
  ...
  await waitForAsyncCondition(
    () =>
      ...
      agGridReact.api.getCellRendererInstances().length > 0,
    5,
  ).then(
    () => null,
    () => fail('Renderer instance not created within expected time'),
  );

  expect(component.render().find('.ag-cell-value').html()).toEqual(
    `<div class="ag-react-container"><span><div>Age: 50</div></span></div>`,
  );
});
